...
export const allLogGet = async (params = {}, header = {}) => {
    ...
    const { id } = params;
    const result = await get(manifest.api_url + '/instantNotification/log/' + `${id}`, {
        params: { ...params },
        headers
    })
    if (result.data.data) {
...
export const sendNotificationPost = async (data, headers = {}) => {
    ...
    const result = await post(manifest.api_url + '/instantNotification/instantSend', data,
        {
            params: data,
...
export const scheduledLogGet = async (params = {}, header = {}) => {
    ...
    const { id } = params;
    const result = await get(manifest.api_url + '/scheduleNotification/log/' + `${id}`, {
        params: { ...params },
        headers
    })
    if (result.data.data) {
...
export const sendScheduledNotificationSearch = async (params = {}, header = {}) => {
    let headers = {
        ...header,
        Authorization: 'Bearer ' + localStorage.getItem('token')
    }
