...
var fileUploader;
angular
  .module("org.ekstep.uploadfile-1.0", [])
  .controller("uploadfileController", [
    ...
    function ($scope, $injector, instance) {
      ...
      $scope.textbookService = ecEditor.getService(
        ServiceConstants.TEXTBOOK_SERVICE
      );
      $scope.showLoaderIcon = false;
      $scope.uploadBtn = true;
      $scope.loaderIcon = ecEditor.resolvePluginResource(
        "org.ekstep.uploadfile",
        "1.0",
        "editor/loader.gif"
      );
      ...
      $scope.$on("ngDialog.opened", function () {
        qq(document.getElementById("uploadTocFile")).attach(
          "click",
          function () {
              ...
              $scope.contentService.uploadDataToSignedURL(
                ...
                function (err, res) {
                  if (err) {
                    ...
                    $scope.textbookService.uploadFile(
                      ecEditor.getContext("contentId"),
                      fileUrl,
                      function (err, res) {
                        if (err) {
                          ...
                          $scope.generateTelemetry({
                            id: "updatetoc",
                            subtype: "toc_updated_successful",
                          });
