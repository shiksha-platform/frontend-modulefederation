...
org.ekstep.mathtext.EditorPlugin = org.ekstep.contenteditor.basePlugin.extend({
  ...
  initialize: function () {
    ...
    ecEditor.addEventListener(
      "org.ekstep.mathtext:showpopup",
      this.loadHtml,
      this
    );
    ecEditor.addEventListener(
      "org.ekstep.mathtext:changeConfig",
      this.onConfigChange,
      this
    );
  ...
  loadHtml: function (event, data) {
    ...
    if (document.getElementsByClassName("mathtextEditor_1").length > 0) {
      return;
  ...
  newInstance: function () {
    ...
    if (ecEditor._.isUndefined(this.config.text)) {
      this.config.latex = ecEditor._.isUndefined(this.attributes.latex)
        ? ""
        : this.attributes.latex;
  ...
  resizeObject: function (e) {
    if (
      ...
      ecEditor.getCurrentObject().manifest.id == "org.ekstep.mathtext"
    ) {
  ...
  addDivElement: function (event, instance) {
    ...
    div.style.width = instance.editorObj.width
      ? instance.editorObj.width + 1 + "px"
      : "auto";
    div.style.height = instance.editorObj.height
      ? instance.editorObj.height + 1 + "px"
      : "auto";
    div.style.pointerEvents = "none";
    ecEditor.jQuery(".canvas-container #" + this.mathTextId).append(div);
    ecEditor.jQuery("#" + instance.id).offset({
      top: instance.editorObj.top + canvasCord.top,
      left:
        Number(
          parseInt(ecEditor.jQuery(".canvas-container").css("margin-left"))
        ) +
        (instance.editorObj.left + canvasCord.left),
    });
  ...
  editMathText: function (event, data) {
    ecEditor
      .jQuery(".canvas-container")
      .find("#" + data.instanceId)
      .html("");
  ...
  dblClickHandler: function (event) {
    ...
    if (
      ...
      currentObject &&
      currentObject.manifest.id === "org.ekstep.mathtext"
    ) {
