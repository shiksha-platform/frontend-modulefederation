...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  getOnClick: function () {
    return {
      id: "stage:select",
      data: { stageId: this.id, prevStageId: ecEditor.getCurrentStage().id },
    };
  },
  ...
  addChild: function (plugin) {
    ...
    if (plugin.editorObj) {
      ...
      if (plugin.editorObj.selectable)
    ...
    if (org.ekstep.contenteditor.stageManager.contentLoading) {
      ...
      html2canvas($("#canvas-wrapper"), {
        onrendered: function (canvas) {
          ...
          org.ekstep.contenteditor.stageManager.thumbnails[stage.id] =
            stage.thumbnail;
  ...
  updateZIndex: function () {
    ...
    ecEditor._.forEach(this.children, function (child) {
      if (child.editorObj) {
        child.attributes["z-index"] = instance.canvas
          .getObjects()
          .indexOf(child.editorObj);
      } else {
        child.attributes["z-index"] = _.isUndefined(child.attributes["z-index"])
          ? instance.canvas.getObjects().length
          : child.attributes["z-index"];
      }
    });
  },
  render: function (canvas) {
    ...
    this.children = ecEditor._.sortBy(this.children, [
      function (o) {
        return o.getAttribute("z-index");
      },
    ]);
    ...
    if (org.ekstep.contenteditor.stageManager.contentLoading) {
      ...
      html2canvas($("#canvas-wrapper"), {
        onrendered: function (canvas) {
          instance.thumbnail = canvas.toDataURL({
            format: "jpeg",
            quality: 0.1,
          });
          var angScope = ecEditor.getAngularScope();
  ...
  objectSelected: function (event, data) {
    if (
      !ecEditor._.isUndefined(this.currentObject) &&
      this.currentObject !== data.id
    ) {
      var plugin = ecEditor.getPluginInstance(this.currentObject);
      ecEditor
        .getCanvas()
        .trigger("selection:cleared", { target: plugin.editorObj });
    }
    this.currentObject = data.id;
  },
  ...
  setSlideBackground: function (color) {
    ...
    if (!shapeInstance) {
      if (color !== "#FFFFFF")
        ecEditor.instantiatePlugin(
          "org.ekstep.shape",
          {
            type: "rect",
            subtype: "slidebackground",
            x: 0,
            y: 0,
            fill: color,
            w: 100,
