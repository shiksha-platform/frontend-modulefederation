...
angular.module("org.ekstep.breadcrumb", []).controller("breadcrumbController", [
  "$scope",
  "$location",
  function ($scope, $location) {
    ...
    $scope.getPath = function () {
      $scope.path = [];
      var showFolder;
      var activeNode = ecEditor
        .jQuery($scope.collectionTreeId)
        .fancytree("getTree")
        .getActiveNode();
      ...
      if (activeNode.folder) {
        ...
        if (parentNode)
          $scope.path = [{ title: "...", nodeId: parentNode.key, show: true }];
        $scope.getBreadcrumb(path, showFolder);
      }
      if (
        ecEditor
    ...
    $scope.hideBreadcrumb = function () {
      $scope.path.forEach(function (data, index) {
        if (index < $scope.path.length - 4 && index != 0) data.show = false;
        else data.show = true;
      });
    };
      ...
      _.forEach(path.split("/"), function (key) {
        if (key) {
          var node = ecEditor
            ...
            .getNodeByKey(key);
          if (
            (showFolder || (!showFolder && !node.folder)) &&
            (node.data.objectType == "Collection" || node.getLevel() !== 1)
          ) {
            var title = node.title;
            $scope.path.push({
              title: title.replace(
                /[^\u0600-\u06FF\uFB50-\uFDFF\uFE70-\uFEFF\uFB50-\uFDFF\u0980-\u09FF\u0900-\u097F\u0D00-\u0D7F\u0A80-\u0AFF\u0C80-\u0CFF\u0B00-\u0B7F\u0A00-\u0A7F\u0B80-\u0BFF\u0C00-\u0C7F\w:&_\-.(\),\/\s]/g,
                ""
              ),
              nodeId: node.key,
              show: true,
    ...
    $scope.setActiveNode = function (data) {
      ...
      if (data.nodeId) {
        ...
        activeNode
          ...
          : "";
        nodeKey =
          data.nodeId === activeNode.key ? data.nodeId : activeNode.parent.key;
    ...
    $scope.updateCollectionBreadcrumb = function (data) {
      ...
      if (index > -1) {
        $scope.path = ecEditor._.dropRight(
          $scope.path,
    ...
    ecEditor.addEventListener(
      "org.ekstep.collectioneditor:breadcrumb",
      $scope.getPath,
      $scope
    );
    ecEditor.addEventListener(
      "org.ekstep.collectioneditor:addToBreadcrumb",
      $scope.addToBreadcrumb,
      $scope
    );
  },
]);
