org.ekstep.contenteditor.basePlugin.extend({
  ...
  telemetryService: org.ekstep.contenteditor.api.getService(
  ...
  popupService: org.ekstep.contenteditor.api.getService(
    ServiceConstants.POPUP_SERVICE
  ),
  ...
  initialize: function () {
    ...
    ecEditor.addEventListener(
      "org.ekstep.contenteditor:save:meta",
      this.saveMetadata,
      this
    );
    ecEditor.addEventListener(
      "org.ekstep.contenteditor:preview",
      function (event, data) {
    ...
    ecEditor.addEventListener(
      "org.ekstep.contenteditor:save:force",
      this.saveBrowserContent,
      this
    );
    ecEditor.addEventListener(
    ...
    ecEditor.addEventListener(
      "org.ekstep.contenteditor:publish",
      this.publishContent,
      this
    );
    ecEditor.addEventListener(
      "org.ekstep.contenteditor:reject",
      this.rejectContent,
      this
    );
    ecEditor.addEventListener(
      "org.ekstep.contenteditor:acceptFlag",
      this.acceptContentFlag,
      this
    );
    ecEditor.addEventListener(
      "org.ekstep.contenteditor:discardFlag",
      this.discardContentFlag,
      this
    );
    ecEditor.addEventListener(
      "org.ekstep.contenteditor:retire",
      this.retireContent,
      this
    );
    ecEditor.addEventListener(
      "org.ekstep.contenteditor:unlistedPublish",
      this.unlistedPublishContent,
      this
    );
    ecEditor.addEventListener(
      "org.ekstep.contenteditor:Unauthorized",
      this.unauthorizedToken,
      this
    );
  },
  unauthorizedToken: function () {
    this.popUpValues.headerMsg =
      "Your session has timed out due to inactivity. Please login to resume!";
  ...
  reviewContent: function (event, callback) {
    ...
    ecEditor
      ...
      .sendForReview(
        { contentId: contentId, channel: ecEditor.getContext("channel") },
        function (err, res) {
  ...
  resolveSaveFn: function (event, data) {
    ...
    switch (meta.mimeType) {
      ...
      default:
        data.callback &&
          data.callback("unable to resolve save call for the mimetype");
        break;
    }
  },
  saveGenericEditorContent: function (event, data) {
    var options = ecEditor._.assign(
      { savingPopup: true, successPopup: true, failPopup: true },
      data
    );
  ...
  saveContent: function (event, options) {
    ...
    org.ekstep.pluginframework.eventManager.dispatchEvent(
      "content:before:save"
    );
    // TODO: Show saving dialog
    var contentBody = org.ekstep.contenteditor.stageManager.toECML();
    if (
      typeof org.ekstep.contenteditor.stageManager.updateStageIcons ===
      "function"
    ) {
      // safe to use the function
      contentMeta.stageIcons = JSON.stringify(
        org.ekstep.contenteditor.stageManager.updateStageIcons()
      );
    } else {
      contentMeta.stageIcons = JSON.stringify(
        org.ekstep.contenteditor.stageManager.getStageIcons()
      );
    }
    ...
    if (summary) {
      ...
      var metaData = ecEditor
        .getService(ServiceConstants.CONTENT_SERVICE)
        .getContentMeta(ecEditor.getContext("contentId"));
      if (
        !ecEditor._.isEqual(
          ecEditor._.map(summary.questions, "identifier").sort(),
          ecEditor._.map(metaData.questions, "identifier").sort()
        )
      ) {
        contentMeta.questions = summary.questions;
      }
    }
    contentMeta.assets = ecEditor._.compact(
      org.ekstep.contenteditor.stageManager.assets
    );
      ...
      function (err, res) {
        if (err) {
          ...
          var meta = ecEditor
            ...
            .getContentMeta(ecEditor.getContext("contentId"));
          if (
            meta.mimeType === "application/vnd.ekstep.content-collection" &&
            org.ekstep.services.stateService.state.dialCodeMap
          ) {
            var dialCode = _.get(
              org.ekstep.services.stateService.state.dialCodeMap,
            ...
            if (!_.isUndefined(dialCode)) {
              instance.dialcodeLink([
                {
                  dialcode: dialCode,
                  identifier: ecEditor.getContext("contentId"),
                },
              ]);
              instance.storeDialCodes(
                ecEditor.getContext("contentId"),
                dialCode
              );
              _.unset(
  ...
  patchContent: function (metadata, body, cb, options) {
    var instance = this;
    if (
      org.ekstep.contenteditor.migration &&
      org.ekstep.contenteditor.migration.isMigratedContent()
    ) {
      if (!metadata) metadata = {};
      metadata.oldContentBody =
        org.ekstep.contenteditor.migration.getBackupContent();
      var migrationPopupCb = function (err, res) {
        if (res)
          instance.contentService.saveContent(
  ...
  showMigratedContentSaveDialog: function (callback) {
    ...
    this.popupService.open({
      template: ecEditor.resolvePluginResource(
        ...
        "editor/partials/migratedContentSaveMsg.html"
      ),
      controller: [
        "$scope",
        function ($scope) {
  ...
  changePopupValues: function (message) {
    if (message === "success") {
      ...
      this.popUpValues.showCloseButton = true;
      this.popUpValues.saveNotificationCloseButton =
        "saveSuccessNotificationCloseButton";
    } else if (message === "error") {
      this.popUpValues.headerMsg = "Failed to save Content";
      this.popUpValues.popUpIcon = "circle remove red";
      this.popUpValues.showCloseButton = true;
      this.popUpValues.saveNotificationCloseButton =
        "saveFailNotificationCloseButton";
    }
    if (message === "saving") {
  ...
  showConflictDialog: function (options) {
    ...
    this.popupService.open({
      template: ecEditor.resolvePluginResource(
        instance.manifest.id,
        instance.manifest.ver,
        "editor/partials/conflictDialog.html"
      ),
      controller: [
        "$scope",
        function ($scope) {
          ...
          if (instance.conflictdialogOptions.showPlatformPreview)
            $scope.showPlatformPreview = true;
          ...
          $scope.firetelemetry = function (menu, menuType) {
            instance.telemetryService.interact({
              id: "button",
              type: "click",
              subtype: "popup",
              target: menuType,
  ...
  previewContent: function (fromBeginning) {
    ...
    org.ekstep.pluginframework.eventManager.dispatchEvent("atpreview:show", {
      contentBody: org.ekstep.contenteditor.stageManager.toECML(),
  ...
  previewPlatformContent: function () {
    ...
    this.fetchPlatformContentBody(function (platformContentBody) {
      org.ekstep.pluginframework.eventManager.dispatchEvent("atpreview:show", {
        contentBody: platformContentBody,
        currentStage: true,
      });
    });
  },
        ...
        function (err, content) {
          ...
          if (content && content.body) {
            try {
              ...
              alert(
                "Failed to parse body from platform. Please report an issue."
              );
  ...
  publishContent: function (event, data) {
    var contentId = ecEditor.getContext("contentId");
    ecEditor.getService(ServiceConstants.CONTENT_SERVICE).publishContent(
      {
        contentId: contentId,
  ...
  rejectContent: function (event, data) {
    ecEditor.getService(ServiceConstants.CONTENT_SERVICE).rejectContent(
      {
  ...
  saveCollectionContent: function (event, data) {
    ...
    var contentMetaData = ecEditor
      .getService(ServiceConstants.CONTENT_SERVICE)
      .getContentMeta(ecEditor.getContext("contentId"));

    if (contentMetaData && contentMetaData.versionKey) {
      if (
        org.ekstep.collectioneditor.cache.nodesModified[
        ...
        org.ekstep.collectioneditor.cache.nodesModified[
          contentMetaData.identifier
        ].metadata.versionKey = contentMetaData.versionKey;
      }
    }

    var contentBody = org.ekstep.collectioneditor.api
    ...
    if (_.size(nodes.invalidNodes) > 0) {
      ...
      data.callback &&
        data.callback("mandatory fields are missing in the data!");
      return false;
    }
    ecEditor
      .getService(ServiceConstants.CONTENT_SERVICE)
      .saveCollectionHierarchy({ body: contentBody }, function (err, res) {
        if (res && res.data && res.data.responseCode == "OK") {
          if (data.showNotification)
            ecEditor.dispatchEvent("org.ekstep.toaster:success", {
  ...
  highlightNodeForInvalidDialCode: function (res, nodesModified) {
    ...
    var mapArr = [];
    ecEditor._.forEach(
      org.ekstep.services.stateService.state.invaliddialCodeMap,
      function (value, key) {
        if (_.has(res.data.result.identifiers, key)) {
          delete org.ekstep.services.stateService.state.invaliddialCodeMap[key];
          org.ekstep.services.stateService.setState(
            "invaliddialCodeMap",
            res.data.result.identifiers[key],
            value
          );
          org.ekstep.services.collectionService.highlightNode(
            res.data.result.identifiers[key]
          );
    ...
    ecEditor._.forEach(
      ...
      function (value, key) {
        if (_.has(res.data.result.identifiers, key)) {
          ...
          mapArr.push({
            identifier: res.data.result.identifiers[key],
            dialcode: value,
          });
          org.ekstep.services.collectionService.lowLightNode(
            res.data.result.identifiers[key]
          );
          instance.storeDialCodes(res.data.result.identifiers[key], value);
        } else {
        ...
        .dialcodeLink(
          ...
          function (err, rep) {
            if (!err) {
              ...
              if (
                !ecEditor._.isUndefined(err.responseJSON) &&
                err.responseJSON.params &&
                err.responseJSON.params.err == "ERR_DIALCODE_LINK"
              ) {
                ecEditor.dispatchEvent("org.ekstep.toaster:error", {
  ...
  storeDialCodes: function (nodeId, dialCode) {
    var node = ecEditor
      .getService(ServiceConstants.COLLECTION_SERVICE)
      .getNodeById(nodeId);
    if (node && node.data) node.data.metadata["dialcodes"] = dialCode;
  },
  ...
  unlistedPublishContent: function (event, data) {
    ...
    ecEditor
      .getService(ServiceConstants.CONTENT_SERVICE)
      .unlistedPublishContent({ contentId: contentId }, function (err, res) {
        if (res && res.data && res.data.responseCode == "OK") {
          ecEditor.dispatchEvent("org.ekstep.toaster:success", {
