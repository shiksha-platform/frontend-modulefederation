  ...
  initialize: function () {
    ...
    ecEditor.registerKeyboardCommand("alt+mod+shift+n", function (event) {
      ...
      if (config.mode === "Edit") {
        ...
        ecEditor
          .jQuery(ecEditor.jQuery("#collection-tree"))
          .trigger("nodeCommand", { cmd: "addSibling" });
        return false;
      }
    });
    ...
    ecEditor.registerKeyboardCommand(
      ["alt+ctrl+n", "alt+command+`"],
      function (event) {
        var config = org.ekstep.services.collectionService.config;
        if (config.mode === "Edit") {
          event.preventDefault();
          ecEditor
            .jQuery(ecEditor.jQuery("#collection-tree"))
            .trigger("nodeCommand", { cmd: "addChild" });
          return false;
        }
      }
    );
    ...
    ecEditor.registerKeyboardCommand(
      ...
      function (event) {
        ...
        if (config.mode === "Edit") {
          ...
          ecEditor
            .jQuery(ecEditor.jQuery("#collection-tree"))
            .trigger("nodeCommand", { cmd: "remove" });
          return false;
        }
      }
    );
    ...
    ecEditor.registerKeyboardCommand("mod+/", function (event) {
      ...
      if (config.mode === "Edit") {
        ...
        ecEditor
          .jQuery(ecEditor.jQuery("#collection-tree"))
          .trigger("nodeCommand", { cmd: "showMenu" });
        return false;
      }
    });
  },
