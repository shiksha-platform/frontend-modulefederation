...
var fileUploader;
angular
  .module("org.ekstep.uploadcontent-1.5", [])
  .controller("uploadController", [
    ...
    function ($scope, $injector, instance) {
      ...
      $scope.validateYoutubeURL = function (fileName) {
        ...
        if (
          (hostName && hostName[2] === "youtube.com") ||
          hostName[2] === "youtu.be"
        ) {
          return true;
        }
      ...
      $scope.upload = function () {
        ...
        if (!mimeType) {
          ecEditor.dispatchEvent("org.ekstep.toaster:error", {
            message:
              "Invalid content type (supported type: pdf, epub, h5p, mp4, youtube, html-zip, webm, whitelisted-domain)",
            position: "topCenter",
            icon: "fa fa-warning",
          });
          $scope.showLoader(false);
              ...
              if (err) {
                ecEditor.dispatchEvent("org.ekstep.toaster:error", {
                  message:
                    ...
                    err.responseJSON.params.errmsg
                      ? err.responseJSON.params.errmsg
          ...
          var data = {
            request: {
              content: {
                ...
                organisation: ecEditor._.values(
                  ecEditor.getContext("user").organisations
                ),
              },
            },
          ...
          $scope.contentService.createContent(data, function (err, res) {
            if (err) {
              ...
              ecEditor
                ...
                .createLock(lockRequest, function (err, lockRes) {
                  if (err) {
                    ecEditor.dispatchEvent("org.ekstep.toaster:error", {
