org.ekstep.contenteditor.basePlugin.extend({
  initialize: function () {
    ecEditor.addEventListener(
      "org.ekstep.developer:loadplugin",
      this.loadPlugin,
      this
    );
    ecEditor.addEventListener(
      "org.ekstep.developer:getPlugins",
      this.listPlugins,
      this
    );
    ecEditor.addEventListener(
      "org.ekstep.developer:updateLocalServerPath",
      this.updateLocalServerPath,
      this
    );

    var scope = ecEditor.getAngularScope();
    scope.localServerPath =
      scope.localServerPath || org.ekstep.pluginframework.hostRepo.basePath;
  ...
  loadPlugin: function (event, data) {
    ...
    org.ekstep.contenteditor.api.loadAndInitPlugin(
      pluginId,
      pluginVer,
      new Date().getTime()
    );
