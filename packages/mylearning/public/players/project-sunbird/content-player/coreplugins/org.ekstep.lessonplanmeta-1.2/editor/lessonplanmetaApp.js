angular
  .module("lessonplanmetaApp", ["Scope.safeApply"])
  .controller("lessonplanmetaController", [
    "$scope",
    "$timeout",
    function ($scope, $timeout) {
      ...
      ecEditor.getService("meta").getConfigOrdinals(function (err, resp) {
        if (!err) {
          ...
          $scope.subjectList = _.uniq(
            _.union(
              _.clone(resp.data.result.ordinals.language),
              $scope.defaultSubjectList
            )
          );
      ...
      $scope.initDropdown = function () {
        setTimeout(function () {
          ...
          $("#lessonplan-medium").dropdown(
            "set selected",
            $scope.lesson.medium
          );
          $("#lessonplan-subject").dropdown(
            "set selected",
            $scope.lesson.subject
          );
          $("#lessonplan-gradeLevel").dropdown(
            "set selected",
            $scope.lesson.gradeLevel
          );
          $("#lessonplan-language").dropdown(
            "set selected",
            $scope.lesson.language
          );
        }, 1000);
      };
      ...
      $scope.updateNode = function () {
        if (!_.isEmpty($scope.nodeId) && !_.isUndefined($scope.nodeId)) {
          ...
          if (
            !_.isEmpty($scope.lesson.language) &&
            _.isString($scope.lesson.language)
          ) {
            $scope.lesson.language = [$scope.lesson.language];
          }
          $scope.lesson.duration = $scope.duration
            ? $scope.duration.toString()
            : "0";
          $scope.lesson.learningObjective = $scope.learningObjective
            ? [$scope.learningObjective]
            : [];
          ...
          org.ekstep.collectioneditor.cache.nodesModified[
            ...
            org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
              .metadata,
            $scope.getUpdatedMetadata($scope.metadataCloneObj, $scope.lesson)
          );
      ...
      $scope.onNodeSelect = function (evant, data) {
        ...
        if (
          ...
          if (!_.isUndefined(activeNode.data.metadata.concepts)) {
            ...
            if ($scope.lesson.concepts.length > 0) {
              $scope.lesson.conceptData =
                "(" + $scope.lesson.concepts.length + ") concepts selected";
          ...
          $scope.lesson.duration = activeNode.data.metadata.duration
            ? parseInt(activeNode.data.metadata.duration)
            : "0";
          if (activeNode.data.metadata.learningObjective)
            $scope.learningObjective =
              activeNode.data.metadata.learningObjective[0];
      ...
      ecEditor.addEventListener(
        "org.ekstep.collectioneditor:node:selected:LessonPlan",
        $scope.onNodeSelect
      );
      ...
      $scope.init = function () {
        $scope.$watch(
          ...
          function () {
            if ($scope.lesson) {
              if (/^[a-z\d\-_\s]+$/i.test($scope.lesson.name) == false)
                $scope.lesson.name =
      ...
      $scope.changeTitle = function () {
        org.ekstep.collectioneditor.api
          .getService("collection")
          .setNodeTitle($scope.lesson.name);
      };
      $scope.init();
    },
  ]);
