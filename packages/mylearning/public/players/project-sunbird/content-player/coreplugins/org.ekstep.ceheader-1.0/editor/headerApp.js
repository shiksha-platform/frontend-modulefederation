angular
  .module("org.ekstep.ceheader:headerApp", ["yaru22.angular-timeago"])
  .controller("headerController", [
    ...
    function ($scope, $window) {
      ...
      $scope.reportIssueLink =
        $window.context && $window.context.reportIssueLink
          ? $window.context.reportIssueLink
          : "";
      $scope.lastSaved = undefined;
      $scope.pendingChanges = false;
      $scope.saveBtnEnabled = false;
      $scope.userDetails = !_.isUndefined(window.context)
        ? window.context.user
        : undefined;
      $scope.telemetryService = org.ekstep.contenteditor.api.getService(
        ServiceConstants.TELEMETRY_SERVICE
      );
      $scope.ekstepLogo = ecEditor.resolvePluginResource(
        $scope.manifest.id,
        $scope.manifest.ver,
        "editor/images/ekstep_logo_white.png"
      );
      ...
      $scope.editContentMeta = function () {
        var config = {
          template: ecEditor.resolvePluginResource(
            $scope.manifest.id,
            $scope.manifest.ver,
            "editor/partials/editContentMetaDialog.html"
          ),
          controller: [
            "$scope",
            "mainCtrlScope",
            function ($scope, mainCtrlScope) {
              ...
              $scope.fireTelemetry = function (data) {
                mainCtrlScope.telemetryService.interact({
                  type: "click",
      ...
      ecEditor.addEventListener(
        "org.ekstep.ceheader:meta:edit",
        $scope.editContentMeta,
        $scope
      );
      org.ekstep.contenteditor.api
        .jQuery(".browse.item.at")
        .popup({ on: "click", setFluidWidth: false, position: "bottom right" });
