...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  initialize: function () {
    ...
    ecEditor.addEventListener(
      this.manifest.id + ":showDialog",
      this.showDialog,
      this
    );
    ecEditor.addEventListener(
      this.manifest.id + ":showConflictDialog",
    ...
    var conflictNgModuleTemplatePath = ecEditor.resolvePluginResource(
      this.manifest.id,
      this.manifest.ver,
      "editor/reviewConflictDialog.html"
    );
    var ngModuleTemplatePath = ecEditor.resolvePluginResource(
      this.manifest.id,
      this.manifest.ver,
      "editor/editMetaDialog.html"
    );
  ...
  review: function (event) {
    ...
    if (
      instance.contentObj["description"] ==
      "Write a short description of your lesson"
    ) {
      isValid = 0;
    ...
    ecEditor._.each(mandatoryFields, function (value, key) {
      if (
        typeof instance.contentObj[key] == "undefined" ||
        instance.contentObj[key] == ""
      ) {
        isValid = 0;
        fieldsToFill.push(value);
      } else if (
        Array.isArray(instance.contentObj[key]) &&
        instance.contentObj[key].length == 0
      ) {
        isValid = 0;
        fieldsToFill.push(value);
      }
