...
org.ekstep.contenteditor.metadataPlugin.extend({
   ...
   */
  eventMap: {
    savemeta: "org.ekstep.contenteditor:save:meta",
    review: "org.ekstep.contenteditor:review",
    save: "org.ekstep.contenteditor:save",
    close: "org.ekstep.collectioneditor:content:notfound",
  },
  ...
  initialize: function () {
    ...
    ecEditor.addEventListener(
      "org.ekstep.editcontentmeta:showpopup",
      this.invoke,
      this
    );
    ecEditor.addEventListener(
      "editor:form:getconfig",
      this.returnConfigs,
      this
    );
  },
  ...
  invoke: function (event, config) {
    ...
    if (!this.isConfigurationsExists(config.subType, config.action)) {
      this.getConfigurations(config, function (error, res) {
        if (res) {
          instance.mapResponse(config.subType, config.action, {
            resourceBundle: res.resourceBundle,
            framework: res.framework.data.result.framework,
            formConfig: res.config.data.result.form.data,
          });
          instance.renderForm(config.popup, {
            resourceBundle: res.resourceBundle,
            framework: res.framework.data.result.framework,
            formConfig: res.config.data.result.form.data,
          });
      ...
      instance.renderForm(config.popup, {
        resourceBundle: mappedRes.resourceBundle,
        framework: mappedRes.framework,
        formConfig: mappedRes.formConfig,
      });
    }
  },
  ...
  successAction: function (event, data) {
    ...
    if (data.isValid) {
      if (
        data.formData.metaData.mimeType ===
    ...
    var saveCallBackFn = function (err, res) {
      if (!err) {
        ecEditor.dispatchEvent(
          instance.eventMap[instance.config.action],
          reviewCallBackFn
        );
      } else {
    ...
    async.parallel(
      {
        config: function (callback) {
          ...
          org.ekstep.services.configuration.getFormConfigurations(
            { request: request },
        ...
        framework: function (callback) {
          ...
          ecEditor
            ...
            .getCategorys(frameworkId, function (error, response) {
              if (!error) callback(undefined, response);
              else callback(error, undefined);
            });
        },
  ...
  renderForm: function (isPopup, config) {
    ...
    this.loadTemplate(this.config.templateName, function (templatePath) {
      isPopup
        ? instance.showForm()
        : ecEditor.dispatchEvent("editor:template:loaded", {
            templatePath: templatePath,
            formAction: instance.config.action,
          });
    });
  },
  ...
  returnConfigs: function (event, callbackFn) {
    callbackFn({
      model: this.getModel(),
      template: this.config.templateName || this.DEFAULT_TEMPLATE_NAME,
      fields: this.form,
