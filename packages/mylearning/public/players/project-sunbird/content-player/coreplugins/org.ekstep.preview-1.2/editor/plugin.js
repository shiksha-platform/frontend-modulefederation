...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  initialize: function () {
    ...
    div.innerHTML =
      '<div class="preview-modal-content"><div class="preview-modal-wrapper"><div class="child preview-bgimage"></div><div class="child preview-iframe"><iframe id="previewContentIframe" width=100% height=100%></iframe></div></div>';
  ...
  initPreview: function (event, data) {
    ...
    if (scope.developerMode) {
      if (!this.contentBody.theme["plugin-manifest"])
        this.contentBody.theme["plugin-manifest"] = { plugin: [] };
      if (!_.isArray(this.contentBody.theme["plugin-manifest"].plugin))
        this.contentBody.theme["plugin-manifest"].plugin = [
          this.contentBody.theme["plugin-manifest"].plugin,
        ];
  ...
  getConfiguration: function () {
    ...
    configuration.data =
      meta.mimeType == "application/vnd.ekstep.ecml-archive"
        ? this.contentBody
        : {};
  ...
  showPreview: function (data) {
    ...
    var modalContent = document.getElementsByClassName(
      "preview-modal-content"
    )[0];
    if (!data.parentElement) modal.style.display = "block";
