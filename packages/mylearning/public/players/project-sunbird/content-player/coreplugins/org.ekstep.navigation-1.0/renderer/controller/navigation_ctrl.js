...
angular
  .module("genie-canvas")
  .directive("customNextNavigation", function ($rootScope, $timeout) {
    return {
      restrict: "E",
      template:
        '<div><a class="nav-icon nav-next" ng-show="showCustomNext !== state_off" href="javascript:void(0);"><img ng-src="{{customNextIcon}}" ng-click="navigate(\'next\')"></a></div>',
      link: function (scope) {
        scope.customNextIcon = EkstepRendererAPI.resolvePluginResource(
          "org.ekstep.overlay",
          "1.0",
          "renderer/assets/icons/next.png"
        );
        var events = [
          "overlayNext",
          "renderer:next:hide",
  ...
  .directive("customPreviousNavigation", function ($rootScope, $timeout) {
    return {
      restrict: "E",
      template:
        '<div><a class="nav-icon nav-previous" ng-show="showCustomPrevious !== state_off" ng-class="{\'nav-disable\': showCustomPrevious == state_disable}" href="javascript:void(0);"><img ng-src="{{customePreviousIcon}}" ng-click="navigate(\'previous\')"></a></div>',
      link: function (scope) {
        var events = [
          "overlayPrevious",
          "renderer:previous:hide",
          "renderer:previous:show",
          "renderer:previous:disable",
          "renderer:previous:enable",
        ];
        scope.customePreviousIcon = EkstepRendererAPI.resolvePluginResource(
          "org.ekstep.overlay",
          "1.0",
          "renderer/assets/icons/back.png"
        );
  ...
  .controllerProvider.register(
    "CustomNavigationCtrl",
    function ($scope, $rootScope, $compile, $stateParams, $timeout) {
      var globalConfig = EkstepRendererAPI.getGlobalConfig();
      ...
      $scope.init = function () {
        EkstepRendererAPI.addEventListener(
          "renderer:content:start",
          $scope.showCustomNavigation
        );
        EkstepRendererAPI.addEventListener(
          "renderer:overlay:show",
          $scope.showCustomNavigation
        );
        EkstepRendererAPI.addEventListener(
          "renderer:overlay:hide",
          $scope.hideCustomNavigation
        );
        EkstepRendererAPI.addEventListener(
        ...
        $scope.pluginInstance = EkstepRendererAPI.getPluginObjs(
        ...
        if ($scope.pluginInstance) {
          if (globalConfig.overlay.showOverlay) {
            $scope.customNavigationVisible =
              $scope.pluginInstance.customNavigationVisible;
            $scope.safeApply();
          }
        }
      };
