  ...
  org.ekstep.contentrenderer.questionUnitPlugin.extend({
    ...
    postQuestionShow: function () {
      ...
      if (this._question.state && _.has(this._question.state, "val")) {
        ...
        _.each(
          $(".org-ekstep-questionunit-mcq-option-element"),
          function (optionElement, index) {
            if (index == selectedIndex) {
              MCQController[layout.toLowerCase()].optionStyleUponClick(
                optionElement
              );
            }
          }
    ...
    getTelemetryResValues: function () {
      ...
      if (!_.isUndefined(selectedIndex))
        value[selectedIndex + 1] = this.getTelemetryParamsValue(
          MCQController.pluginInstance._question.data.options[selectedIndex]
        );
