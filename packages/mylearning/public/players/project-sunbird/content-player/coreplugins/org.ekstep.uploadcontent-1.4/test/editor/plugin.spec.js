describe("uploadContent plugin", function () {
  ...
  describe("Upload content", function () {
    it("should initialize the plugin", function () {
      ...
      expect(EventBus.hasEventListener("org.ekstep.uploadcontent:show")).toBe(
        true
      );
      var templatePath = expect(
      ...
      expect(ecEditor.getService("popup").loadNgModules).toHaveBeenCalledWith(
        templatePath,
        controllerPath
      );
    });
  ...
  describe("detectMimeType", () => {
    ...
    it("should return mimeType as video/webm", () => {
      ...
      spyOn($scope, "detectMimeType").and.callThrough();
      var returnType = $scope.detectMimeType(
        "https://www.youtube.com/watch?v=hQyUwF2vaac&t=15s"
      );
      expect($scope.validateUploadURL).toHaveBeenCalledWith(
        "https://www.youtube.com/watch?v=hQyUwF2vaac&t=15s"
      );
    });
  });
  ...
  describe("validateUploadURL", () => {
    it("should return empty string if if the given url is not valid youtube URL", () => {
      ...
      var response = $scope.validateUploadURL(
        "https://www.dailymotion.com/video/x6ztzkr"
      );
  ...
  describe("isWhitelistedURL", () => {
    it("should return false if the given URL is not a whitelisted URL", () => {
      ...
      $scope.getWhitelistedDomains = jasmine
        .createSpy()
        .and.returnValue(["youtube.com", "youtu.be"]);
      var response = $scope.isWhitelistedURL(
        "https://www.dailymotion.com/video/x6ztzkr"
      );
  ...
  describe("getHostName", () => {
    it("should return hostName from the url", () => {
      ...
      var response = $scope.getHostName(
        "https://www.youtube.com/watch?v=PDKxDbt6EGQ"
      );
  ...
  describe("validateYoutubeURL", () => {
    it("should return false if its not valid youtube URL", () => {
      ...
      var response = $scope.validateYoutubeURL(
        "https://www.you.tube.com/watch?v=PDbt6EGQ"
      );
      expect(response).toBe(false);
    });
  ...
  describe("upload", () => {
    xit("should upload a file, if its valid", () => {
      ...
      $scope.uploader = {
        getFile: function () {},
        getName: function () {
          return "file:///home/ttpllt44/Documents/git/sunbird_fork/test.mp4";
        },
      };
      $scope.uploader.getFile = jasmine
        .createSpy()
        .and.returnValue(
          "file:///home/ttpllt44/Documents/git/sunbird_fork/test.mp4"
        );
      $scope.contentService.createContent = jasmine
        .createSpy()
        .and.callFake((data, callback) => {
      ...
      ecEditor.getService(ServiceConstants.CONTENT_LOCK_SERVICE).createLock =
        jasmine.createSpy().and.callFake((data, callback) => {
      ...
      expect(ecEditor.getContext("contentId")).toEqual(
      ...
      expect(ecEditor.setConfig).toHaveBeenCalledWith(
        ServiceConstants.CONTENT_LOCK_SERVICE,
        lockResponse.data.result
      );
