"use strict";
angular
  .module("activityBrowserApp", ["angular-inview"])
  .controller("activityBrowserCtrl", [
    ...
    function ($scope, instance) {
      ...
      ctrl.loading = false;
      ctrl.inViewLogs = [];
      ctrl.defaultActivityImage = ecEditor.resolvePluginResource(
        instance.manifest.id,
        instance.manifest.ver,
        "assets/default-activity.png"
      );
      ...
      ctrl.viewMorePlugins = function (event) {
        ...
        ctrl.expandedGroupId = ctrl.clickedViewMoreLinkId.replace(
          "ViewMore",
          ""
        );
        $scope.$safeApply();
          ...
          .pluginsSearch(url, data, function (err, resp) {
            ...
            ecEditor._.forEach(resp.data.result.content, function (val) {
              if (_.isUndefined(val.category)) {
                ...
                val.category &&
                val.category.indexOf("library") == -1
              ) {
                ctrl.activitiesList.push(val);
              }
            });
      ...
      ctrl.addPlugin = function (activity) {
        var publishedDate = new Date(
          activity["lastPublishedOn"] || new Date().toString()
        ).getTime();
        ecEditor.loadAndInitPlugin(
          activity.code,
          activity.semanticVersion,
          publishedDate
        );
        ctrl.generateImpression({
          type: "view",
      ...
      ecEditor.dispatchEvent("org.ekstep.conceptselector:init", {
        ...
        callback: function (data) {
          ctrl.activityOptions.conceptsPlaceHolder =
      ...
      ctrl.getPluginDetails = function (pluginId) {
        ...
        ecEditor
          .getService("content")
          .getContent(pluginId, function (err, res) {
            ctrl.loading = false;
            if (res) {
              ctrl.pluginDetails = res;
              if (res.usedByContent && res.usedByContent.length)
                ctrl.getPluginScreenshots(res.usedByContent);
              else {
                ctrl.images.push({
                  image: ecEditor.resolvePluginResource(
                    instance.manifest.id,
                    instance.manifest.ver,
                    "assets/default-activity.png"
                  ),
          ...
          if (res && res.responseCode === "OK" && res.result.count > 0) {
            res.result.content.forEach(function (content) {
              if (content.downloadUrl)
                ctrl.images.push({ image: content.downloadUrl });
            });
          } else {
            ctrl.images.push({
              image: ecEditor.resolvePluginResource(
