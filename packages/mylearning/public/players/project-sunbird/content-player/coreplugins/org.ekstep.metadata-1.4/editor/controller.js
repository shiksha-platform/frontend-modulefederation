...
angular.module("org.ekstep.metadataform", []).controller("metadataForm", [
  "$scope",
  function ($scope) {
    /**
          ...
          if (field.depends && field.depends.length) {
            $scope.getAssociations(
              ...
              function (associations) {
                var target =
                  object && object.target ? object.target : undefined;
    ...
    $scope.onConfigChange = function (object) {
      if (object.field) {
        var type =
          object.field.inputType == "select" ||
          object.field.inputType == "multiselect"
            ? "change"
            : "click";
        object.field &&
          logTelemetry(
            {
              type: type,
              subtype: object.field.inputType,
              target: { id: object.field.code, type: "field", ver: "" },
            },
            $scope.manifest
          );
      }
        ...
        $scope.getAssociations(
          ...
          function (associations) {
            var data = $scope.getScopeMeta("", {
              target: "#" + object.target.tempalteName,
            });
            $scope.getParentAssociations(
              object.field,
              associations,
              data,
              function (commonAssociations) {
                $scope.applyDependencyRules(
                  object.field,
                  commonAssociations,
                  true,
                  "#" + object.target.tempalteName
                );
              }
            );
          }
    ...
    $scope.getParentAssociations = function (
      field,
      associations,
      formData,
      callback
                ...
                if (_.isArray(formData[val]) && formData[val].length > 0) {
                  _.forEach(formData[val], function (metadata) {
                    if (range.name === metadata) {
                      associations = $scope.getCommonAssociations(
                        range.associations,
                        associations
                      );
                    }
    ...
    $scope.getCommonAssociations = function (
      parentAssociations,
      childAssociations
    ) {
      var intersectionData = [];
      if (parentAssociations && parentAssociations.length) {
        intersectionData = _.filter(parentAssociations, function (e) {
     ...
     */
    $scope.applyDependencyRules = function (
      field,
      associations,
      resetSelected,
      target
      ...
      ecEditor.dispatchEvent("editor:field:association", {
        field: field,
      ...
      if (field.depends && field.depends.length) {
        _.forEach(field.depends, function (id) {
          ...
          dependedValues = _.map(associations, (i) =>
            _.pick(i, ["name", "category"])
          );
          ...
          if (groupdFields.length) {
            _.forEach(groupdFields, function (value, key) {
              $scope.updateDropDownList(
                value.category,
                _.map(value.name, (i) => _.pick(i, "name"))
              );
              $scope.$safeApply();
            });
          } else {
    ...
    $scope.success = function (event, object) {
      ...
      if (!validationStatus) {
        ...
        ecEditor.dispatchEvent("org.ekstep.toaster:error", {
          message:
            $scope.messages.validationError || $scope.validationErrorMessage,
          position: "topCenter",
          icon: "fa fa-warning",
        });
      }
      ...
      var form = {};
      form.metaData = getUpdatedMetadata(
        object.target.contentMeta,
        $scope.originalContentMeta,
        $scope.fields
      );
        ...
        if (form[value.code] && form[value.code].$invalid) {
          ...
          switch (_.keys(form[value.code].$error)[0]) {
            case "pattern": // When input validation of type is regex
              scope.validation[value.code]["errorMessage"] =
                value.validation.regex.message;
              break;
      ...
      logTelemetry(
        {
          ...
          type: "click",
          subtype: "button",
          target: { id: "close", type: "button", ver: "" },
        },
        $scope.manifest
      );
      ecEditor.dispatchEvent("editor:form:cancel", {
        callback: $scope.closeThisDialog,
      });
    };
     ...
     */
    $scope.init = function () {
      !EventBus.hasEventListener("metadata:form:onsuccess") &&
        ecEditor.addEventListener(
          "metadata:form:onsuccess",
          $scope.success,
          $scope
        );
      !EventBus.hasEventListener("metadata:form:oncancel") &&
        ecEditor.addEventListener(
          "metadata:form:oncancel",
          $scope.cancel,
          $scope
        );
      !EventBus.hasEventListener("metadata:form:getdata") &&
        ecEditor.addEventListener(
      ...
      var callbackFn = function (config) {
        ...
        if ($scope.tempalteName === "defaultTemplate") {
          ...
          if (ecEditor.getContext("ownershipType")) {
            if (
              ecEditor.getContext("ownershipType").indexOf("createdFor") != -1
            ) {
              Object.keys(ownershipObject.organisations).map(function (key) {
                $scope.ownership.push({
                  value: key,
                  name: ownershipObject.organisations[key],
                });
              });
            }
            if (
              ecEditor.getContext("ownershipType").indexOf("createdBy") != -1
            ) {
              $scope.ownership.push({
                value: ownershipObject.id,
                name: ownershipObject.name,
              });
            }
          } else {
            $scope.ownership.push({
        ...
        if (license) {
          ...
          if (
            !_.isUndefined(license.renderingHints.value) &&
            !_.isUndefined(license.renderingHints.value[$scope.contentMimeType])
          ) {
            $scope.licenseText =
              license.renderingHints.value[$scope.contentMimeType];
          }
        }
        if (!_.isUndefined($scope.originalContentMeta["copyright"])) {
          $scope.contentMeta["copyright"] =
            $scope.originalContentMeta["copyright"];
        } else if (
          ecEditor.getContext("user") &&
          ecEditor.getContext("user").organisations
        ) {
          $scope.contentMeta["copyright"] = config.editMode
            ? _.values(ecEditor.getContext("user").organisations).join(", ")
            : "";
        }
        if (
          !_.isUndefined($scope.originalContentMeta["contributors"]) &&
          !_.isEmpty($scope.originalContentMeta["contributors"])
        ) {
        ...
        if (
          !_.isUndefined($scope.originalContentMeta["creators"]) &&
          !_.isEmpty($scope.originalContentMeta["creators"])
        ) {
        ...
        $scope.fixedLayoutConfigurations = _.uniqBy(
          layoutConfigurations.fixedLayout,
          "code"
        );
        $scope.dynamicLayoutConfigurations = _.sortBy(
          _.uniqBy(layoutConfigurations.dynamicLayout, "code"),
          "index"
        );
    ...
    $scope.isValidInputs = function (object) {
      ...
      if (
        appIconConfig &&
        appIconConfig.visible &&
        appIconConfig.required &&
        !scope.contentMeta["appIcon"]
      ) {
        isValid = false;
      }
      if (
        conceptSelector &&
        conceptSelector.required &&
        !_.size(scope.contentMeta["concepts"])
      ) {
        isValid = false;
      }
      if (
        topicSelector &&
        topicSelector.required &&
        !_.size(scope.contentMeta["topic"])
      ) {
        isValid = false;
      }
    ...
    $scope.resetAllFilters = function () {
      _.forEach($scope.dynamicLayoutConfigurations, function (field) {
        $scope.contentMeta[field.code] = [];
        if (field.code === "topic")
          ecEditor.dispatchEvent("editor.topic.change", {
            key: "topic",
            value: [],
          });
        if (field.code === "concepts")
          ecEditor.dispatchEvent("editor.concept.change", {
            key: "concepts",
            value: [],
          });
        $scope.$safeApply();
