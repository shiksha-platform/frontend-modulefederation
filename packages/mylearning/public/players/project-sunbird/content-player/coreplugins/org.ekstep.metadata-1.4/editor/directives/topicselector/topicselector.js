...
formApp.directive("topicSelector", function () {
  ...
  var topicController = [
    ...
    "$controller",
    "$timeout",
    function ($scope, $rootScope, $controller, $timeout) {
      $scope.contentMeta = $scope.$parent.contentMeta;
      $scope.topicSelectorMessage = $scope.contentMeta.topic
        ? "(" + $scope.contentMeta.topic.length + ") topics selected"
        : "(0) topics selected";
      /**Added for more than one topic selector on same page, element id should be unique as per template **/
      $scope.framework =
        $scope.$parent.$parent.tempalteName == "resourceFilterTemplate"
          ? ecEditor.getContext("resource_framework")
          : ecEditor.getContext("framework");
      $scope.templateId = !_.isUndefined($scope.$parent.$parent.tempalteName)
        ? $scope.$parent.$parent.tempalteName
        : "metaform";
      ...
      $scope.callbackFn = function (data) {
        ...
        $scope.$parent.onConfigChange({
          target: "#" + $scope.templateId,
          field: $scope.$parent.$parent.dynamicField,
          form: $scope.$parent.$parent.metaForm,
          value: $scope.contentMeta.topic,
        });
  ...
  return {
    ...
    templateUrl: ecEditor.resolvePluginResource(
      manifest.id,
      manifest.ver,
      "editor/directives/topicselector/template.html"
    ),
