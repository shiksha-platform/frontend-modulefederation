...
Plugin.extend({
  ...
  initPlugin: function (data) {
    ...
    EkstepRendererAPI.addEventListener(
      ...
      function () {
        if (
          _.size(
            _.pick(EkstepRendererAPI.getCurrentStage().params, ["previous"])
          )
        )
          this.disposeStageVideos;
      },
    ...
    var assetSrc = data.asset
      ? this._theme.getAsset(data.asset)
      : this._theme.getAsset(data.id);
  ...
  resetOtherStagePlayers: function (event) {
    ...
    var tempStageVideos =
      EkstepRendererAPI.getCurrentStage()._data["org.ekstep.video"];
    if (!_.isArray(tempStageVideos)) {
      tempStageVideos = [tempStageVideos];
    }
    var currentStagePlayers = _.map(
      _.pluck(tempStageVideos, ["videoPlayer"]),
      "id_"
    );
  ...
  loadYoutube: function (path) {
    ...
    var videoInstanceID = instance.asset
      ? "ID" + instance.asset
      : "ID" + instance._id;
    if (!navigator.onLine) {
      ...
      EkstepRendererAPI.dispatchEvent("renderer:toast:show", undefined, {
        type: "warning",
        message:
          "No internet connectivity. Turn on WiFi or mobile data and try again",
      });
    }
    ...
    this.isAudioMuted()
      ? videojs(videoInstanceID).muted(true)
      : videojs(videoInstanceID).muted(instance.muted);
  ...
  addVideoEventListener: function (videoPlayer) {
    ...
    videoPlayer.on("play", function (e) {
      instance.logTelemetry("TOUCH", "PLAY", "play-button", [
        Math.floor(videoPlayer.currentTime()) * 1000,
      ]);
    });

    videoPlayer.on("pause", function (e) {
      instance.logTelemetry("TOUCH", "PAUSE", "pause-button", [
        Math.floor(videoPlayer.currentTime()) * 1000,
      ]);
    });

    videoPlayer.on("ended", function () {
      instance.logTelemetry("TOUCH", "STOP", "stop-button", [
        Math.floor(videoPlayer.currentTime()) * 1000,
      ]);
    });
    videoPlayer.on("seeked", function (e) {
      instance.logTelemetry("TOUCH", "DRAG", "seek-button", [
        Math.floor(videoPlayer.currentTime()) * 1000,
      ]);
    });
  },
