  ...
  function inViewDirective($parse) {
    return {
      ...
      link: function inViewDirectiveLink(scope, element, attrs, container) {
        ...
        if (container) {
          viewportEventSignal = viewportEventSignal.merge(
            container.eventsSignal
          );
        }
        ...
        var inviewInfoSignal = viewportEventSignal
          ...
          .map(function (event) {
            ...
            var isVisible = !!(
              ...
              element[0].getClientRects().length
            );
            var info = {
          ...
          .scan({}, function (lastInfo, newInfo) {
            ...
            if (
              options.generateDirection &&
              newInfo.inView &&
              lastInfo.elementRect
            ) {
              newInfo.direction = {
                horizontal:
                  newInfo.elementRect.left - lastInfo.elementRect.left,
  ...
  function signalFromEvent(target, event) {
    return new QuickSignal(function (subscriber) {
      ...
      subscriber.$dispose = function () {
        event
          .split(" ")
          .map((e) => el[0].removeEventListener(e, handler, true));
      };
    });
  }
