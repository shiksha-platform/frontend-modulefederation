...
org.ekstep.contentrenderer.keyboardRenderer = Plugin.extend({
  ...
  initialize: function () {
    ...
    EkstepRendererAPI.addEventListener(
      "org.ekstep.keyboard:invoke",
      this.showKeyboard,
      instance
    );
    EkstepRendererAPI.addEventListener(
      "org.ekstep.keyboard:hide",
      this.hideKeyboard
    );
  },
  showKeyboard: function (event, callback) {
    ...
    if (_.isUndefined(keyboardConfig) || keyboardConfig.type == "Device") {
      ...
      if ($(Keyboard.constant.keyboardElement).length <= 0) {
        // eslint-disable-line no-undef
        $("#gameArea").append(
          template({ inputValue: Keyboard.targetInput.value.trim() })
        ); // eslint-disable-line no-undef
      }
      $("#firstRowAlpha").ready(function () {
        ...
        if (position.top > parentHeight)
          EkstepRendererAPI.dispatchEvent("renderer:toast:show", undefined, {
            type: "info",
            message: "Please reduce number of keys and check preview again.",
          });
      });
    }
  },
  ...
  getAssetUrl: function (url) {
    if (isbrowserpreview) {
      ...
      if (EkstepRendererAPI.isStreamingContent()) {
        ...
        return this.validateUrl(
          "file:///" + EkstepRendererAPI.getBaseURL() + url
        );
      }
    }
  },
