angular.module("suggestcontentApp", []).controller("suggestcontentController", [
  "$scope",
  "$timeout",
  function ($scope, $timeout) {
    ...
    $scope.init = function () {
      _.forEach(
        ecEditor.jQuery("#collection-tree").fancytree("getRootNode").children[0]
          .children,
        function (content) {
          if (!content.folder) $scope.excludeContents.push(content.data.id);
        }
      );
      var rootNodeMetadata = ecEditor
        .jQuery("#collection-tree")
        .fancytree("getRootNode")
        .getFirstChild().data.metadata;
      if (rootNodeMetadata.board)
        $scope.metaData.board = rootNodeMetadata.board;
      if (rootNodeMetadata.subject)
        $scope.metaData.subject = rootNodeMetadata.subject;
      if (rootNodeMetadata.gradeLevel)
        $scope.metaData.gradeLevel = rootNodeMetadata.gradeLevel;
      if (rootNodeMetadata.language)
        $scope.metaData.language =
          typeof rootNodeMetadata.language === "object"
            ? rootNodeMetadata.language[0]
            : rootNodeMetadata.language;
    ...
    $scope.searchLessons = function () {
      ...
      searchBody.request.filters.contentType = org.ekstep.collectioneditor.api
        .getService("collection")
        .getObjectTypeByAddType("Browser");
      $scope.suggestedContentList = { count: 0, content: [] };
      var concepts = "";
      ...
      if ($scope.responseData[concepts]) {
        ...
        if ($scope.metaData.gradeLevel)
          searchBody.request.filters.gradeLevel = $scope.metaData.gradeLevel;
        if ($scope.metaData.board)
          searchBody.request.filters.board = $scope.metaData.board;
        if ($scope.metaData.concepts) {
        ...
        searchService.search(searchBody, function (err, res) {
          if (err) {
            ...
            $scope.responseData[concepts] = _.concat(
              _.uniqBy($scope.responseData, "identifier"),
            ...
            angular.forEach(res.data.result.content, function (lessonContent) {
              ...
              if ($scope.excludeContents.length) {
                if (
                  _.indexOf($scope.excludeContents, lessonContent.identifier) <
                  0
                )
                  $scope.suggestedContentList.content.push(lessonContent);
              } else {
            ...
            $scope.suggestedContentList.content = _.uniqBy(
    ...
    $scope.addContent = function (lesson, index) {
      ...
      if (org.ekstep.services.collectionService.getActiveNode().folder) {
        org.ekstep.services.collectionService.filterResource(undefined, [
          lesson,
        ]);
    ...
    $scope.onNodeSelect = function () {
      ...
      _.forEach(
        ...
        function (obj) {
          if (
            obj.type == activeNode.data.objectType &&
            obj.editable &&
            activeNode.data.metadata.concepts
          ) {
      ...
      if (activeNodeConcepts.length == 0) $scope.metaData.concepts = undefined;
      $scope.searchLessons();
    };
    ...
    $scope.updateMetaData = function () {
      _.forEach(
        org.ekstep.collectioneditor.api
          .getService("collection")
          .getCollectionHierarchy().nodesModified,
        function (node) {
          var updatedConcepts = [];
          ...
          if (node.root) {
            ...
            if (
              ...
              node.metadata.subject != $scope.metaData.subject
            ) {
            ...
            if (
              ...
              !_.isEqual(
                updatedConcepts.sort(),
                $scope.metaData.concepts.sort()
              )
            ) {
            ...
            if (
              node.metadata.gradeLevel &&
              node.metadata.gradeLevel != $scope.metaData.gradeLevel
            ) {
            ...
            if (
              updatedConcepts.length &&
              !_.isEqual(updatedConcepts.sort(), $scope.metaData.concepts)
            ) {
              $scope.metaData.concepts = updatedConcepts;
              $scope.searchLessons();
            }
