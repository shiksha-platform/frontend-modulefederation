angular.module("contentmetaApp", []).controller("contentmetaController", [
  "$scope",
  "$timeout",
  function ($scope, $timeout) {
    ...
    $scope.updateNode = function () {
      if (!_.isEmpty($scope.nodeId) && !_.isUndefined($scope.nodeId)) {
        var activeNode = org.ekstep.collectioneditor.api
        ...
        if (
          !_.isEmpty($scope.content.language) &&
          _.isString($scope.content.language)
        ) {
        ...
        org.ekstep.collectioneditor.cache.nodesModified[
          ...
          org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
            .metadata,
          $scope.getUpdatedMetadata($scope.metadataCloneObj, $scope.content)
        );
        var keywords =
          org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
            .metadata.keywords;
        if (keywords) {
          org.ekstep.collectioneditor.cache.nodesModified[
            $scope.nodeId
          ].metadata.keywords = keywords.map(function (a) {
        ...
        if (activeNode.data && activeNode.data.root)
          ecEditor.dispatchEvent("content:title:update", $scope.content.name);
        $scope.getPath();
    ...
    $scope.initDropdown = function () {
      $timeout(function () {
        if ($scope.content.language)
          $("#contentmeta-language").dropdown(
            "set selected",
            $scope.content.language[0]
          );
      });
    };
    ...
    $scope.onNodeSelect = function (evant, data) {
      ...
      $scope.editable =
        !data.data.root && data.data.metadata.visibility === "Default"
          ? false
          : true;
      if (_.indexOf(contentArr, data.data.objectType) != -1) {
        ...
        var activeNode = org.ekstep.collectioneditor.api
          .getService("collection")
          .getActiveNode();
        $scope.content = _.isUndefined(cache)
          ? activeNode.data.metadata
          : _.assign(activeNode.data.metadata, cache.metadata);
        ...
        if (
          !_.isEmpty(activeNode.data.metadata) &&
          _.has(activeNode.data.metadata, ["name"])
        ) {
          if (!$scope.editable) {
      ...
      $scope.showImageIcon = true;
      if (data.data.objectType == "Collection" && $scope.showSubCollection)
        $scope.getSubCollection(
        ...
        function (err, content) {
          if (!err) {
            ...
            org.ekstep.pluginframework.eventManager.dispatchEvent(
              "atpreview:show",
              { contentBody: content.body, currentStage: false }
            );
    ...
    $scope.generateTelemetry = function (data) {
      if (data)
        org.ekstep.services.telemetryService.interact({
          ...
          target: data.target,
          pluginid: "org.ekstep.contentmeta",
          pluginver: "1.2",
    ...
    ecEditor.jQuery("#collection-meta-tree").fancytree({
      ...
      renderNode: function (event, data) {
        ...
        if (
          !$nodeSpan.data("rendered") &&
          !node.folder &&
          !$nodeSpan.find("span.collection-preview-icon.popup-item").length
        ) {
          var contextButton = $(
            '<span class="collection-preview-icon popup-item" data-content="Preview" data-variation="tiny inverted" data-position="top center" onclick="ecEditor.dispatchEvent(\'org.ekstep.contentmeta:preview\', {id: \'' +
              node.data.id +
    ...
    $scope.buildSubCollectionTree = function (data, tree) {
      ...
      _.forEach(data.children, function (child) {
        ...
        tree.push({
          ...
          title:
            child.name.length > 22
              ? child.name.substring(0, 22) + "..."
              : child.name,
    ...
    $scope.toggleCollectionAccodrion = function () {
      $scope.toggleCollectionAccodrionState =
        !$scope.toggleCollectionAccodrionState;
    };

    ecEditor.addEventListener(
      "org.ekstep.contentmeta:preview",
    ...
    setTimeout(function () {
      ...
      ecEditor
        .jQuery(".collection-metadata-accordion .title:first-child")
        .click();
    }, 0);
    ...
    $scope.init = function () {
      $scope.$watch(
        ...
        function () {
          if ($scope.content) {
            if (
              /^[a-z\d\-_\s]+$/i.test($scope.content.name) == false &&
              $scope.editMode
            )
              $scope.content.name =
                org.ekstep.services.collectionService.removeSpecialChars(
                  $scope.content.name
                );
            var activeNode = org.ekstep.collectioneditor.api
              .getService("collection")
              .getActiveNode();
            if (
              $scope.nodeType === DEFAULT_NODETYPE &&
              (activeNode.data.root || $scope.content.visibility != "Default")
            ) {
              $scope.updateNode();
            }
          }
    ...
    $scope.changeTitle = function () {
      org.ekstep.collectioneditor.api
        .getService("collection")
        .setNodeTitle($scope.content.name);
    };

    $scope.loadKeywords = function ($query) {
      if ($query.length >= 3) {
        return org.ekstep.services.collectionService
          .fetchKeywords($query)
          .then(function (keywords) {
            return keywords.filter(function (keyword) {
              return (
                keyword.lemma.toLowerCase().indexOf($query.toLowerCase()) != -1
              );
            });
          });
      }
