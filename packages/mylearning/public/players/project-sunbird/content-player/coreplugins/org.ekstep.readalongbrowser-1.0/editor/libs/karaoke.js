(function ($, window, document) {
  window.Karaoke = function () {
    ...
    this.fc_template +=
      '<button class="picker" style="background:#FFFF00;"></button>';
    this.fc_template +=
      '<button class="picker" style="background:#FF0000;"></button>';
    this.fc_template +=
      '<button class="picker" style="background:#336666;"></button>';
    this.fc_template +=
      '<button class="picker" style="background:#000099;"></button>';
    this.fc_template +=
      '<button class="picker" style="background:#333399;"></button>';
    this.fc_template +=
      '<button class="picker" style="background:#663366;"></button>';
    this.fc_template +=
      '<button class="picker" style="background:#330000;"></button>';
    this.fc_template +=
      '<button class="picker" style="background:#663300;"></button>';
    this.fc_template +=
      '<button class="picker" style="background:#333300;"></button>';
    this.fc_template +=
      '<button class="picker" style="background:#003300;"></button>';
    this.fc_template +=
      '<button class="picker" style="background:#003333;"></button>';
    this.fc_template +=
      '<button class="picker" style="background:#330099;"></button>';
  ...
  Karaoke.prototype.initSplit = function () {
        ...
        return function (e) {
          ...
          if (e.which === 32) {
            ...
            if ($("#word-" + self.wordIdx, _this.textBox).length > 0) {
              $("#word-" + self.wordIdx, _this.textBox).css(
                "background-color",
                self.highlightColor
              );
              if (_this.wordIdx + 1 > wordLength * 10) {
                $(".textWrapper").scrollTo(
                  "#word-" + (_this.wordIdx + 1),
                  _this.textBox
                );
                wordLength += 1;
              }
    ...
    this.textBox.append(
      $('<div class="open-audio" ><i class="icon-headphones"></i></li>')
    );
    ...
    $(".slideStep").on("click", function () {
      self.playFromTheStep(this);
    });
  };
  ...
  Karaoke.prototype.splitwords = function (element) {
    ...
    splitted = _.map(textArray, function (text) {
      ...
      if (splitted_text.length > 1) {
        formatted =
          _.reduce(splitted_text, function (acc, item, idx) {
            if ($.trim(item) === "") {
              return acc + "<br>";
            } else {
              if (idx === 1) {
  ...
  Karaoke.prototype.setColor = function (color) {
    ...
    if ($(".highlight-color-picker").length === 0) {
      ...
      return $(".highlight-color-picker .color-reset", this.el).on(
        "click",
        function () {
  ...
  Karaoke.prototype.markWords = function () {
    ...
    if ($("#word-" + this.wordIdx, this.text_box).length > 0) {
      $("#word-" + this.wordIdx, this.text_box).css(
        "background-color",
        this.highlightColor
      );
  ...
  Karaoke.prototype.initPlayer = function (play_synced) {
    ...
    if (this.highlightColor === "#FFFF00") {
      this.highlightColor = audio_obj.highlightColor
        ? audio_obj.highlightColor
        : this.highlightColor;
    }
    ...
    if (!$("#jplayerSync", this.$el).data("jPlayer")) {
      ...
      $("#jplayerSync", this.$el).bind(
        ...
        function (event) {
          ...
          self.duration_in_split_seconds = parseInt(
            parseFloat(event.jPlayer.status.duration.toFixed(1)) / 0.1
          );
          ...
          _.times(self.duration_in_split_seconds, function (step) {
            ...
            html =
              "<div id=" +
              slideStep +
              " class='slideStep' data-step=" +
              (step + 1) +
              ">";
            html +=
              "<span class='stepTop'>" +
              ((step + 1) * 0.1).toFixed(1) +
              "</span>";
            html +=
              "<span class='stepMiddle' draggable='" +
              wordPosBool +
              "' data-wordidx='" +
              wordPosIdx +
              "'>" +
            ...
            if (!_.isEmpty(words)) {
              ...
              if (!_.isEmpty(word)) {
                ...
                $("#" + slideStep + " span.stepMiddle").data(
                  "wordidx",
                  word.wordIdx
                );
              }
        ...
        (function (_this) {
          return function (event) {
            ...
            if (event.jPlayer.status.currentTime >= timings[_this.wordIdx]) {
              ...
              $("#word-" + (_this.wordIdx + 1), _this.text_box).css(
                "background-color",
