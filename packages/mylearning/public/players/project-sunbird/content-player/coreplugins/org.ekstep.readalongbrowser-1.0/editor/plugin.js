...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  initialize: function () {
    ...
    setTimeout(function () {
      var templatePath = ecEditor.resolvePluginResource(
        ...
        "editor/readalongbrowser.html"
      );
      var controllerPath = ecEditor.resolvePluginResource(
        instance.manifest.id,
        instance.manifest.ver,
        "editor/readalongbrowserapp.js"
      );
  ...
  loadHtml: function (event, data) {
    ...
    ecEditor.getService("popup").open(
      ...
      function () {
        if (
          !ecEditor._.isUndefined(currentInstance.editorObj) &&
          !currentInstance.editorObj.text
        ) {
  ...
  invokeKaraoke: function (audioSrc, attrs) {
    ...
    if (attrs) {
      var timings = !ecEditor._.isEmpty(attrs.config.timings)
          ? ecEditor._.split(attrs.config.timings, ",")
          : "",
        wordTimes = [],
        words = [],
    ...
    ecEditor
      ...
      .bind(
        "click",
        ecEditor.jQuery.proxy(karaoke.changePlaybackRate, karaoke)
      );
    ecEditor
      .jQuery("#changeaudio")
      .bind("click", ecEditor.jQuery.proxy(karaoke.res, karaoke));
    ecEditor
      .jQuery("#syncStart")
      .bind("click", ecEditor.jQuery.proxy(karaoke.startSync, karaoke));
    ecEditor
      .jQuery("#pick-hcolor")
      .bind("click", ecEditor.jQuery.proxy(karaoke.setColor, karaoke));
    ecEditor
      .jQuery("#stopAudio")
      .bind("click", ecEditor.jQuery.proxy(karaoke.stopAudio, karaoke));
    ecEditor
      .jQuery(".slideStep")
      .bind("drop", ecEditor.jQuery.proxy(karaoke.handleWordDrop, karaoke));
    ecEditor
      .jQuery("#syncMark")
      .bind("click", ecEditor.jQuery.proxy(karaoke.markWords, karaoke));
    ecEditor
      .jQuery("#sync-play")
      .bind("click", ecEditor.jQuery.proxy(karaoke.playSyncedLayer, karaoke));
    ecEditor
      .jQuery("#sync-pause")
      .bind("click", ecEditor.jQuery.proxy(karaoke.pauseAudio, karaoke));
