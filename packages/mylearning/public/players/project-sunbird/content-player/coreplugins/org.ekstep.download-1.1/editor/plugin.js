...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  downloadContent: function () {
    ...
    var modalController = function ($scope) {
      ...
      instance.getDownloadUrl(function (downloadUrl) {
        if (downloadUrl) {
          ...
          $scope.status = false;
          $scope.getMessage =
            "Content is not ready to download, please try again later";
    ...
    instance.generateTelemetry({
      ...
      targetid: ecEditor.getContext("contentId"),
    });
  },
  ...
  sendEmail: function ($scope, data) {
    ecEditor.jQuery.ajax({
      url:
        ...
        "/index.php?option=com_api&app=ekcontent&resource=download&format=raw",
      headers: {
        "x-auth": "session",
      },
      type: "POST",
