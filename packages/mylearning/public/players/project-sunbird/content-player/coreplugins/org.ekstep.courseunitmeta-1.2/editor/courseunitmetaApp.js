angular.module("courseunitmetaApp", []).controller("courseunitmetaController", [
  "$scope",
  function ($scope) {
    ...
    $scope.updateNode = function () {
      if (!_.isEmpty($scope.nodeId) && !_.isUndefined($scope.nodeId)) {
        ...
        org.ekstep.collectioneditor.cache.nodesModified[
          ...
          org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
            .metadata,
          $scope.getUpdatedMetadata($scope.metadataCloneObj, $scope.courseunit)
        );
        var keywords =
          org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
            .metadata.keywords;
    ...
    $scope.onNodeSelect = function (evant, data) {
      ...
      var activeNode = org.ekstep.collectioneditor.api
        .getService("collection")
        .getActiveNode();
      $scope.courseunit = _.isUndefined(
        org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
      )
        ...
        if (!_.isUndefined(activeNode.data.metadata.concepts)) {
          ...
          if ($scope.courseunit.concepts.length > 0) {
            $scope.courseunit.conceptData =
              "(" + $scope.courseunit.concepts.length + ") concepts selected";
    ...
    ecEditor.addEventListener(
      "org.ekstep.collectioneditor:node:selected:CourseUnit",
      $scope.onNodeSelect
    );
    ...
    $scope.generateTelemetry = function (data) {
      if (data)
        org.ekstep.services.telemetryService.interact({
          ...
          target: data.target,
          pluginid: "org.ekstep.courseunitmeta",
          pluginver: "1.2",
    ...
    $scope.init = function () {
      $scope.$watch(
        ...
        function () {
          if ($scope.courseunit) {
            if (/^[a-z\d\-_\s]+$/i.test($scope.courseunit.name) == false)
              $scope.courseunit.name =
    ...
    $scope.changeTitle = function () {
      org.ekstep.collectioneditor.api
        .getService("collection")
        .setNodeTitle($scope.courseunit.name);
    };
