...
angular.module("ngTokenField", []).directive("ngTokenField", [
  "$parse",
  "$timeout",
  function ($parse, $timeout) {
    return {
      ...
      template:
        '<div class="ng-token-field"><input type="text" placeholder={{placeholder}}></div>',
      link: function (scope, element, attrs) {
        var _tokenField;
        var _input;
        var _SEPARATOR = ",";
        var _triggeredUpdate = false;
        var _unbindWatcher = scope.$watch(
          "ngModel",
        ...
        function resizeInput(tokenWrapper) {
          var xoffset =
            tokenWrapper.offset().left +
            tokenWrapper.width() -
            _tokenField.offset().left;
...
org.ekstep.pluginframework.pluginManager.registerPlugin(
  {
    id: "org.ekstep.courseunitmeta",
    ver: "1.5",
    author: "Kartheek Palla",
    title: "Course Unit Meta plugin",
    description: "",
    publishedDate: "",
    editor: {
