 ...
 */
angular
  .module("editorApp", ["ngDialog", "oc.lazyLoad", "Scope.safeApply"])
  .directive("dialcode", function () {
    const manifest = org.ekstep.pluginframework.pluginManager.getPluginManifest(
      "org.ekstep.metadata"
    );
    ...
    template = ecEditor.resolvePluginResource(
      manifest.id,
      manifest.ver,
      "editor/directives/dialCode/template.html"
    );
    var dialCodeController = [
      "$scope",
      "$controller",
      "$filter",
      function ($scope, $controller, $filter) {
        ...
        $scope.contentMeta = ecEditor
          .getService("content")
          .getContentMeta(org.ekstep.contenteditor.api.getContext("contentId"));
        $scope.maxChar = 6;
        $scope.minChar = 0;
        ...
        $scope.searchDialCode = function (dialcode, callback) {
          ...
          ecEditor
            .getService("dialcode")
            .getAllDialCodes(channel, reqObj, function (err, res) {
              if (!err) {
                if (res.data.result.count) {
                  ecEditor._.uniq(
                    org.ekstep.services.collectionService.dialcodeList.push(
                      res.data.result.dialcodes[0].identifier
                    )
                  );
                  callback &&
                    callback({
                      isValid: true,
                      dialcode: res.data.result.dialcodes,
                    });
                } else {
        ...
        $scope.validateDialCode = function () {
          ...
          if (String(this.dialcodes).match(/^[A-Z0-9]{6}$/)) {
            ...
            if (
              ...
              org.ekstep.collectioneditor.cache.nodesModified[node.data.id]
            ) {
              org.ekstep.collectioneditor.cache.nodesModified[
                node.data.id
              ].metadata["dialcodes"] = this.dialcodes;
            }
            if (
              ecEditor._.indexOf(
              ...
              $scope.status = true;
              _.has(stateService.state.invaliddialCodeMap, node.data.id)
                ? _.unset(stateService.state.invaliddialCodeMap, node.data.id)
                : "";
              if (
                $scope.contentMeta.mimeType ==
              ...
              $scope.searchDialCode(this.dialcodes, function (response) {
                if (response.isValid) {
                  ...
                  _.has(stateService.state.invaliddialCodeMap, node.data.id)
                    ? _.unset(
                        stateService.state.invaliddialCodeMap,
                        node.data.id
                      )
                    : "";
                  if (
                  ...
                  _.has(stateService.state.dialCodeMap, node.data.id)
                    ? _.unset(stateService.state.dialCodeMap, node.data.id)
                    : "";
                  stateService.setState(
                    "invaliddialCodeMap",
                    node.data.id,
        ...
        $scope.clearDialCode = function () {
          $scope.dialcodes = "";
          var currentNode = ecEditor
            .jQuery("#collection-tree")
            .fancytree("getRootNode")
            .getFirstChild().data;
          _.has(stateService.state.invaliddialCodeMap, currentNode.id)
            ? _.unset(stateService.state.invaliddialCodeMap, currentNode.id)
            : "";
          _.has(stateService.state.dialCodeMap, currentNode.id)
            ? _.unset(stateService.state.dialCodeMap, currentNode.id)
            : "";
          if (
            currentNode.metadata &&
            currentNode.metadata.dialcodes &&
            currentNode.metadata.dialcodes.length
          ) {
            if (
              org.ekstep.collectioneditor.cache.nodesModified &&
              org.ekstep.collectioneditor.cache.nodesModified[currentNode.id]
            ) {
              org.ekstep.collectioneditor.cache.nodesModified[
                currentNode.id
              ].metadata["dialcodes"] = [];
        ...
        $scope.changeDialCode = function () {
          ...
          if (_.isEmpty(this.dialcodes)) {
            ...
            $scope.editFlag = false;
            $scope.errorMessage =
              this.dialcodes.length &&
              !String(this.dialcodes).match(/^[A-Z0-9]{6}$/)
                ? "Please enter valid DIAL code"
                : "";
          }
        };
        ...
        $scope.init = function () {
          ecEditor.addEventListener(
            "editor:dialcode:get",
            $scope.getCurrentDialCode,
            $scope
          );
          ecEditor.addEventListener(
            "editor:metadata:update:dialcode",
            $scope.updateDialCode
          );
        };
        ...
        $scope.updateDialCode = function (event, data) {
          ...
          if (
            ...
            if (node.data.metadata.dialcodes) {
              ...
              !_.isEmpty(org.ekstep.collectioneditor.cache.nodesModified) &&
              org.ekstep.collectioneditor.cache.nodesModified[node.data.id]
            ) {
              $scope.dialcodes =
                org.ekstep.collectioneditor.cache.nodesModified[
          ...
          if ($scope.dialcodes) {
            ...
            $scope.editFlag =
              $scope.dialcodes && $scope.dialcodes.length == $scope.maxChar
                ? true
                : false;
            if (
              ecEditor._.indexOf(
