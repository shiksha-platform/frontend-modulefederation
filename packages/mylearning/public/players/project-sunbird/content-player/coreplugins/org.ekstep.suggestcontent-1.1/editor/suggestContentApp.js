angular.module("suggestcontentApp", []).controller("suggestcontentController", [
  ...
  function ($scope, $timeout) {
    ...
    $scope.init = function () {
      ...
      $scope.metaData.topic = rootNodeMetadata.topic
    ...
    $scope.searchLessons = function () {
      Object.assign(
        ...
        {
          contentType: org.ekstep.collectioneditor.api
      ...
      if ($scope.responseData[topics]) {
        ...
        searchService.search(searchBody, function (err, res) {
          if (err) {
            ...
            $scope.responseData[topics] = _.concat(
    ...
    $scope.onNodeSelect = function () {
        ...
        function (obj) {
          if (obj.type === activeNode.data.objectType && obj.editable) {
            if (
              ...
              obj.editable &&
              activeNode.data.metadata.topic
            ) {
            ...
            if (
              activeNode.data.metadata.keywords &&
              activeNode.data.metadata.keywords.length > 0
            ) {
              var lemmaCheck = _.isObject(
                _.first(activeNode.data.metadata.keywords)
              );
              activeNodeKeywords = lemmaCheck
                ? _.map(activeNode.data.metadata.keywords, "lemma")
                : activeNode.data.metadata.keywords;
            }
          }
        }
      ...
      if (
        activeNodeTopics.length &&
        !_.isEqual(activeNodeTopics.sort(), $scope.metaData.topic)
      )
    ...
    $scope.updateMetaData = function () {
      var getNodesModified = org.ekstep.collectioneditor.api
        .getService("collection")
        ...
        if (node.root) {
          Object.assign(
            ...
            updatedTopics &&
              ...
              !_.isEqual(updatedTopics.sort(), $scope.metaData.topic.sort())
              ? {
                  topic: updatedTopics,
                }
              : {}
          ...
          if (
            updatedTopics &&
            updatedTopics.length &&
            !_.isEqual(updatedTopics.sort(), $scope.metaData.topic)
          ) {
    ...
    $scope.refreshSuggestions = function () {
      ...
      if (currentNodeData.root) {
        Object.assign(
          $scope.metaData,
          _.pick(currentNodeData.metadata, [
            "gradeLevel",
            "board",
            "subject",
            "medium",
