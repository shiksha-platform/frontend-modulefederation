  ...
  function ($scope, $injector, instance) {
    ...
    ctrl.hasInputChanged = function () {
      ...
      ctrl.toastManager(
        ...
        true,
        false,
        ctrl.previewMessages.emptyState
      );
    };
    ctrl.videoLibraryTab = function () {
      ctrl.previewHandler();
      ecEditor
        .jQuery("#" + ctrl.videoLibraryTabElement)
        .unbind("scroll")
    ...
    ctrl.initPopup = function (item) {
      ...
      $scope.$on("ngDialog:opened", function () {
        ecEditor.jQuery(".video-modal .infopopover").popup({
          inline: true,
        ...
        .getVideoLicense(requestObj, fields, function (err, res) {
          if (err) {
            ctrl.toastManager(
              ...
              true,
              false,
              ctrl.previewMessages.invalidYoutubeURL
            );
            ctrl.isPreviewPlaying = false;
            ...
            ctrl.toastManager(
              ...
              true,
              false,
              ctrl.previewMessages.invalidLicense
            );
          }
        });
    };
        ...
        .getVideoLicense(requestObj, fields, function (err, res) {
          if (err) {
            ctrl.toastManager(
              ...
              true,
              false,
              ctrl.previewMessages.invalidDriveURL
            );
            ctrl.isPreviewPlaying = false;
          } else if (
            res.data.result.size.valid &&
            res.data.result.size.value <= ctrl.videoSizeInBytes
          ) {
            video
              .play()
              .then(function () {
              ...
              .catch(function (err) {
                ctrl.toastManager(
                  ...
                  true,
                  false,
                  ctrl.previewMessages.previewError
                );
                ctrl.isPreviewPlaying = false;
                ...
                var object = {
                  ...
                  ver:
                    (!_.isUndefined(pkgVersion) && pkgVersion.toString()) ||
                    "0",
                  type: "Content",
                };
                org.ekstep.contenteditor.api
    ...
    setTimeout(function () {
      ctrl.generateImpression({
        ...
        subtype: "popup-open",
        pageid: "VideoBrowser",
        duration: new Date() - instance.pluginLoadStartTime,
      });
    ...
    ctrl.VideoSource = function (video) {
      ecEditor
        .jQuery("#checkBox_" + video.identifier + " >.radioBox")
        .not(":checked")
        .prop("checked", true);
