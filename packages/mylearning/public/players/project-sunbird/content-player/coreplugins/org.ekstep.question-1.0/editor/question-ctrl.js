angular
  .module("org.ekstep.question", ["org.ekstep.metadataform"])
  .controller("QuestionCreationFormController", [
    "$scope",
    "instance",
    "questionData",
    function ($scope, instance, questionData) {
      ...
      _.each($scope.templatesType, function (template, key) {
        ...
        $scope.templateIcons.push({
          icon: ecEditor.resolvePluginResource(
            instance.manifest.id,
            instance.manifest.ver,
            "assets/" + templateIconName + ".png"
          ),
          name: template,
        });
      });
      ...
      $scope.messages = {
        ...
        formulaLimitMsg:
          "Preview the question and split long formulae to ensure they are displayed correctly.",
        layoutChangeMsg:
          "Please check preview before saving. Entire question might not fit in the layout selected.",
      };
      $scope.init = function () {
        ...
        var formSuccessEvents =
          EventBus.listeners[$scope._constants.EVENT_FORM_SUCCESS];
        _.each(formSuccessEvents, function (obj) {
          var eventScope = obj.scope;
          if (
            eventScope &&
            eventScope._constants &&
            eventScope._constants.formName
          ) {
            ecEditor.removeEventListener(
              $scope._constants.EVENT_FORM_SUCCESS,
              obj.callback,
              eventScope
            );
          }
        ...
        ecEditor.addEventListener(
          "org.ekstep.mathtext:addEquation",
          $scope.showEquationMessage,
          $scope
        );
      };
      $scope.showTemplates = function () {
        ...
        if ($scope.questionTemplates.length == 0) {
          ...
          ecEditor.dispatchEvent(
            $scope._constants.questionsetPlugin + ":getPlugins",
            function (pluginData) {
              PluginsData = pluginData;
            }
          );
          _.each(PluginsData, function (val, key) {
            ...
            if (val.contentType == "Plugin") {
              ...
              if (!_.isUndefined(pluginManifest)) {
                if (!_.isUndefined(pluginManifest.templates)) {
                  _.each(pluginManifest.templates, function (v, k) {
                    // eslint-disable-line no-unused-vars
                    v.pluginID = pluginID;
                    v.ver = ver;
      ...
      $scope.validateQuestionCreationForm = function () {
        var pluginInstance = $scope.createPluginInstance(
          $scope.selectedTemplatePluginData.plugin.id
        ); // Plugin id is based on template choosen
      ...
      $scope.getPreviewInstance = function () {
        ...
        if (_.isUndefined(previewInstance)) {
          previewInstance = $scope.createPluginInstance(
      ...
      $scope.setPreviewData = function () {
        ...
        var qObj = {
          config:
            '{"metadata":{"title":"question title","description":"question description","medium":"English"},"max_time":0,"max_score":' +
            $scope.questionData.max_score +
            ',"partial_scoring":' +
            $scope.questionData.isPartialScore +
            ',"isShuffleOption":' +
            $scope.questionData.isShuffleOption +
            ',"layout":' +
            JSON.stringify($scope.questionData.templateType) +
            ',"evalUnordered":' +
            $scope.questionData.evalUnordered +
            "}",
      ...
      $scope.formIsValid = function () {
        ...
        if ($scope.category == "FTB") {
          $scope.questionData.title = _.isUndefined(
            $scope.questionData.questionTitle
          )
            ? $scope.questionCreationFormData.question.text.replace(
                /\[\[.*?\]\]/g,
                "____"
              )
            : $scope.questionData.questionTitle;
          ...
          $scope.questionData.title = _.isUndefined(
            ...
            ? $scope.questionCreationFormData.question.text
            : $scope.questionData.questionTitle;
        }
        $scope.questionData.title = $scope.extractHTML(
          $scope.questionData.title
        );
        ...
        if ($scope.questionMetaData.concepts) {
          $scope.questionMetaData.conceptData =
            "(" + $scope.questionData.concepts.length + ") concepts selected";
        }
        if ($scope.questionMetaData.topic) {
          $scope.questionMetaData.topicData =
            "(" + $scope.questionData.topic.length + ") topics selected";
        }
      ...
      $scope.sendMetaData = function (newQuestionCreate) {
        ...
        if (
          ...
          EventBus.hasEventListener("org.ekstep.questionunit:ready")
        ) {
          EventBus.listeners["org.ekstep.questionunit:ready"] = [];
        }
        var formElement = $($scope._constants.formElementId).find(
          "#content-meta-form"
        );
        var frmScope = formElement.scope();
        ecEditor.dispatchEvent("metadata:form:onsuccess", {
          target: $scope._constants.formElementId,
          form: frmScope.metaForm,
        });
      };
      $scope.saveMetaData = function (event, object) {
        if (
          object.formData.target &&
          object.formData.target.tempalteName &&
          object.formData.target.tempalteName.toLowerCase() ==
            $scope._constants.metadataFormName.toLowerCase()
        ) {
          if (object.isValid) {
            ...
            data.config = {
              ...
              max_time: 0,
              max_score: $scope.questionData.max_score,
              partial_scoring: $scope.questionData.isPartialScore,
              layout: $scope.questionData.templateType,
              isShuffleOption: $scope.questionData.isShuffleOption,
              questionCount: 1,
              evalUnordered: $scope.questionData.evalUnordered,
            };
            ...
            var dynamicOptions = [
              { answer: true, value: { type: "text", asset: "1" } },
            ];
            var mtfoptions = [
              {
                value: {
      ...
      $scope.saveQuestion = function (assessmentId, data) {
        ...
        ecEditor
          ...
          .saveQuestionV3(assessmentId, data, function (err, resp) {
            if (!err) {
              $scope.savingQuestion = false;
              ...
              if ($scope.isNewQuestion) {
                ...
                ecEditor.dispatchEvent(
                  $scope._constants.questionbankPlugin + ":saveQuestion",
                  qMetadata
                );
                $scope.resetPreview();
                $scope.$safeApply();
      ...
      $scope.showQuestionForm = function (qData) {
        ...
        $scope.editMode = true;
        var questionData1 =
          typeof questionData.body == "string"
            ? JSON.parse(questionData.body)
            : questionData.body;
        ...
        if (!questionData.identifier) {
          ...
          $scope.questionData.gradeLevel =
            questionData1.data.config.metadata.gradeLevel;
          $scope.questionData.medium =
            questionData1.data.config.metadata.medium;
          $scope.questionData.subject =
            questionData1.data.config.metadata.subject;
          if (questionData1.data.config.metadata.learningOutcome)
            $scope.questionData.learningOutcome =
              questionData1.data.config.metadata.learningOutcome;
        }
        $scope.questionData.templateType = questionData1.data.config.layout;
        $scope.questionData.isPartialScore =
          questionData1.data.config.partial_scoring;
        $scope.questionData.isShuffleOption =
          questionData1.data.config.isShuffleOption;
        $scope.questionData.evalUnordered =
          questionData1.data.config.evalUnordered || false;
        $scope.category = questionData.category;
        if (questionData1.data.config.metadata.concepts) {
          $scope.Totalconcepts =
            questionData1.data.config.metadata.concepts.length;
        }
        if (questionData.topic) {
          $scope.Totaltopics = questionData.topic.length;
        }
        $scope.questionData.concepts =
          questionData1.data.config.metadata.concepts;
        $scope.questionData.topic = questionData.topic;
        $scope.selectedConceptsData =
          questionData1.data.config.metadata.concepts;
        $scope.selectedTopicsData = questionData.topic;
        $scope.questionData.description =
          questionData1.data.config.metadata.description;
        $scope.questionData.max_score =
          questionData1.data.config.metadata.max_score;
        ...
        var editCreateQuestionFormInstance =
          org.ekstep.pluginframework.pluginManager.getPluginManifest(
            questionData1.data.plugin.id
          );
        ...
        _.each(editCreateQuestionFormInstance.templates, function (value, key) {
          ...
          if (value.editor.template == questionData1.data.plugin.templateId) {
            var templatePathEdit = ecEditor.resolvePluginResource(
              pluginID,
              pluginVer,
        ...
        $scope.selectedTemplatePluginData.plugin = {
          // Question Unit Plugin Information
