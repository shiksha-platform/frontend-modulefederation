angular.module("editorApp").controller("org.ekstep.config:configController", [
  "$scope",
  "$timeout",
  "$ocLazyLoad",
  function ($scope, $timeout, $ocLazyLoad) {
    ...
    var addWatch = function () {
      return $scope.$watch(
        ...
        function (newValue, oldValue) {
          org.ekstep.contenteditor.api.dispatchEvent("config:updateValue", {
            newValue: newValue,
            oldValue: oldValue,
          });
        },
    ...
    $scope.showProperties = function (event, data) {
      var properties = org.ekstep.contenteditor.api.getCurrentObject()
        ? org.ekstep.contenteditor.api.getCurrentObject().getProperties()
        : org.ekstep.contenteditor.api.getCurrentStage().getProperties();
    ...
    $scope.showConfig = function () {
      unregisterWatch();
      $scope.settingsCategory.selected = "customize";
      var pluginConfigManifest = org.ekstep.contenteditor.api._.clone(
        org.ekstep.contenteditor.api.getCurrentObject()
          ? org.ekstep.contenteditor.api.getCurrentObject().getConfigManifest()
          : org.ekstep.contenteditor.api.getCurrentStage().getConfigManifest()
      );
      $scope.pluginConfig = pluginConfigManifest;
      $scope.configData = org.ekstep.contenteditor.api._.clone(
        org.ekstep.contenteditor.api.getCurrentObject()
          ? org.ekstep.contenteditor.api.getCurrentObject().getConfig()
          : org.ekstep.contenteditor.api.getCurrentStage().getConfig()
      );
      if (org.ekstep.contenteditor.api._.isUndefined(pluginConfigManifest)) {
        pluginConfigManifest = [];
        org.ekstep.contenteditor.api.getCurrentObject()
          ? org.ekstep.contenteditor.api.getCurrentObject().renderConfig()
          : org.ekstep.contenteditor.api.getCurrentStage().renderConfig();
      }
      unregisterWatch = addWatch();
      org.ekstep.contenteditor.api._.forEach(
        pluginConfigManifest,
        function (config) {
    ...
    $scope._showConfig = function (config, configData) {
      ...
      if (config.dataType === "colorpicker") {
        var eventData = {
          ...
          callback: function (key, value) {
            org.ekstep.contenteditor.api.dispatchEvent("config:on:change", {
              key: key,
              value: value,
            });
          },
        ...
        setTimeout(function () {
          org.ekstep.contenteditor.api.dispatchEvent(
            "colorpicker:state",
            eventData
          );
        }, 500);
      ...
      if (config.dataType === "rangeslider") {
        setTimeout(function () {
          org.ekstep.contenteditor.api
            .jQuery("#" + config.propertyName)
            .on("change mouseclick", function () {
              org.ekstep.contenteditor.api
                .jQuery("#" + config.propertyName + "label")
                .html(org.ekstep.contenteditor.api.jQuery(this).val());
              org.ekstep.contenteditor.api.dispatchEvent(
                "config:on:change",
                config.propertyName,
                org.ekstep.contenteditor.api.jQuery(this).val()
              );
            });
        }, 500);
      }
      if (config.dataType === "inputSelect") {
        if (
          !_.includes(
            config.range,
            parseInt(instance.configData[config.propertyName])
          )
        ) {
            ...
            .dropdown({
              ...
              action: function (text, value, element) {
                if (
                  ...
                  parseInt(text, 10) < config.minValue ||
                  parseInt(text, 10) > config.maxValue
                ) {
                  instance.configData[config.propertyName] =
                    config.defaultValue;
                  org.ekstep.contenteditor.api.dispatchEvent(
                    "config:on:change",
                    config.propertyName,
                  ...
                  org.ekstep.contenteditor.api
                    .jQuery("#" + config.propertyName)
                    .parent()
                    .dropdown("set text", config.defaultValue);
                } else {
                  ...
                  org.ekstep.contenteditor.api
                    .jQuery("#" + config.propertyName)
                    .parent()
                    .dropdown("set text", parseInt(text));
                }
                $scope.$safeApply();
              },
            });
    ...
    org.ekstep.contenteditor.api
      .jQuery("#actionTypeDropdown")
      .on("change", function (e) {
        $scope.actionTargetObject = {};
        var selectedOption = org.ekstep.contenteditor.api
          .jQuery(this)
          .val()
          .split(":")[1];
    ...
    $scope.addAction = function (data) {
      if (data.command && data.asset) {
        if (stageActionsList[data.command]) {
          ecEditor.getCurrentObject().addEvent({
            type: "click",
            action: [
              {
                id: UUID(),
                ...
                asset: "theme",
                value: data.asset,
                name: $scope.actionTargetObject[data.asset],
              },
            ],
          });
        } else {
      ...
      if (events && events.length) {
        org.ekstep.contenteditor.api._.forEach(events, function (e) {
          if (e.action && e.action.length) {
            eventsActionList.push(e.action[0]);
          }
        });
      }
    ...
    $scope.highlightTargetObject = function () {
      var instance = this;
      ecEditor
        .jQuery("#actionTargetDropdown:not(.addClick)")
        .parent()
        .on("click", function () {
          ecEditor
            .jQuery("#actionTargetDropdown")
            .nextAll(".menu.transition")
            .find(".item")
            .mouseover(function (event) {
              ...
              if (pluginInstance && pluginInstance["editorObj"]) {
                ...
                ecEditor
                  .jQuery("#objectPointer")
                  .show()
                  .offset({ left: left, top: top })
                  .css({
                    height: editorObj.getHeight() + 10,
                    width: editorObj.getWidth() + 10,
                  });
              } else {
    ...
    $scope.setVisibleObjects = function () {
      ...
      var pluginInstances = ecEditor.getStagePluginInstances(
        ecEditor.getCurrentStage().id,
        null,
        ["org.ekstep.audio", "org.ekstep.image"],
        [ecEditor.getCurrentObject().id]
      );
      ecEditor._.forEach(pluginInstances, function (pi) {
        pluginInstanceIds[pi.id] =
          pi.getDisplayName() + " (" + pi["id"].substr(0, 15) + "..." + ")";
      });
      var imageInstances = ecEditor.getStagePluginInstances(
        ecEditor.getCurrentStage().id,
        ["org.ekstep.image"],
        null,
        [ecEditor.getCurrentObject().id]
    ...
    $scope.setStageObjects = function () {
      var stageOptions = {};
      org.ekstep.contenteditor.api._.forEach(
        org.ekstep.contenteditor.api._.clone(
    ...
    $scope.removeAction = function (data) {
      if (data.index > -1) {
        org.ekstep.contenteditor.api
          .getCurrentObject()
          .event.splice(parseInt(data.index), 1);
        $scope.updateActions();
      }
    };
    ...
    org.ekstep.contenteditor.api.addEventListener(
      "object:unselected",
      $scope.objectUnselected,
      $scope
    );
    org.ekstep.contenteditor.api.addEventListener(
      "config:show",
      $scope.showSettingsTab,
      $scope
    );
    org.ekstep.contenteditor.api.addEventListener(
    ...
    org.ekstep.contenteditor.api.addEventListener(
      "config:comments:show",
      $scope.showCommentsTab,
      $scope
    );
    org.ekstep.contenteditor.api.addEventListener(
      "config:show:actions",
