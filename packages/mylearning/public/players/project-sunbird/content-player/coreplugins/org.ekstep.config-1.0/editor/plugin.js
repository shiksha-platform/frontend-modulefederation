...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  initialize: function () {
    ...
    org.ekstep.contenteditor.api.addEventListener(
      "org.ekstep.config:invoke",
      this.invoke,
      this
    );
    org.ekstep.contenteditor.api.addEventListener(
      "org.ekstep.config:toggleStageEvent",
      this.toggleEventToStage,
      this
    );
    org.ekstep.contenteditor.api.addEventListener(
      "config:updateValue",
    ...
    var colorpickerDirPath = org.ekstep.contenteditor.api.resolvePluginResource(
      this.manifest.id,
      this.manifest.ver,
      "editor/colorpicker-directive.js"
    );
    org.ekstep.contenteditor.api
      .getService(ServiceConstants.POPUP_SERVICE)
      .loadNgModules(undefined, colorpickerDirPath);
  },
  updateConfig: function (event, data) {
    var instance = this;
    var changedValues = org.ekstep.contenteditor.api._.reduce(
      data.oldValue,
      function (result, value, key) {
        return org.ekstep.contenteditor.api._.isEqual(value, data.newValue[key])
          ? result
          : result.concat(key);
      },
      []
    ...
    if (data.type == "browser" || data.type == "button") {
      ...
      org.ekstep.contenteditor.api.dispatchEvent(data.event.id, {
        ...
        callback: function (data) {
          instance.onConfigChange(type, data);
        },
      });
    }
  },
  toggleEventToStage: function (event, data) {
    ...
    if (data.flag === true && eventIndex === -1) {
      currentStage.addEvent({
        type: "enter",
        action: [
          { id: UUID(), type: "command", command: "play", asset: data.id },
        ],
      });
