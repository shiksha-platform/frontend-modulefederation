...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  initialize: function () {
    ...
    ecEditor.addEventListener(
      this.type + ":readalong:show",
      this.showReadalong,
      this
    );
    ecEditor.addEventListener(
      this.type + ":wordinfo:show",
      this.showWordInfo,
      this
    );
    ecEditor.addEventListener(
      this.type + ":delete:enhancement",
      this.deleteEnhancement,
      this
    );
    ...
    var templatePath = ecEditor.resolvePluginResource(
      ...
      "editor/delete_confirmation_dialog.html"
    );
    ecEditor.getService("popup").loadNgModules(templatePath);
    var transliterationTemplatePath = ecEditor.resolvePluginResource(
      instance.manifest.id,
      instance.manifest.ver,
      "editor/transliterationconfig.html"
    );
    var controllerPath = ecEditor.resolvePluginResource(
      instance.manifest.id,
      instance.manifest.ver,
      "editor/transliterationapp.js"
    );
    var popupService = org.ekstep.contenteditor.api.getService(
  ...
  createTransliteratedText: function (transliteratedText) {
    ...
    org.ekstep.contenteditor.api.instantiatePlugin(
      currentObject.type,
      textAttributes,
      org.ekstep.contenteditor.api.getCurrentStage()
    );
  },
  /**
  ...
  newInstance: function () {
    ...
    if (
      !ecEditor._.isUndefined(this.attributes.timings) ||
      ...
      if (ecEditor._.isUndefined(this.attributes.textType)) {
        ...
        audioObj.src = org.ekstep.contenteditor.mediaManager.getMediaOriginURL(
          audioObj.src
        );
        if (ecEditor._.isUndefined(audioObj.preload)) audioObj.preload = true;
      ...
      !ecEditor._.isUndefined(this.attributes.words) ||
      this.attributes.textType === "wordinfo"
    ) {
      var instance = this;
      this.addMedia({
        id: "org.ekstep.text.popuptint",
        src: ecEditor.resolvePluginResource(
          instance.manifest.id,
          instance.manifest.ver,
          "assets/popuptint.png"
        ),
        type: "image",
  ...
  dblClickHandler: function (event) {
    ...
    var leftSt =
      ecEditor.jQuery("#canvas").offset().left +
      ecEditor.getCurrentObject().editorObj.left;
    var leftEnd = leftSt + ecEditor.getCurrentObject().editorObj.width;
    var topSt =
      ecEditor.jQuery("#canvas").offset().top +
      ecEditor.getCurrentObject().editorObj.top;
    ...
    if (
      event.clientX > leftSt &&
      event.clientX < leftEnd &&
      event.clientY > topSt &&
      event.clientY < topEnd
    ) {
      textEditor.showEditor(ecEditor.getEditorObject().id);
    }
    textEditor.generateTelemetry({
      type: "click",
      subtype: "doubleClick",
      target: "textEditor",
    });
  ...
  getAttributes: function () {
    var attributes = ecEditor._.omit(ecEditor._.clone(this.attributes), [
      "top",
      "left",
      "width",
      "height",
    ...
    attributes.fontsize = this.updateFontSize(
      this.editorObj.get("fontSize"),
      false
    );
    var fontWeight = ecEditor._.isUndefined(this.editorObj.get("fontWeight"))
      ? ""
      : this.editorObj.get("fontWeight") === "bold"
      ? "bold"
      : "";
    var fontStyle = ecEditor._.isUndefined(this.editorObj.get("fontStyle"))
      ? ""
      : this.editorObj.get("fontStyle") === "italic"
      ? "italic"
      : "";
  ...
  getProperties: function () {
    var props = ecEditor._.omitBy(
      ecEditor._.clone(this.attributes),
      ecEditor._.isObject
    );
      ...
      controller: [
        ...
        function ($scope) {
          $scope.warningMessage =
            ecEditor.getCurrentObject().attributes.textType == "readalong"
              ? "Read-Along"
              : "Word Info Popup";
  ...
  isV2Plugin: function (instance) {
    ...
    if (
      !textInstance.attributes.version ||
      textInstance.attributes.version !== "V2"
    ) {
      return false;
    } else {
      return true;
