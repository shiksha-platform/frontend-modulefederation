angular.module("editorApp").controller("org.ekstep.text:config-controller", [
  "$scope",
  function ($scope) {
    var pluginInstance =
      org.ekstep.pluginframework.pluginManager.plugins["org.ekstep.text"].p
        .prototype;
    ...
    $scope.fontSize = [
      18, 20, 22, 24, 26, 28, 32, 36, 40, 44, 48, 54, 60, 66, 72, 80, 88, 96,
    ];
    ...
    ecEditor.jQuery("#WordInfo").click(function (event) {
      if (
        ...
        !$(this).hasClass("word-info-enabled")
      ) {
    ...
    ecEditor.jQuery("#readAlong").click(function (event) {
      if (
        !$(this).hasClass("disabled") &&
        !$(this).hasClass("read-along-enabled")
      ) {
    ...
    setTimeout(function () {
      ...
      if ($scope.configData.autoplay)
        ecEditor.jQuery("#readalongautoplay").checkbox("set checked");
    }, 0);
    ...
    $scope.showWordInfo = function (data) {
      $scope.showColorpicker(
        "wordfontcolorpicker",
        data.config.wordfontcolor || "#0000FF"
      );
      $scope.showColorpicker(
        "wordhighlightcolorpicker",
        data.config.wordhighlightcolor || "#FFFF00"
      );
      $scope.showColorpicker(
        "wordunderlinecolorpicker",
        data.config.wordunderlinecolor || "#0000FF"
      );
    };
    ...
    $scope.toggleActiveAcordionTitle = function (clickEvent) {
      let targetId = ($scope.activeTextPluginControlItem =
    ...
    $scope.unregisterListeners = function () {
      ecEditor.removeEventListener(
        "org.ekstep.text:addWordInfo",
        $scope.onTextSelect,
        $scope
      );
      ecEditor.removeEventListener(
        "org.ekstep.text:addReadAlong",
        $scope.onTextSelect,
        $scope
      );
      ecEditor.removeEventListener(
      ...
      ecEditor.removeEventListener(
        "org.ekstep.text:unselected",
        $scope.unregisterListeners,
        $scope
      );
