...
org.ekstep.questionunitmtf.RendererPlugin =
  org.ekstep.contentrenderer.questionUnitPlugin.extend({
    ...
    preQuestionShow: function (event) {
      ...
      this._question.template = MTFController.getQuestionTemplate(
        this._question.config.layout,
        this._constant
      );
      //The state will created once an is selected, Only once a quesiton will be shuffled
      if (!this._question.state) {
        this._question.data.option.optionsRHS = this.shuffleOptions(
    ...
    evaluateQuestion: function (event) {
      ...
      $(".rhs-block").each(function (elemIndex, elem) {
        ...
        telObj["RHS"][0] =
          instance._question.data.option.optionsRHS[elemMappedIndex];
      ...
      if (this._question.config.partial_scoring) {
        questionScore =
          (correctAnswersCount / totalLHS) * this._question.config.max_score;
      } else {
    ...
    getTelemetryResValues: function () {
      ...
      _.each(rhsOptions, function (obj, index) {
        rhsRerendered.push(
          _.findWhere(rhsOptions, { mapIndex: instance.rhs_rearranged[index] })
        );
      });

      var resValues = [];
      resValues.push({
        lhs: instance.getTelOptions(instance._question.data.option.optionsLHS),
      });
    ...
    getAnswers: function (options) {
      var lhsOptions = options.optionsLHS,
        rhsOptions = options.optionsRHS;
