...
org.ekstep.contenteditor.basePlugin.extend({
      ...
      ecEditor.jQuery("#colUsersDropdown").dropdown({
        apiSettings: {
          url:
            ...
            "/index.php?option=com_ekcontent&task=contentform.getUsersToInvite&id=" +
            window.context.id +
            "&isEditor=true&search={query}",
          cache: true,
        },
        saveRemoteData: true,
        fields: {
  ...
  getUrlLink: function () {
    ...
    try {
      ...
      successful
        ? (instance.copyAnswer = "Copied!")
        : (instance.copyAnswer = "Unable to copy!");
    } catch (err) {
  ...
  sendInvites: function () {
    var instance = this;
    ecEditor.jQuery("#colInviteForm").form({
      inline: true,
      fields: {
        inviteUsers: {
  ...
  notifyUser: function (event, fields) {
    ...
    ecEditor.jQuery.ajax({
      url:
        ecEditor.getConfig("baseURL") +
        "/index.php?option=com_ekcontent&task=contentform.inviteUsers",
      headers: {
        "x-auth": "session",
      },
      type: "POST",
  ...
  collaboratorsInfo: function () {
    ...
    ecEditor.jQuery.ajax({
      url:
        ecEditor.getConfig("baseURL") +
        "/index.php?option=com_api&app=ekcontent&resource=collaborator&format=raw",
      headers: {
        "x-auth": "session",
      },
      type: "GET",
