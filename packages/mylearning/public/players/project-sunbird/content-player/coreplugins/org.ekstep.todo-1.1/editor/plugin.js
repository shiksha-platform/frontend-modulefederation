...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  controllerCallback: function (event, data) {
    ...
    ctrl.initializeTodoWidget = function () {
      ...
      var widgetRef;
      ecEditor
        .jQuery("#todo-sidebar-tab .item")
        .tab("change tab", "reviewerTab");
      /**Check for collabortors tab**/
      isCollaborators = _.isUndefined(
        ecEditor.getService("content").getContentMeta(ctrl.context.contentId)
          .collaborators
      )
        ? false
        : true;
      if (isCollaborators)
        ecEditor.jQuery("#collaboratorTab").removeClass("disabled");
      else {
      ...
      if (!ecEditor._.isUndefined(ctrl.context)) {
        if (
          !ecEditor._.isUndefined(ctrl.context.contentId) &&
          ctrl.context.contentId != ""
        ) {
          // Content url
          requestParams["url"] =
            "index.php?option=com_ekcontent&view=content&id=" + ctrl.context.id;
          ...
          // Content title
          requestParams["title"] = ecEditor
            .getService("content")
            .getContentMeta(ctrl.context.contentId).name;
          ...
          ecEditor
            .jQuery(widgetRef)
            .attr("data-jlike-url", requestParams["url"]);
          ecEditor
            .jQuery(widgetRef)
            .attr("data-jlike-status", requestParams["status"]);
          ecEditor
            .jQuery(widgetRef)
            .attr("data-jlike-type", requestParams["type"]);
          ecEditor
            .jQuery(widgetRef)
            .attr("data-jlike-subtype", requestParams["subtype"]);
          ecEditor
            .jQuery(widgetRef)
            .attr("data-jlike-client", requestParams["client"]);
          ecEditor
            .jQuery(widgetRef)
            .attr("data-jlike-cont-id", requestParams["cont_id"]);
          ecEditor
            .jQuery(widgetRef)
            .attr("data-jlike-title", requestParams["title"]);

          requestParams["content_id"] = ecEditor
            .jQuery(widgetRef)
            .attr("data-jlike-contentid");
          requestParams["assigned_by"] = ecEditor
            .jQuery(widgetRef)
            .attr("data-jlike-assigned_by");
          requestParams["assigned_to"] = ecEditor
            .jQuery(widgetRef)
            .attr("data-jlike-assigned_to");
          ...
          ecEditor
            .jQuery("#collaboratorsCommentsDiv")
            .attr("data-jlike-url", requestParams["url"]);
          // Update subtype based on stageId. subtype is very IMP to load comments
          ecEditor
            .jQuery("#collaboratorsCommentsDiv")
            .attr("data-jlike-subtype", "collaborators#" + data.stageId);
          ecEditor
            .jQuery("#collaboratorsCommentsDiv")
            .attr("data-jlike-cont-id", requestParams["cont_id"]);
    ...
    (ctrl.updateStatus = function (ref) {
      ...
      (ctrl.save = function (widgetRef, status, id, todotext, context) {
        ...
        // Content url
        apiPostParams["url"] = ecEditor
          .jQuery(widgetRef)
          .attr("data-jlike-url");
        apiPostParams["cont_id"] = ecEditor
          .jQuery(widgetRef)
          .attr("data-jlike-cont-id");
        apiPostParams["type"] = "todos";

        // Joomla subtype along with stage Id
        apiPostParams["subtype"] = context;
        apiPostParams["client"] = ecEditor
          .jQuery(widgetRef)
          .attr("data-jlike-client");
        apiPostParams["content_id"] = ecEditor
          .jQuery(widgetRef)
          .attr("data-jlike-contentid");
        apiPostParams["assigned_by"] = ecEditor
          .jQuery(widgetRef)
          .attr("data-jlike-assigned_by");
        apiPostParams["assigned_to"] = ecEditor
          .jQuery(widgetRef)
          .attr("data-jlike-assigned_to");
        ...
        ecEditor
          .jQuery(widgetRef)
          .hybridtodo({ apiPostParams: apiPostParams, action: "createTodo" });
        ctrl.generateTelemetry({
          type: "click",
          subtype: "todo",
          target: "createTodo",
          targetid: ctrl.context.contentId,
        });
      });
    ...
    ctrl.renderHybridTodos = function (result, status, todoThreadsWrapperDiv) {
      ...
      if (result.success == true) {
        ...
        for (var i = 0; i < result.data.result.length; i++) {
          ...
          result.data.result[i].buttonName =
            result.data.result[i].status == "I" ? "Resolve" : "Resolved";
          // Button disabled or not
          result.data.result[i].disabledAttr =
            result.data.result[i].status == "I" ? "" : "disabled";
          // Resolved todo thread should be readonly
          result.data.result[i].readOnly =
            result.data.result[i].status == "I" ? false : true;

          for (var c = 0; c < result.data.result[i].comments.length; c++) {
            result.data.result[i].comments[c].user_name =
              result.data.result[i].comments[c].user.name;
            result.data.result[i].comments[c].user_id =
              result.data.result[i].comments[c].user.id;
            result.data.result[i].comments[c].profile_picture_url =
              result.data.result[i].comments[c].user.avatar;
            result.data.result[i].comments[c].profile_url =
              result.data.result[i].comments[c].user.profile_link;
            result.data.result[i].comments[c].created = moment(
              result.data.result[i].comments[c].annotation_date
          ...
          // Append widget data
          result.data.result[i].status == "I"
            ? ecEditor.jQuery(todoThreadsWrapperDiv).append(markup)
            : ecEditor.jQuery(todoThreadsWrapperDiv).append(markup);
      ...
      // Error handling/show empty message. status == 'I' means Incomplete/unresolved todo
        ...
        var emptyMessage =
          '<div class="ui grid" id="noIssueFound" style="padding-top: 22px;"><div class="ui one column stackable center aligned page grid"><i class="fa fa-comments-o fa-2x" aria-hidden="true"></i><br>No issues found</div></div>';
