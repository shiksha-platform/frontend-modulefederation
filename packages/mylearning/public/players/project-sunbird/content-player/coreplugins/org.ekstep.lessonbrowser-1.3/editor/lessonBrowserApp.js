angular
  .module("org.ekstep.lessonbrowserapp", ["angular-inview", "luegg.directives"])
  .controller("lessonController", [
    ...
    "callback",
    "callerFilters",
    function ($scope, $timeout, instance, callback, callerFilters) {
      var ctrl = this;
      ctrl.facetsResponse = undefined;
      const DEFAULT_PAGEAPI = "ContentBrowser";
      // different html configuration to render dynamically
      $scope.headerTemplate = ecEditor.resolvePluginResource(
        instance.manifest.id,
        instance.manifest.ver,
        "editor/header.html"
      );
      $scope.footerTemplate = ecEditor.resolvePluginResource(
        instance.manifest.id,
        instance.manifest.ver,
        "editor/footer.html"
      );
      $scope.filterTemplate = ecEditor.resolvePluginResource(
        instance.manifest.id,
        instance.manifest.ver,
        "editor/filterTemplate.html"
      );
      $scope.cardTemplate = ecEditor.resolvePluginResource(
        instance.manifest.id,
        instance.manifest.ver,
        "editor/cardRendererTemplate.html"
      );
      $scope.facetsTemplate = ecEditor.resolvePluginResource(
        instance.manifest.id,
        instance.manifest.ver,
        "editor/facetsRenderTemplate.html"
      );
      $scope.cardDetailsTemplate = ecEditor.resolvePluginResource(
        instance.manifest.id,
        instance.manifest.ver,
        "editor/cardDetailsTemplate.html"
      );
      ctrl.contentNotFoundImage = ecEditor.resolvePluginResource(
        instance.manifest.id,
        instance.manifest.ver,
        "assets/content_not_found.jpg"
      );
      ctrl.defaultImage = ecEditor.resolvePluginResource(
        instance.manifest.id,
        instance.manifest.ver,
      ...
      ctrl.learningConfig = function () {
        ...
        ecEditor
          .getService("meta")
          .getCategorys(framework, function (err, res) {
            if (err) {
              ctrl.langErr =
              ...
              ctrl.meta.grades = [];
              ctrl.meta.subjects = [];
              var medium = _.find(res.data.result.framework.categories, [
                "code",
                "medium",
              ]);
              if (medium && medium.terms) ctrl.meta.languages = medium.terms;
              var gradeLevel = _.find(res.data.result.framework.categories, [
                "code",
                "gradeLevel",
              ]);
              if (gradeLevel && gradeLevel.terms)
                ctrl.meta.grades = gradeLevel.terms;
              var subject = _.find(res.data.result.framework.categories, [
                "code",
                "subject",
              ]);
              if (subject && subject.terms) ctrl.meta.subjects = subject.terms;
            }
            $scope.$safeApply();
          });
      };
      ...
      ctrl.searchConcepts = function (content, cb) {
        angular.forEach(content, function (content) {
          conceptRequestBody.request.filters.identifier = content.concepts
            ? conceptRequestBody.request.filters.identifier.concat(
                content.concepts
              )
            : conceptRequestBody.request.filters.identifier;
        });
        conceptRequestBody.request.filters.identifier =
          conceptRequestBody.request.filters.identifier.filter(function (
            currentValue,
            index,
            array
          ) {
      ...
      ctrl.toggleContent = function (Contents) {
        angular.forEach(Contents, function (resource) {
          if ($scope.selectedResources.indexOf(resource.identifier) !== -1) {
            ecEditor
              .jQuery("#checkBox_" + resource.identifier + " >.checkBox")
              .prop("checked", true);
          } else {
            ecEditor
              .jQuery("#checkBox_" + resource.identifier + " >.checkBox")
        ...
        ctrl.searchLessons(function (res) {
          if ($scope.mainTemplate === "selectedResult") {
            ...
            ctrl.meta.lessonTypes =
              collectionService.getObjectTypeByAddType("Browser");
            ctrl.dropdownAndCardsConfig();
            ctrl.setFilterValues();
            ctrl.conceptSelector();
      ...
      $scope.getFiltersValue = function () {
        ...
        $scope.filterSelection.language = ecEditor
          .jQuery("#lessonBrowser_language")
          .dropdown("get value");
        $scope.filterSelection.grade = ecEditor
          .jQuery("#lessonBrowser_grade")
          .dropdown("get value");
        $scope.filterSelection.lessonType = ecEditor
          .jQuery("#lessonBrowser_lessonType")
          .dropdown("get value");
        $scope.filterSelection.subject = ecEditor
          .jQuery("#lessonBrowser_subject")
          .dropdown("get value");

        if ($scope.filterSelection.language.length) {
          $scope.filterSelection.language = _.isString(
            $scope.filterSelection.language
          )
            ? $scope.filterSelection.language.split(",")
            : $scope.filterSelection.language;
        ...
        if ($scope.filterSelection.grade.length) {
          $scope.filterSelection.grade = _.isString(
            $scope.filterSelection.grade
          )
            ? $scope.filterSelection.grade.split(",")
            : $scope.filterSelection.grade;
        ...
        if ($scope.filterSelection.lessonType.length) {
          $scope.filterSelection.lessonType = _.isString(
            $scope.filterSelection.lessonType
          )
            ? $scope.filterSelection.lessonType.split(",")
            : $scope.filterSelection.lessonType;
          searchBody.request.filters.contentType =
            $scope.filterSelection.lessonType;
        } else {
        ...
        if ($scope.filterSelection.subject.length) {
          $scope.filterSelection.subject = _.isString(
            $scope.filterSelection.subject
          )
            ? $scope.filterSelection.subject.split(",")
            : $scope.filterSelection.subject;
        ...
        if ($scope.filterSelection.concept.length) {
          $scope.filterSelection.concept = _.isString(
            $scope.filterSelection.concept
          )
            ? $scope.filterSelection.concept.split(",")
            : $scope.filterSelection.concept;
      ...
      $scope.resetFilters = function () {
        ...
        if ($scope.filterSelection.subject.length) {
          $scope.filterSelection.subject.splice(
            0,
            $scope.filterSelection.subject.length
          );
        }
        if ($scope.filterSelection.grade.length) {
        ...
        if ($scope.filterSelection.language.length) {
          $scope.filterSelection.language.splice(
            0,
            $scope.filterSelection.language.length
          );
        }
        if ($scope.filterSelection.lessonType.length) {
      ...
      ctrl.setFilterValues = function () {
        if (
          ...
          angular.forEach(
            $scope.filterSelection.lessonType,
            function (lessonType) {
              $("#lessonBrowser_lessonType").dropdown(
                "set selected",
                lessonType
              );
            }
        ...
        if (
          ...
          $("#lessonBrowser_concepts").val(
            $scope.filterSelection.concept.length + " selected"
          );
        }
      };
      ...
      $scope.toggleSelectionLesson = function (lesson) {
        ...
        if (idx > -1) {
          ...
          if ($scope.mainTemplate != "addedItemsView") {
            ecEditor
              .jQuery("#checkBox_" + lesson.identifier + " >.checkBox")
              .prop("checked", false);
          }
      ...
      $scope.searchByKeyword = function () {
        ...
        searchService.search(searchBody, function (err, res) {
          if (err) {
            ...
            ecEditor
              .jQuery(".searcher ul.searchList li.searchResult")
              .each(function () {
                var matchStart = ecEditor
                  .jQuery(this)
                  .text()
                  .toLowerCase()
                  .indexOf("" + searchQuery.toLowerCase() + "");
                var matchEnd = matchStart + searchQuery.length - 1;
                var beforeMatch = ecEditor
                  .jQuery(this)
                  .text()
                  .slice(0, matchStart);
                var matchText = ecEditor
                  .jQuery(this)
                  .text()
                  .slice(matchStart, matchEnd + 1);
                var afterMatch = ecEditor
                  .jQuery(this)
                  .text()
                  .slice(matchEnd + 1);
                ecEditor
      ...
      $scope.getPageAssemble = function (cb) {
        ...
        let service = org.ekstep.contenteditor.api.getService(
          ServiceConstants.META_SERVICE
        );
          ...
          $scope.getPageAssemble(function (err, res) {
            if (res) {
              ...
              if (ctrl.facetsResponse.result.response.sections) {
                angular.forEach(
                  ctrl.facetsResponse.result.response.sections,
              ...
              ecEditor.dispatchEvent("org.ekstep.toaster:warning", {
                message:
                  "Unable to load categories to browse resources. You can use search to find resources.",
                position: "topCenter",
                icon: "fa fa-warning",
              });
      ...
      $scope.viewAll = function (query, sectionIndex) {
        ...
        if (query.request.filters.contentType) {
          query.request.filters.contentType = _.isString(
            query.request.filters.contentType
          )
            ? query.request.filters.contentType.split(",") || []
            : query.request.filters.contentType;
          query.request.filters.contentType = _.intersectionWith(
            ctrl.meta.lessonTypes,
            query.request.filters.contentType,
            _.isEqual
          );
        ...
        if (query.request.filters.gradeLevel) {
          query.request.filters.gradeLevel = _.isString(
            query.request.filters.gradeLevel
          )
            ? query.request.filters.gradeLevel.split(",") || []
            : query.request.filters.gradeLevel;
          $scope.filterSelection.grade = query.request.filters.gradeLevel;
          searchBody.request.filters.gradeLevel =
            query.request.filters.gradeLevel;
        } else {
          delete searchBody.request.filters.gradeLevel;
        }
        if (query.request.filters.language) {
          query.request.filters.language = _.isString(
            query.request.filters.language
          )
            ? query.request.filters.language.split(",") || []
            : query.request.filters.language;
        ...
        if (query.request.filters.subject) {
          query.request.filters.subject = _.isString(
            query.request.filters.subject
          )
            ? query.request.filters.subject.split(",") || []
            : query.request.filters.subject;
          ...
          if (!$scope.viewAllAvailableResponse.hasOwnProperty(sectionIndex)) {
            ...
            ctrl.searchLessons(function (res) {
              ...
              if (!_.isUndefined(sectionIndex))
                $scope.viewAllAvailableResponse[sectionIndex] =
                  $scope.defaultResources;
              $scope.$safeApply();
            });
          } else {
...
// slider directive
angular
  .module("org.ekstep.lessonbrowserapp")
