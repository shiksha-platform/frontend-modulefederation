angular.module("coursemetaApp", []).controller("coursemetaController", [
  "$scope",
  "$timeout",
  function ($scope, $timeout) {
    ...
    $scope.updateNode = function () {
      if (!_.isEmpty($scope.nodeId) && !_.isUndefined($scope.nodeId)) {
        ...
        if (
          !_.isEmpty($scope.course.language) &&
          _.isString($scope.course.language)
        ) {
          $scope.course.language = [$scope.course.language];
        }
        if (
          !_.isEmpty($scope.course.audience) &&
          _.isString($scope.course.audience)
        ) {
        ...
        org.ekstep.collectioneditor.cache.nodesModified[
          ...
          org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
            .metadata,
          $scope.getUpdatedMetadata($scope.metadataCloneObj, $scope.course)
        );
        var keywords =
          org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
            .metadata.keywords;
        ...
        if (!_.isUndefined(activeNode.data.metadata.concepts)) {
          ...
          if ($scope.course.concepts.length > 0) {
            $scope.course.conceptData =
              "(" + $scope.course.concepts.length + ") concepts selected";
    ...
    ecEditor.addEventListener(
      "org.ekstep.collectioneditor:node:selected:Course",
      $scope.onNodeSelect
    );
    ...
    $scope.generateTelemetry = function (data) {
      if (data)
        org.ekstep.services.telemetryService.interact({
          ...
          target: data.target,
          pluginid: "org.ekstep.coursemeta",
          pluginver: "1.2",
    ...
    $scope.init = function () {
      $scope.$watch(
        ...
        function () {
          if ($scope.course) {
            if (/^[a-z\d\-_\s]+$/i.test($scope.course.name) == false)
              $scope.course.name =
    ...
    $scope.changeTitle = function () {
      org.ekstep.collectioneditor.api
        .getService("collection")
        .setNodeTitle($scope.course.name);
    };
