...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  initialize: function () {
    ...
    ecEditor.addEventListener(
      instance.manifest.id + ":assetbrowser:open",
      this.openBrowser,
      this
    );
    ecEditor.addEventListener(
      instance.manifest.id + ":removeAudio",
      this.removeAudio,
      this
    );
    ecEditor.addEventListener(
      instance.manifest.id + ":toggleAudio",
      this.toggleAudio,
      this
    );
    ecEditor.addEventListener(
      instance.manifest.id + ":jplayerInit",
      this.jplayerInit,
      this
    );
  },
  ...
  newInstance: function () {
    ...
    //this.id = media.id;
    if (media && media.src) {
      media.src = org.ekstep.contenteditor.mediaManager.getMediaOriginURL(
        media.src
      );
      this.addMedia(media);
      this.audioData = {
      ...
      ecEditor.dispatchEvent(
        "org.ekstep.stageconfig:addcomponent",
        this.audioData
      );
    } else {
  ...
  toggleAudio: function (event, data) {
    ...
    ecEditor.dispatchEvent("org.ekstep.config:toggleStageEvent", {
      flag: data.autoplay,
      id: data.assetId,
    });
  },
