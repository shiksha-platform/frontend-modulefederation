window.assessmentBrowserUtil = (function () {
    ...
    try {
      ...
      var story = {
        theme: {
          ...
          startStage: "assessmentStage",
          id: "theme",
          ver: 0.3,
          stage: [
            { id: "baseStage", preload: true, image: [], audio: [], voice: [] },
            {
              id: "assessmentStage",
              x: 0,
              y: 0,
      ...
      story.theme.controller[0].__cdata = {
        total_items: 1,
        SET_TYPE: "MATERIALISED_SET",
        SET_OBJECT_TYPE_KEY: "AssessmentItem",
        item_sets: [{ id: "itemSet", count: 1 }],
        items: { itemSet: [itemJson] },
        identifier: "itemSet",
      };

      var templates = ecEditor._.isUndefined(templateJson.theme.template)
        ? []
        : ecEditor._.isArray(templateJson.theme.template)
        ? templateJson.theme.template
        : [templateJson.theme.template];
      ecEditor._.forEach(templates, function (t) {
        if (
          t &&
          ecEditor._.findIndex(story.theme.template, function (st) {
            return st.id == t.id;
          }) < 0
        ) {
      ...
      if (
        ecEditor._.has(templateJson, "theme.manifest") &&
  ...
  function addMediaToStory(story, media) {
    media = ecEditor._.isUndefined(media)
      ? []
      : ecEditor._.isArray(media)
      ? media
      : [media];
    var idIndex, srcIndex;
    ecEditor._.forEach(media, function (m) {
      if (m.id && m.src) {
        srcIndex = ecEditor._.findIndex(
          story.theme.manifest.media,
          function (sm) {
            return sm.src === m.src;
          }
        );
        idIndex = ecEditor._.findIndex(
          story.theme.manifest.media,
          function (sm) {
            return sm.id === m.id;
          }
