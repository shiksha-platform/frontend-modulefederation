 ...
 */
angular
  .module("seqApp", ["org.ekstep.question"])
  .controller("seqQuestionFormController", [
    "$scope",
    "$rootScope",
    "questionServices",
    function ($scope, $rootScope, questionServices) {
      ...
      var questionInput = CKEDITOR.replace("seqQuestion", {
      ...
      questionInput.on("focus", function () {
        $scope.generateTelemetry({
          ...
          id: "input",
          target: {
            id: "questionunit-sequence-question-on-focus",
            ver: "",
            type: "input",
          },
        });
      ...
      angular.element(".innerScroll").on("scroll", function () {
        $scope.generateTelemetry({
          ...
          id: "form",
          target: {
            id: "questionunit-sequence-form-scroll",
            ver: "",
            type: "form",
          },
        });
      ...
      $scope.init = function () {
        ...
        ecEditor.addEventListener(
          "org.ekstep.questionunit.sequence:validateform",
        ...
        EventBus.listeners["org.ekstep.questionunit.sequence:editquestion"] =
          [];
        ecEditor.addEventListener(
          "org.ekstep.questionunit.sequence:editquestion",
          $scope.editSEQQuestion,
          $scope
        );
      ...
      $scope.addOption = function (index) {
        var telemetryObject = {
          ...
          id: "button",
          target: {
            id: "questionunit-sequence-add-sequence",
            ver: "",
            type: "button",
          },
        };
      ...
      $scope.formValidation = function () {
        ...
        formValid =
          $scope.seqForm.$valid && $scope.seqFormData.options.length > 1;
        if (
          !(
            $scope.seqFormData.question.text.length ||
            $scope.seqFormData.question.image.length ||
            $scope.seqFormData.question.audio.length
          )
        ) {
      ...
      $scope.deleteOption = function (id, $repeatScope) {
        var telemetryObject = {
          ...
          id: "button",
          target: {
            id: "questionunit-sequence-delete-sequence",
            ver: "",
            type: "button",
          },
        };
      ...
      $scope.addMedia = function (type, index, mediaType) {
        var telemetryObject = {
          ...
          id: "button",
          target: {
            id: "questionunit-sequence-add-" + mediaType,
            ver: "",
            type: "button",
          },
        };
        ...
        mediaObject.callback = function (data) {
          ...
          if (type == "q") {
            telemetryObject.target.id =
              "questionunit-sequence-question-add-" + data.assetMedia.type;
            $scope.seqFormData.question[data.assetMedia.type] =
              org.ekstep.contenteditor.mediaManager.getMediaOriginURL(
                data.assetMedia.src
              );
            data.assetMedia.type == "audio"
              ? ($scope.seqFormData.question.audioName = data.assetMedia.name)
              : "";
            $scope.questionMedia[data.assetMedia.type] = media;
          } else {
            telemetryObject.target.id =
              "questionunit-sequence-option-add-" + data.assetMedia.type;
            $scope.seqFormData.options[index][data.assetMedia.type] =
              org.ekstep.contenteditor.mediaManager.getMediaOriginURL(
                data.assetMedia.src
              );
            data.assetMedia.type == "audio"
              ? ($scope.seqFormData.options[index].audioName =
      ...
      $scope.deleteMedia = function (type, index, mediaType) {
        var telemetryObject = {
          ...
          id: "button",
          target: {
            id: "questionunit-sequence-delete-" + mediaType,
            ver: "",
            type: "button",
          },
        };
        if (type == "q") {
          telemetryObject.target.id =
            "questionunit-sequence-delete" + mediaType;
          $scope.seqFormData.question[mediaType] = "";
          delete $scope.questionMedia[mediaType];
        } else {
          telemetryObject.target.id =
            "questionunit-sequence-option-delete-" + mediaType;
