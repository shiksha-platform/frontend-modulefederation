angular
  .module("org.ekstep.genericeditor", ["Scope.safeApply", "oc.lazyLoad"])
  .controller("mainController", [
    "$scope",
    "$location",
    "$ocLazyLoad",
    function ($scope, $location, $ocLazyLoad) {
      ...
      org.ekstep.contenteditor.canvasManager.initialize({
        loadNgModules: $scope.loadNgModules,
        scope: $scope,
      });
      ...
      $scope.fireSidebarTelemetry = function (menu, menuType) {
        ...
        $scope.telemetryService.interact({
          type: "modify",
          subtype: "sidebar",
          target: menuType,
          pluginid: pluginId || "",
      ...
      $scope.telemetry = function (data) {
        var manifest =
          org.ekstep.pluginframework.pluginManager.getPluginManifest(
            "org.ekstep.genericeditor"
          );
        org.ekstep.services.telemetryService.interact({
          type: "click",
          subtype: data.subtype,
          target: data.target,
          pluginid: manifest.ver,
          pluginver: manifest.id,
      ...
      ecEditor.addEventListener(
        "org.ekstep.genericeditor:preview",
        function () {
          ...
          if (ecEditor.getContext("contentId")) {
            ...
            if (!ecEditor.getContext("framework")) {
              ...
              ecEditor
                ...
                .getFrameworks(channel, function (err, res) {
                  if (res && res.data) {
                    ecEditor.setContext(
                      "framework",
                      res.data.result.channel.defaultFramework
                    );
                    org.ekstep.services.telemetryService.impression({
                      type: "edit",
                    ...
                    ecEditor
                      .jQuery(".pg-loading-html")
                      .prepend(
                        '<p class="loading-message">Unable to fetch defaultFramework! Please try again later</p><button class="ui red button" onclick="ecEditor.dispatchEvent(\'org.ekstep.collectioneditor:content:notfound\');"><i class="window close icon"></i>Close Editor!</button>'
                      );
                  }
                });
            } else {
