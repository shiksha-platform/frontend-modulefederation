angular.module("unitmetaApp", []).controller("unitmetaController", [
  "$scope",
  function ($scope) {
    ...
    $scope.updateNode = function () {
      if (!_.isEmpty($scope.nodeId) && !_.isUndefined($scope.nodeId)) {
        ...
        org.ekstep.collectioneditor.cache.nodesModified[
          ...
          org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
            .metadata,
          $scope.getUpdatedMetadata($scope.metadataCloneObj, $scope.unit)
        );
        org.ekstep.collectioneditor.cache.nodesModified[
          $scope.nodeId
        ].metadata.framework = frameworkId;
        delete org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
          .metadata.topicData;
        var keywords =
          org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
            .metadata.keywords;
    ...
    $scope.onNodeSelect = function (event, data) {
      ...
      if (
        ...
        if (!_.isUndefined(activeNode.data.metadata.topic)) {
          ...
          if ($scope.unit.topic.length > 0) {
            $scope.unit.topicData =
              "(" + $scope.unit.topic.length + ") topics selected";
      ...
      if (
        org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId] &&
        org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId].metadata
      ) {
        delete org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
          .metadata.topicData;
      }
    ...
    ecEditor.addEventListener(
      "org.ekstep.collectioneditor:node:selected:TextBookUnit",
      $scope.onNodeSelect
    );
        ...
        function () {
          if ($scope.unit) {
            ...
            if ($scope.nodeType === DEFAULT_NODETYPE) {
              activeNode = org.ekstep.collectioneditor.api
                .getService("collection")
                .getActiveNode();
