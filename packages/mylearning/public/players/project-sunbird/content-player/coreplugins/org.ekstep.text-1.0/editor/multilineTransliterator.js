...
org.ekstep.text.MultilineTransliterator = Class.extend({
  ...
  transliterate: function (text, languageCode, callback) {
    ...
    var promises = _.map(texts, function (text) {
      ...
      return promisify(
        instance.transliterateService.getTransliteration.bind(
          instance.transliterateService
        ),
        data
      );
    });
    ...
    var mapValues = function (obj, callback) {
      if (angular.isArray(obj)) return obj.map(callback);
    ...
    allSettled(promises).then(function (result) {
      ...
      var transliteratedText = _.map(result, function (item, index) {
        if (
          ...
          item.value.data.result.transliterations[languageCode] != undefined
        ) {
          var val =
            item["value"]["data"]["result"]["transliterations"][languageCode][
              "output"
            ];
          if (val) {
...
org.ekstep.text.MultilineTransliterator.create = function (
  $q,
