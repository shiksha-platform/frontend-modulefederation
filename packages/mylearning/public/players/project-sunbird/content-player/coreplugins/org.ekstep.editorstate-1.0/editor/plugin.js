org.ekstep.contenteditor.basePlugin.extend({
    ...
    ecEditor.addEventListener(
      ...
      function () {
        ecEditor.dispatchEvent(
          instance.manifest.id + ":state",
          instance.getEditorState(),
          instance
        );
      },
      this
  ...
  _getStateFromService: function () {
    if (
      ecEditor
        ...
        .getContentMeta(ecEditor.getContext("contentId")).editorState
    ) {
      return JSON.parse(
        ecEditor
          .getService(ServiceConstants.CONTENT_SERVICE)
  ...
  setStageState: function () {
    ...
    if (editorState && editorState.stage) {
      ecEditor.dispatchEvent("stage:select", {
        stageId: editorState.stage.currentStage,
      });
      //set current selected object
      if (editorState.stage.selectedPluginObject) {
        _.forEach(
          ecEditor.getStage(editorState.stage.currentStage).children,
          function (child) {
            if (child.id === editorState.stage.selectedPluginObject)
              ecEditor
                .getCurrentStage()
                .canvas.setActiveObject(child.editorObj);
          }
        );
      }
    } else {
      ecEditor.dispatchEvent("stage:select", {
        stageId: ecEditor.getAllStages()[0].id,
      });
    }
  },
