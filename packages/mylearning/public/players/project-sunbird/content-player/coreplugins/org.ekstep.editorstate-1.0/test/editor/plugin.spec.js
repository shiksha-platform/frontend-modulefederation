describe("Editor state plugin", function () {
  ...
  it("should get the state from content-service", function () {
    spyOn(
      ecEditor.getService(ServiceConstants.CONTENT_SERVICE),
      "getContentMeta"
    ).and.returnValue({ editorState: "{}" });
    pluginInstance._getStateFromService();
    expect(
      ecEditor.getService(ServiceConstants.CONTENT_SERVICE).getContentMeta
    ).toHaveBeenCalledWith(ecEditor.getContext("contentId"));
  });
  it("should construct the state", function () {
    pluginInstance.extPluginsList = [
      { plugin: "org.ekstep.someplugin", version: "1.0" },
    ];
  ...
  it("should set default stage state when state is not available", function () {
    ...
    expect(ecEditor.dispatchEvent).toHaveBeenCalledWith("stage:select", {
      stageId: 34567,
    });
  });
