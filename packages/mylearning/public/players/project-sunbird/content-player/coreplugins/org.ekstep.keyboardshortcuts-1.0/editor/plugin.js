...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  initialize: function () {
    ...
    var templatePath = ecEditor.resolvePluginResource(
      ...
      "editor/templates/shortcutspopup.html"
    );
    ...
    ecEditor.registerKeyboardCommand("ctrl+m", function (event) {
      event.preventDefault();
      ecEditor.dispatchEvent(instance.events.createStage, {
    ...
    ecEditor.registerKeyboardCommand("mod+d", function (event) {
      event.preventDefault();
      ecEditor.dispatchEvent(instance.events.duplicateStage, {
        stageId: org.ekstep.contenteditor.api.getCurrentStage().id,
      });
      ecEditor.dispatchEvent("object:modified", {
        target: ecEditor.getEditorObject(),
      });
    ...
    org.ekstep.contenteditor.api
      .getCanvas()
      .on("object:moving", instance.moveRichText);
    ...
    ecEditor.registerKeyboardCommand(["home", "command+up"], function (event) {
      event.preventDefault();
      ecEditor.dispatchEvent(instance.events.selectStage, {
        prevStageId: org.ekstep.contenteditor.api.getCurrentStage().id,
        stageId: org.ekstep.contenteditor.api.getAllStages()[0].id,
      });
    });
      ...
      ecEditor.dispatchEvent(instance.events.selectStage, {
        ...
        stageId:
          org.ekstep.contenteditor.stageManager.stages[
            org.ekstep.contenteditor.stageManager.stages.length - 1
          ].id,
      });
    });
    ...
    ecEditor.registerKeyboardCommand("up", function (event) {
      ...
      if (activeGroup) {
        event.preventDefault();
        _.each(
          org.ekstep.contenteditor.api.getCurrentGroup(),
          function (element) {
            var richText = ecEditor.jQuery("#" + element.id);
            if (richText.length != 0) {
              richText[0].style.top =
                parseInt(richText[0].style.top, 10) - 3 + "px";
            }
      ...
      if (activeGroup) {
        ...
        _.each(
          ...
          function (element) {
            var richText = ecEditor.jQuery("#" + element.id);
            if (richText.length != 0) {
              richText[0].style.left =
                parseInt(richText[0].style.left, 10) - 3 + "px";
            }
      ...
      if (activeGroup) {
        ...
        _.each(
          ...
          function (element) {
            ...
            if (richText.length != 0) {
              richText[0].style.top =
                parseInt(richText[0].style.top, 10) - 1 + "px";
            }
      ...
      if (activeGroup) {
        ...
        _.each(
          ...
          function (element) {
            ...
            if (richText.length != 0) {
              richText[0].style.left =
                parseInt(richText[0].style.left, 10) - 1 + "px";
            }
            element.setAttribute("x", element.attributes.x - 1);
          }
        );
