...
(function () {
  $.fn.topicTreePicker = function (options) {
    ...
    nodes = [];
    tabs = {};
    $("#" + options.nodeName).length == 0
      ? ""
      : $("#" + options.nodeName).remove();
    modal = $(
      "<div id=" +
        options.nodeName +
        ' class="ui tree-picker small modal">\n  <div class="header">\n    ' +
        options.name +
    ...
    initialize = function () {
        ...
        if (!nodes.length) {
          if (config.url) {
            ...
            setTimeout(function () {
              ...
              $(".ui.tree-picker.modal .ui.warning.message").removeClass(
                "hidden"
              );
      ...
      actionButtons.pickSearch.on("click", function (e) {
        return $(".topic-search-tab .node:not(.picked) .name", modal).trigger(
          "click"
        );
      });
      actionButtons.unpickSearch.on("click", function (e) {
        return $(".topic-search-tab .node.picked .name", modal).trigger(
          "click"
        );
      });
    ...
    showSearch = function (query) {
      ...
      if (query !== null && query.length >= config.minSearchQueryLength) {
        foundNodes = recursiveNodeSearch(nodes, function (node) {
          return (
            node.name &&
            node.name.toLowerCase().indexOf(query.toLowerCase()) > -1
          );
        });
        ...
        return $(".name", list).each(function () {
          ...
          name = name.replace(
            regex,
            "<strong class='search-query'>$1</strong>"
          );
    ...
    renderTree = function (nodes, css) {
      ...
      for (i = 0, len = nodes.length; i < len; i++) {
        ...
        if (node[config.childrenKey] && node[config.childrenKey].length) {
          $(".content", nodeElement).append(
            renderTree(node[config.childrenKey])
          );
        } else {
      ...
      for (i = 0, len = nodes.length; i < len; i++) {
        ...
        if (node[config.childrenKey] && node[config.childrenKey].length) {
          results = results.concat(
            recursiveNodeSearch(node[config.childrenKey], comparator)
          );
        }
      }
      return results;
