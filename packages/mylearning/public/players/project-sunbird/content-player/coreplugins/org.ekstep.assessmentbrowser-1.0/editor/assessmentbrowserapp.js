    ...
    function ($scope, instance) {
      ...
      if (ecEditor._.isUndefined(instance.data.questionnaire)) {
        ...
        ctrl.activityOptions = {
          ...
          showImmediateFeedback:
            instance.data.questionnaire.showImmediateFeedback,
        ...
        ctrl.activityOptions.range = ecEditor._.times(
          ctrl.activityOptions.total_items
        ).splice(1);
        ctrl.activityOptions.range.push(ctrl.activityOptions.total_items);
        ecEditor
          .jQuery(".displayCount .text")
          .html(ctrl.activityOptions.total_items);
      ...
      ctrl.searchQuestions = function () {
        ...
        if (ctrl.activityOptions.myQuestions) {
          ...
          data.request.filters.createdBy = ecEditor._.isUndefined(ctrl.context)
            ? ""
            : ctrl.context.uid || ctrl.context.user.id;
        } else {
          ctrl.isMyQuestions = false;
        }
              ...
              if (!resp.data.result.count || resp.data.result.count <= 0) {
                ...
                ecEditor._.forEach(resp.data.result.items, function (value) {
                  if (!ecEditor._.isUndefined(value.template_id)) {
                    ...
                    item = value;
                    if (
                      ecEditor._.findIndex(ctrl.cart.items, function (i) {
                ...
                if (
                  ecEditor._.isUndefined(ctrl.cart.items) ||
                  ctrl.cart.items.length <= 0
                ) {
      ...
      ctrl.getFrameworkData = function (callback) {
        var framework =
          ...
          ecEditor
            .getService("content")
            .getContentMeta(ecEditor.getContext("contentId")).framework;
        ecEditor
          .getService("meta")
          .getCategorys(framework, function (err, respCat) {
            if (!err) {
              ecEditor._.forEach(
                respCat.data.result.framework.categories,
                function (category) {
              ...
              ecEditor
                .getService("meta")
                .getDefinitions("AssessmentItem", function (err, resp) {
                  if (!err) {
                    var questionTypes = {};
                    ecEditor._.forEach(
                      resp.data.result.definition_node.properties,
                      function (prop) {
                    ...
                    ecEditor
                      ...
                      .getResourceBundles(
                        ctrl.languagecode,
                        function (err, resourceResp) {
                          if (!err) {
                            ecEditor._.forEach(
                              ctrl.assessment.type,
                              function (data) {
                                if (
                                  resourceResp.data.result.en[data] == undefined
                                ) {
                                  questionTypes[data] = data;
                                } else {
                                  questionTypes[data] =
                                    resourceResp.data.result.en[data];
                                }
                              }
                            );
                    ...
                    ecEditor
                      .jQuery(".ui.dropdown.lableCls")
                      .dropdown({ useLabels: false, forceSelection: false });
                  } else {
                    ctrl.errorMessage = true;
                    $scope.$safeApply();
                    return;
      ...
      ctrl.cart = {
        items: ecEditor._.isUndefined(instance.data.questionnaire)
          ? []
          : instance.data.questionnaire.items[
              instance.data.questionnaire.item_sets[0].id
            ],
        ...
        remove: function (item) {
          ...
          ecEditor
            .jQuery(
              ".displayCount #total_items option[value='number:" +
                parseInt(this.items.length + 1) +
                "']"
            )
            .remove();
      ...
      $scope.$on("ngDialog.opened", function (e, $dialog) {
        ...
        if (itemIframe.src == "") itemIframe.src = instance.previewURL;
        ...
        ecEditor
          ...
          .getItem(item.identifier, function (err, resp) {
            if (!err) {
              ...
              var templateRef = item.template_id
                ? item.template_id
                : item.template;
              if (templateRef) {
                ecEditor
                  ...
                  .getTemplate(templateRef, function (err, response) {
                    if (!err) {
                      ...
                      ctrl.itemPreviewContent =
                        assessmentBrowserUtil.getQuestionPreviwContent(
                          templateJson,
                          item
                        );
                      ctrl.itemPreviewDisplay = !ecEditor._.isUndefined(
                        ctrl.itemPreviewContent.error
                      )
                        ? ctrl.itemPreviewContent.error
                        : "";
