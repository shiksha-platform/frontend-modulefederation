...
angular.module("assetbrowserapp").controller("browsercontroller", [
  "$scope",
  "$injector",
  "instance",
  function ($scope, $injector, instance) {
    ...
    ctrl.previewMessages = {
      ...
      invalidLicense:
        "The video you are trying to upload is not license by CC-BY. Please try another video.",
    ...
    ctrl.assetFileError = "";
    ctrl.createdBy = ecEditor._.isUndefined(ctrl.context)
      ? ""
      : ctrl.context.user.id;
    ctrl.offset = 0;
    ctrl.maxLimit = 200;
    ...
    ctrl.assetMeta = {
      ...
      keywords: [],
      creator: "",
      createdBy: ecEditor._.isUndefined(ctrl.context)
        ? ""
        : ctrl.context.user.id,
    ...
    function uploadIndicator(val) {
      ecEditor
        .jQuery(".uploadingAssetIndicator")
    ...
    ctrl.myAssetTab = function () {
      ...
      ecEditor
        .jQuery("#" + ctrl.myTabScrollElement)
        .unbind("scroll")
        .scroll(ctrl.bindScroll);
      ctrl.showLoadMoreWarningMsg = false;
      var callback,
      ...
      imageTabSelected = true;
      audioTabSelected = false;
      ctrl.selectBtnDisable = ecEditor._.isUndefined(lastSelectedImage)
        ? true
        : false;
      ctrl.buttonToShow = "select";
      ...
      if (instance.mediaType == "video") {
        ...
        callback &&
          ctrl.toggleImageCheck() &&
          ctrl.toggleAudioCheck() &&
          ctrl.toggleVideoCheck();
        ctrl.setSearchFilter(
          ctrl.videoMimeTypes.all,
          new Array("Asset"),
          ctrl.createdBy
        );
    ...
    ctrl.getMediaType = function () {
      if (instance.mediaType === "image") {
        ...
        if (
          ecEditor.jQuery("#audioDropDown").dropdown("get value") == "" ||
          ecEditor.jQuery("#audioDropDown").dropdown("get value") == "all"
        ) {
          return new Array("audio", "voice");
        } else if (
          ecEditor.jQuery("#audioDropDown").dropdown("get value") == "voice"
        ) {
    ...
    ctrl.allAssetTab = function () {
      /**rebind the scoll event to the element**/
      ecEditor
        .jQuery("#" + ctrl.allTabScrollElement)
        .unbind("scroll")
        .scroll(ctrl.bindScroll);
      ctrl.showLoadMoreWarningMsg = false;
      var callback,
    ...
    ctrl.audioTab = function () {
      ...
      ctrl.selectBtnDisable = ecEditor._.isUndefined(lastSelectedAudio)
        ? true
        : false;
      ctrl.buttonToShow = "select";
    };
    ...
    ctrl.search = function () {
      var callback, searchText, searchFilter;
      ...
      if (ctrl.tabSelected == "my" && instance.mediaType != "video") {
        ...
        callback &&
          instance.getAsset(
            searchText,
            ctrl.searchFilter.mimeType,
            ctrl.plugin,
            ctrl.searchFilter.contentType,
            ctrl.searchFilter.createdBy,
            (ctrl.offset = 0),
            callback
          );
        ecEditor
    ...
    ctrl.VideoSource = function (video, index, tab) {
      ecEditor
        .jQuery("#checkBox_" + tab + "_" + video.identifier + " >.radioBox")
        .not(":checked")
        .prop("checked", true);
      var videoEl;
    ...
    ctrl.initPopup = function (item) {
      ...
      $scope.$on("ngDialog:opened", function () {
        ecEditor.jQuery(".assetbrowser .infopopover").popup({
          inline: true,
      ...
      ctrl.generateImpression({
        ...
        subtype: "popup-open",
        pageid: "AssetsBrowser",
        duration: new Date() - ctrl.pluginLoadStartTime,
      });
    };
    ...
    ecEditor.getService("language").getLanguages(function (err, resp) {
      if (
        ...
        resp.data &&
        resp.data.result &&
        ecEditor._.isArray(resp.data.result.languages)
      ) {
        var assetlanguages = {};
    ...
    ctrl.uploadAsset = function (event, fields, assetFile) {
      ...
      if (ctrl.record == true) {
        ...
        ecEditor.jQuery.each(
          ecEditor.jQuery("#assetfile")[0].files,
          function (i, file) {
            data.append("file", file);
            ctrl.assetMeta.mimeType = file.type;

            // @Todo for audio
            ctrl.assetMeta.mediaType =
              instance.mediaType != "audio"
                ? instance.mediaType
                : ctrl.audioType;
          }
        );
      ...
      if (
        !ecEditor._.isUndefined(ctrl.languageText) &&
        ctrl.languageText != null
      ) {
      ...
      if (
        !ecEditor._.isUndefined(ctrl.keywordsText) &&
        ctrl.keywordsText != null
      ) {
      ...
      angular.forEach(
        ...
        function (value, key) {
          if (
            (ecEditor._.isUndefined(value) || value == null || value == "") &&
            key != "body"
          ) {
            delete content[key];
          }
      ...
      if (ctrl.plugin == "video") {
        ...
        var data = {
          request: {
            content: {
              ...
              createdFor: ecEditor._.keys(
                ecEditor.getContext("user").organisations
              ),
              contentType: "Asset",
              osId: "org.ekstep.quiz.app",
        ...
        ecEditor
          .getService("asset")
          .saveAsset(ctrl.assetId, content, function (err, resp) {
            if (resp) {
      ...
      $scope.contentService.uploadContent(
        ...
        function (err, resp) {
          if (err) {
            ...
            ecEditor.dispatchEvent("org.ekstep.toaster:success", {
              message:
                instance.mediaType.charAt(0).toUpperCase() +
                instance.mediaType.slice(1) +
    ...
    ctrl.doUpload = function (event) {
      ctrl.assetMeta.name == "" || ctrl.assetMeta.creator == ""
        ? (ctrl.assetNameError = "error")
        : true;
      ecEditor.jQuery("#assetfile").val() == ""
        ? (ctrl.assetFileError = "error")
        : true;
      ...
      if (
        ...
        if (ctrl.record == true) {
          ...
          var validateFile = instance.fileValidation(
            "assetfile",
            ctrl.allowedFileSize,
            ctrl.allowedMimeTypes
          );
          if (validateFile) {
            ctrl.uploadAsset(event, ctrl.assetMeta, file);
    ...
    ctrl.preFillForm = function (data) {
      ctrl.assetMeta.name = data.name
        ? data.name.replace(/\.[^/.]+$/, "").replace(/[_.^/_]/g, " ")
        : "";
    ...
    function loadMoreAssetCb(err, res) {
      ...
      if (res && res.data.result.content) {
        ...
        ctrl.initPopup(res.data.result.content);
        ecEditor
          .jQuery(ctrl.loadMoreCurrentTab)
          .bind("scroll", ctrl.bindScroll);
      } else {
    ...
    ctrl.searchTextClicked = function () {
      ...
      if (ctrl.myTabScrollElement === "my-image-tab")
        ecEditor.jQuery("#searchMyImageAssets").focus();
      else ecEditor.jQuery("#searchMyAudioAssets").focus();

      /**Check for all asset search tab**/
      if (ctrl.allTabScrollElement === "all-image-tab")
        ecEditor.jQuery("#searchAllImageAssets").focus();
    ...
    ctrl.assetFileValidation = function (file) {
      ...
      var isFileValid = instance.fileValidation(
        "assetfile",
        allowedFileSize,
        allowedMimeTypes
      );
    ...
    ctrl.previewVideo = function (file, mediaType, provider) {
      if (mediaType == "video" && provider == "file") {
        ...
        (mediaType == "video" && provider == "youtube") ||
    ...
    ctrl.getVideoLicense = function (request) {
      ...
      org.ekstep.contenteditor.api
        ...
        .getVideoLicense(requestObj, fields, function (err, res) {
          if (err) {
            ecEditor.dispatchEvent("org.ekstep.toaster:error", {
    ...
    setTimeout(function () {
      ...
      ecEditor
        .jQuery(document)
        .one("change", "#assetName, #assetfile", function () {
          ctrl.uploadBtnDisabled = false;
      ...
      ecEditor.jQuery("#audioDropDown").dropdown({
        onChange: function (value) {
          /**check if searchText is blank**/
          ctrl.offset = 0;
          searchText = ctrl.query === "" ? undefined : ctrl.query;
          var selectedValue =
            value != "all" ? new Array(value) : new Array("audio", "voice");
          instance.getAsset(
            searchText,
            selectedValue,
            "audio",
      ...
      ecEditor.jQuery("#myVideoDropDown").dropdown({
      ...
      ecEditor.jQuery("#allVideoDropDown").dropdown({
        onChange: function (value, text, $selectedItem) {
          /**check if searchText is blank**/
          ctrl.filterType = value;
          ecEditor.jQuery("#allVideoDropDown").find(".text").first().text(text);
