...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  initialize: function () {
    ...
    org.ekstep.contenteditor.api.addEventListener(
      this.manifest.id + ":show",
      this.initPreview,
      this
    );
    var templatePath = org.ekstep.contenteditor.api.resolvePluginResource(
      instance.manifest.id,
      instance.manifest.ver,
      "editor/assetBrowser.html"
    );
    var controllerPath = org.ekstep.contenteditor.api.resolvePluginResource(
      instance.manifest.id,
      instance.manifest.ver,
  ...
  getAsset: function (
        ...
        requestObj = {
          request: {
            filters: {
              ...
              mimeType:
                mediaType == "video"
                  ? new Array("video/x-youtube", "video/mp4", "video/webm")
                  : mediaType,
              contentType: _.includes(contentType, "Asset")
                ? new Array("Asset")
                : new Array("Resource"),
    ...
    org.ekstep.contenteditor.api._.isUndefined(searchText)
      ? null
      : (requestObj.request.query = searchText);

    // Public assets only
    if (org.ekstep.contenteditor.api._.isUndefined(createdBy)) {
      requestObj.request.filters.license =
        "Creative Commons Attribution (CC BY)";
      allowableFilter = org.ekstep.contenteditor.api._.omit(
        this.search_filter,
        ["mediaType", "license", "limit"]
      );
    } else {
      // All assets
      ...
      allowableFilter = org.ekstep.contenteditor.api._.omit(
        this.search_filter,
        ["mediaType", "limit", "createdBy"]
      );
    }

    org.ekstep.contenteditor.api._.merge(
      requestObj.request.filters,
      allowableFilter
    );
    ...
    ecEditor.getService("search").search(requestObj, function (err, res) {
      if (!err && res.data.responseCode == "OK") {
        if (_.includes(contentType, "Resource") && !hasResourceLoaded) {
          ...
          instance
            .searchAsset(resourceObj)
            .then(function (resourceRes) {
              var videoResources = {};
              if (
                _.includes(contentType, "Asset") &&
                !_.isUndefined(res.data.result.content)
              ) {
                Array.prototype.push.apply(
                  res.data.result.content,
                  resourceRes.data.result.content
                );
                res.data.result.content = _.orderBy(
                  res.data.result.content,
                  ["createdOn"],
                  ["desc"]
                );
  ...
  fileValidation: function (fieldId, allowedFileSize, allowedMimeTypes) {
    ...
    if (window.File && window.FileList && window.Blob) {
      ...
      var fsize = org.ekstep.contenteditor.api.jQuery("#" + fieldId)[0].files[0]
        .size;
      var ftype = org.ekstep.contenteditor.api.jQuery("#" + fieldId)[0].files[0]
        .type;
      ...
      if (ftype) {
        if (
          org.ekstep.contenteditor.api.jQuery.inArray(
            ftype,
            allowedMimeTypes
          ) == -1
        ) {
