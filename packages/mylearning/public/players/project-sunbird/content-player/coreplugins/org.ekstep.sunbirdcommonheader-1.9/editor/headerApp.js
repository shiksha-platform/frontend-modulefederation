  ...
  .controller("headerController", [
    ...
    function ($scope, $interval) {
      ...
      $scope.CONSTANTS = {
        ...
        tocUpdateSampleCsvFileLink:
          ecEditor.getConfig("absURL") +
          ecEditor.resolvePluginResource(
            plugin.id,
            plugin.ver,
            "assets/updatesamplecsvfile.csv"
          ),
      };
      ...
      $scope.setEditorDetails = function () {
        ...
        $scope.collaboratorTooltip =
          ecEditor.getContext("uid") === meta.createdBy
            ? "Add Collaborator"
            : "View Collaborator";
        ...
        ecEditor.dispatchEvent("org.ekstep.contenteditor:save", {
          ...
          callback: function (err, res) {
            if (res && res.data && res.data.responseCode == "OK") {
              $scope.generateTelemetry({
                id: "button",
                subtype: "save_successful",
                duration: (Date.now() - $scope.saveInitiated).toString(),
              });
              ...
              if (
                res.responseJSON.responseCode == "CLIENT_ERROR" &&
                !_.isUndefined(res.responseJSON.result.messages)
              ) {
                ecEditor.dispatchEvent("org.ekstep.toaster:error", {
      ...
      $scope.sendForReview = function () {
        ...
        if (meta.status === "Draft") {
          if (
            ...
            if (
              rootNode &&
              rootNode.data.metadata &&
              _.isUndefined(rootNode.data.metadata.dialcodes)
            ) {
      ...
      $scope.validateUnitsDialcodes = function (rootNode) {
        ...
        rootNode.visit(function (iterateNodes) {
          var unitDialCode =
            typeof iterateNodes.data.metadata.dialcodes === "object"
              ? iterateNodes.data.metadata.dialcodes[0]
              : iterateNodes.data.metadata.dialcodes;
          if (
            iterateNodes.data.metadata.dialcodeRequired == "Yes" &&
            !_.includes(
            ...
            org.ekstep.services.collectionService.highlightNode(
              iterateNodes.data.id
            );
          } else if (
            iterateNodes.data.metadata.dialcodeRequired == "Yes" &&
            (_.isUndefined(iterateNodes.data.metadata.dialcodes) ||
              iterateNodes.data.metadata.dialcodes == "")
          ) {
            dialCodeMisssing = true;
            org.ekstep.services.collectionService.highlightNode(
            ...
            iterateNodes.data.metadata.dialcodeRequired === "No" &&
            !_.isUndefined(iterateNodes.data.metadata.dialcodes) &&
            iterateNodes.data.metadata.dialcodes != ""
          ) {
            dialCodeMisssing = true;
        ...
        if (dialCodeMisssing) {
          ecEditor.dispatchEvent("org.ekstep.toaster:error", {
            message:
              "Errors found in linked QR Codes. Please check and correct.",
            position: "topCenter",
            icon: "fa fa-warning",
          });
        } else {
      ...
      $scope.getQRCodeRequestCount = function () {
        ...
        $scope.qrCodeCount.reserve = rootMeta.reservedDialcodes
          ? Object.keys(rootMeta.reservedDialcodes).length
          : 0;
        rootNode.visit(function (node) {
          node.data.metadata.dialcodeRequired == "Yes"
            ? ($scope.qrCodeCount.request += 1)
            : $scope.qrCodeCount.request;
        });
        $scope.$apply();
      };
      /**
      ...
      $scope.showUploadForm = function () {
        ...
        if (!ecEditor.getContext("contentId")) {
          ...
          $scope.generateImpression({
            ...
            subtype: "popup-open",
            pageid: "uploadForm",
            duration: (new Date() - $scope.uploadFormStart).toString(),
          });
        }
        $scope.$safeApply();
      };
      ...
      $scope.openRequestPopup = function () {
        if ($scope.qrCodeCount.request > 0) {
          ...
          $scope.requestNumber =
            $scope.qrCodeCount.reserve > 0 ? $scope.qrCodeCount.reserve : "";
      ...
      $scope.refreshContentLock = function () {
        if ($scope.internetStatusObj.status === true) {
          ...
          ecEditor
            .getService(ServiceConstants.CONTENT_LOCK_SERVICE)
            .refreshLock(
              {
                request: request,
              },
        ...
        if (meta) {
          switch (type) {
            ...
            case "LOCK_NOT_AVAILABLE":
              $scope.contentLockstatusMessage =
                "Someone is currently working on " +
                meta.name +
                ". Try again later.";
              $scope.isClose = true;
              break;
            case "INVALID_LOCK_ID":
              $scope.contentLockstatusMessage =
                "Close and Re-open content to resume editing";
              $scope.isClose = true;
              break;
            case "SESSION_TIMEOUT":
              $scope.contentLockstatusMessage =
                meta.name +
                " locked due to inactivity, click Resume to continue editing. Closing will result in loss of unsaved changes.";
              $scope.isClose = true;
              $scope.isResume = true;
              break;
          }
      ...
      $scope.validateContentLock = function () {
        ...
        if (
          $scope.idleTimer >= $scope.contentLockIdleTimeOut &&
          $scope.contentLockExpired === false
        ) {
      ...
      $scope.setContentLockListener = function (event) {
        ...
        if ($scope.contentLock && $scope.contentLock.lockKey) {
          ...
          $scope.contentLockIdleTimeOut = Math.floor(
            $scope.contentLockExpiresIn / 3
          );
          $scope.contentLockRefershInterval = Math.floor(
            $scope.contentLockIdleTimeOut / 5
          );
          $scope.idleTimer = 0;
          // set lock refresh interval
          $scope.contentLockListener = $interval(
            $scope.validateContentLock,
            $scope.contentLockRefershInterval * 1000
          );
        }
      };
      ...
      ecEditor.addEventListener(
        ...
        $scope.setPreviewStatus,
        $scope
      );
      ecEditor.addEventListener(
        "org.ekstep.contenteditor:preview:close",
        $scope.revertPreviewStatus,
        $scope
      );
      ecEditor.addEventListener(
        "org.ekstep.editor:keepalive",
        $scope.contentDataChanged,
        $scope
      );
