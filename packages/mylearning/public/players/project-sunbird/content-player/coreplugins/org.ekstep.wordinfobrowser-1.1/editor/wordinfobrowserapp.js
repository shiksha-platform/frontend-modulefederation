    ...
    function ($scope, $injector, instance) {
      ...
      ctrl.template = {
        ...
        g: {
          ...
          image: [
            {
              asset: "org.ekstep.wordinfobrowser.popuptint",
              x: "0",
              y: "0",
              w: "100",
              h: "100",
      ...
      ctrl.selectWords = function ($index, word, $event) {
        if (ecEditor._.indexOf(ctrl.slectedWords, word) != -1) {
          ctrl.slectedWords.splice(
            ecEditor._.indexOf(ctrl.slectedWords, word),
            1
          );
      ...
      ctrl.getKeywords = function () {
        ...
        ecEditor
          .getService("language")
          .getKeyWords(requestData, function (err, response) {
            ctrl.loading = false;
            if (err) {
      ...
      ctrl.addToLesson = function () {
        ...
        ecEditor
          .getService("language")
          .getWords(requestData, function (err, response) {
            if (!err) {
              var dictionary = {};
              ecEditor._.forEach(response.data.result.words, function (w) {
                dictionary[w.lemma] = {
                  lemma: w.lemma.toUpperCase(),
                  picture: _.isArray(w.pictures)
                    ? ctrl.generateId(w.pictures[0])
                    : "org.ekstep.wordinfobrowser.image",
                  picturepath: _.isArray(w.pictures)
                    ? w.pictures[0]
                  ...
                  exampleSentences: _.isArray(w.exampleSentences)
                    ? "Example: " + w.exampleSentences[0]
                    : "",
      ...
      ctrl.generateId = function (value) {
        ...
        return (
          tempValue[tempValue.length - 1].split(".").join().replace(",", "") +
          "_Id"
        );
      };
      ...
      if (!ecEditor._.isUndefined(instance.editorObj)) {
        ctrl.selectedSentence = instance.config.text;
        ctrl.prevSlectedWords = ctrl.slectedWords = !ecEditor._.isUndefined(
          instance.config.words
        )
          ? instance.config.words.split(",")
          : [];
        $scope.$safeApply();
      }
    },
