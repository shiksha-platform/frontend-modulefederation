    ...
    var dialCodeController = [
      ...
      function ($scope, $controller, $filter) {
        ...
        $scope.searchDialCode = function (dialcode, callback) {
          ...
          ecEditor
            .getService("dialcode")
            .getAllDialCodes(channel, reqObj, function (dialerr, dialrep) {
              if (!dialerr) {
                if (dialrep.data.result.count === 1)
                  ecEditor._.uniq(
                    org.ekstep.services.collectionService.dialcodeList.push(
                      dialrep.data.result.dialcodes[0].identifier
                    )
                  );
        ...
        $scope.updateDialCode = function (event, data) {
          ...
          if ($scope.dialcodes) {
            ...
            $scope.editFlag =
              $scope.dialcodes && $scope.dialcodes.length == $scope.maxLength
                ? true
                : false;
            if (
              ecEditor._.indexOf(
