    ...
    function ($timeout, $document, $window, $q, tagsInputConfig, tiUtil) {
      function TagList(options, events, onTagAdding, onTagRemoving) {
        ...
        canAddTag = function (tag) {
          ...
          return $q
            .when(valid && onTagAdding({ $tag: tag }))
            .then(tiUtil.promisifyValue);
        };

        canRemoveTag = function (tag) {
          return $q
            .when(onTagRemoving({ $tag: tag }))
            .then(tiUtil.promisifyValue);
        };
        ...
        link: function (scope, element, attrs, ngModelCtrl) {
          ...
          setElementValidity = function () {
            ngModelCtrl.$setValidity(
              "maxTags",
              tagList.items.length <= options.maxTags
            );
            ngModelCtrl.$setValidity(
              "minTags",
              tagList.items.length >= options.minTags
            );
            ngModelCtrl.$setValidity(
              "leftoverText",
              scope.hasFocus || options.allowLeftoverText
                ? true
                : !scope.newTag.text()
            );
          };
          ...
          scope.$watch("tags", function (value) {
            if (value) {
              tagList.items = tiUtil.makeObjectArray(
                value,
                ...
                $timeout(function () {
                  var activeElement = $document.prop("activeElement"),
                    ...
                    lostFocusToChildElement =
                      element[0].contains(activeElement);
      ...
      return {
        ...
        controller: [
          ...
          function ($scope, $element, $attrs) {
            ...
            $scope.suggestionList = new SuggestionList(
              $scope.source,
              $scope.options,
              $scope.events
            );
        ...
        link: function (scope, element, attrs, tagsInputCtrl) {
          ...
          shouldLoadSuggestions = function (value) {
            return (
              (value && value.length >= options.minLength) ||
    ...
    function ($sce, tiUtil) {
      return {
        ...
        link: function (scope, element, attrs, autoCompleteCtrl) {
          ...
          scope.$getDisplayText = function () {
            return tiUtil.safeToString(
              scope.data[
                options.displayProperty || options.tagsInput.displayProperty
              ]
            );
          };
        },
        ...
        return {
          load: function (directive, scope, attrs, options) {
            ...
            angular.forEach(options, function (value, key) {
              ...
              getDefault = function () {
                var globalValue =
                  globalDefaults[directive] && globalDefaults[directive][key];
                return angular.isDefined(globalValue)
                  ? globalValue
                  : localDefault;
              };
              ...
              if (
                interpolationStatus[directive] &&
                interpolationStatus[directive][key]
              ) {
                ...
                updateValue(
                  attrs[key] && $interpolate(attrs[key])(scope.$parent)
                );
              }
            });
          },
  ...
  tagsInput.factory("tiUtil", [
    ...
    function ($timeout, $q) {
      ...
      self.defaultComparer = function (a, b) {
        ...
        return (
          self.safeToString(a).toLowerCase() ===
          self.safeToString(b).toLowerCase()
        );
      };
