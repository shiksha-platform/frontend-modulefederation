"use strict";
angular.module("activityBrowserApp", []).controller("activityBrowserCtrl", [
  "$scope",
  "instance",
    ...
    ctrl.getPluginDetails = function (pluginId) {
      ...
      ecEditor.getService("search").search(request, function (err, res) {
        ...
        if (res && res.responseCode === "OK" && res.result.count > 0) {
          ...
          if (
            res.result.content[0].usesContent &&
            res.result.content[0].usesContent.length
          )
            ctrl.getPluginScreenshots(res.result.content[0].usesContent);
          else {
            ctrl.images.push({
