...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  bringfront: function (event, data) {
    ...
    ecEditor.dispatchEvent("object:modified", {
      id: ecEditor.getEditorObject().id,
    });
  },
  /**
   *
   ...
   */
  copyItem: function () {
    this.clipboard = _.cloneDeep(
      ecEditor.getCurrentObject()
        ? ecEditor.getCurrentObject()
        : ecEditor.getCurrentGroup()
    );
    this.activeGroup =
      ecEditor.getCanvas().getActiveGroup() &&
      ecEditor.getCanvas().getActiveGroup().getObjects();
    if (this.clipboard) {
  ...
  remove: function (object) {
    ecEditor.dispatchEvent("delete:invoked", {
      editorObj: ecEditor.getPluginInstance(object.id).attributes,
    });
  ...
  objectSelected: function (event, data) {
    ecEditor.updateContextMenus([
      { id: "reorder", state: "SHOW", data: {} },
