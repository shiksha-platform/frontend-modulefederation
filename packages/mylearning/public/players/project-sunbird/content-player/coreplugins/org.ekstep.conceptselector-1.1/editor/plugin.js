...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  initData: function (instance, cb) {
    ...
    var data = {
      request: { filters: { objectType: ["Dimension", "Domain"] } },
    };
    ...
    ecEditor.getService("search").search(data, function (err, resp) {
      if (
        ...
        resp.data &&
        resp.data.result &&
        ecEditor._.isArray(resp.data.result.domains)
      ) {
        ecEditor._.forEach(resp.data.result.domains, function (value) {
          ...
          /**Get domain child**/
          ecEditor._.forEach(
            getChild(value.identifier, resp.data.result.dimensions),
            function (value) {
              var dimension = {};
  ...
  getConcept: function (offset, limit, instance, callback) {
    ...
    ecEditor.getService("search").search(data, function (err, resp) {
      if (
        ...
        resp.data &&
        resp.data.result &&
        ecEditor._.isArray(resp.data.result.concepts)
      ) {
        ecEditor._.forEach(resp.data.result.concepts, function (value) {
          instance.concepts.push(value);
        });
        if (
          resp.data.result.count >
          resp.data.result.concepts.length + offset
        ) {
