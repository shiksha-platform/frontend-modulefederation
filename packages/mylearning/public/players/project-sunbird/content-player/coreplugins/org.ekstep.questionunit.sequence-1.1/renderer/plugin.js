...
org.ekstep.questionunitseq.RendererPlugin =
  org.ekstep.contentrenderer.questionUnitPlugin.extend({
    ...
    preQuestionShow: function (event) {
      this._super(event);
      this._question.template = SEQController.getQuestionTemplate(
    ...
    evaluateQuestion: function (event) {
      ...
      if (this._question.config.partial_scoring) {
        questionScore =
          (correctAnswersCount / totalOptions) *
    ...
    getTelemetryParams: function () {
      ...
      var instance = this;
      var params = [],
        questionData = instance._question.data;
      var answer = [];
    ...
    getTelemetryResValues: function () {
      ...
      _.each(options, function (obj, index) {
        rerenderedOptions.push(
          _.findWhere(options, {
            sequenceOrder: instance.seq_rearranged[index],
          })
        );
      });
      _.each(instance.seq_rearranged, function (seqIndex, index) {
        ...
        value[index + 1] = instance.getTelemetryParamsValue(
          options.find(function (seq) {
