...
angular.module("richtexteditorapp", []).controller("richtexteditorcontroller", [
  "$scope",
  "$injector",
  "instance",
  function ($scope, $injector, instance) {
    ...
    $scope.$on("ngDialog.opened", function (e, $dialog) {
      ...
      CKEDITOR.replace("editor1", {
        customConfig: ecEditor.resolvePluginResource(
          manifest.id,
          manifest.ver,
          "editor/libs/config.js"
        ),
        skin: "moono-lisa," + CKEDITOR.basePath + "skins/moono-lisa/",
        contentsCss: ecEditor.resolvePluginResource(
          manifest.id,
          manifest.ver,
          "editor/libs/contents.css"
        ),
      });
      var textObj = ecEditor.getCurrentObject();
      if (
        e.currentScope.ngDialogData &&
        e.currentScope.ngDialogData.textSelected &&
        textObj
      ) {
      ...
      if (data) {
        org.ekstep.contenteditor.api
          ...
          .interact({
            ...
            objectid: ctrl.selectedText
              ? org.ekstep.contenteditor.api.getCurrentObject().id
              : "",
    ...
    ctrl.addText = function () {
      ...
      if (textObj && ctrl.selectedText) {
        ...
        ecEditor
          .jQuery("#richtext-wrapper div#" + textObj.id)
          .html(textObj.config.text);
      } else {
