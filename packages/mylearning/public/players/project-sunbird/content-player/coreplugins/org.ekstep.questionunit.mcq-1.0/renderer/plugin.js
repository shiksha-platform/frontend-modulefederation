...
org.ekstep.questionunitmcq.RendererPlugin =
  org.ekstep.contentrenderer.questionUnitPlugin.extend({
    ...
    preQuestionShow: function (event) {
      this._super(event);
      if (this._question.config.layout == this._constant.gridLayout) {
    ...
    postQuestionShow: function () {
      ...
      if (this._question.state && _.has(this._question.state, "val")) {
        ...
        $("input[name='radio']", $(this._constant.mcqParentDiv))[
          this._selectedIndex
        ].checked = true; // eslint-disable-line no-undef
        if (this._question.config.layout == "Horizontal") {
          $($("input[name='radio']")[this._selectedIndex])
            .parents(".option")
            .addClass("selected");
        }
      } else {
    ...
    evaluateQuestion: function (event) {
      ...
      selectedAns = _.isUndefined(selectedAnsData)
        ? false
        : selectedAnsData.isCorrect;
      ...
      if (!_.isUndefined(MCQController.pluginInstance._selectedIndex))
        telValues["option" + MCQController.pluginInstance._selectedIndex] =
          selectedAnsData.image.length > 0
            ? selectedAnsData.image
            : selectedAnsData.text; // eslint-disable-line no-undef
      result = {
        eval: correctAnswer,
        state: {
        ...
        score: correctAnswer
          ? MCQController.pluginInstance._question.config.max_score
          : 0, // eslint-disable-line no-undef
        values: [telValues],
      };
    ...
    playAudio: function (audio) {
      ...
      if (this._lastAudio && this._lastAudio != audio) {
        // eslint-disable-line no-undef
    ...
    selectedvalue: function (event, index) {
      var state = {},
        value,
        telValues = {};
      $(this._constant.mcqSelectOption).removeClass(
        this._constant.optionSelectionUI
      );
      ...
      telValues["option" + index] =
        value.image.length > 0 ? value.image : value.text;
      ...
      if (this._question.config.layout != "Horizontal") {
        if (
          $(event.target).hasClass(
            this._constant.mcqSelectOption.replace(".", "")
          )
        ) {
          $(event.target).addClass(this._constant.optionSelectionUI);
        } else {
          $(event.target)
            .parents(this._constant.mcqSelectOption)
            .addClass(this._constant.optionSelectionUI);
        }
      } else {
