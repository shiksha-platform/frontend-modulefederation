 ...
 */
angular
  .module("mcqApp", ["org.ekstep.question"])
  .controller("mcqQuestionFormController", [
    "$scope",
    "$rootScope",
    "questionServices",
    "$timeout",
    function ($scope, rootScope, questionServices, $timeout) {
      ...
      $scope.init = function () {
        ...
        ecEditor.addEventListener(
          "org.ekstep.questionunit.mcq:validateform",
        ...
        ecEditor.addEventListener(
          "org.ekstep.questionunit.mcq:editquestion",
      ...
      $scope.formValidation = function () {
        ...
        var valid = false;
        var formValid =
          $scope.mcqForm.$valid && $scope.mcqFormData.options.length > 1;
        $scope.submitted = true;
        if (
          !(
            $scope.mcqFormData.question.text.length ||
            $scope.mcqFormData.question.image.length ||
            $scope.mcqFormData.question.audio.length
          )
        ) {
        ...
        $scope.editMedia = _.isEmpty(temp)
          ? 0
          : _.union($scope.editMedia, temp);
        $scope.mcqFormData.media = _.isEmpty($scope.editMedia[0])
          ? temp
          : $scope.editMedia;
        ...
        var addAllMedia = [
          {
            ...
            src: ecEditor.resolvePluginResource(
              $scope.mcqPluginInstance.id,
              $scope.mcqPluginInstance.ver,
              "renderer/assets/audio.png"
            ),
          ...
          },
          {
            ...
            src: ecEditor.resolvePluginResource(
              $scope.mcqPluginInstance.id,
              $scope.mcqPluginInstance.ver,
              "renderer/assets/default-image.png"
            ),
        ...
        mediaObject.callback = function (data) {
          ...
          if (type == "q") {
            ...
            telemetryObject.target.id =
              "questionunit-mcq-option-add-" + mediaType;
            $scope.mcqFormData.options[index][mediaType] =
              org.ekstep.contenteditor.mediaManager.getMediaOriginURL(
                data.assetMedia.src
              );
            data.assetMedia.type == "audio"
              ? ($scope.mcqFormData.options[index].audioName =
