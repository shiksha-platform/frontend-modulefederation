angular
  .module("org.ekstep.collectioneditor", ["Scope.safeApply", "ui.sortable"])
  .controller("mainController", [
    ...
    function ($scope, $location) {
      ...
      var manifest = org.ekstep.pluginframework.pluginManager.getPluginManifest(
        "org.ekstep.collectioneditor"
      );
      $scope.defaultImage = ecEditor.resolvePluginResource(
        manifest.id,
        manifest.ver,
        "assets/default.png"
      );
      $scope.playImage = ecEditor.resolvePluginResource(
        manifest.id,
        manifest.ver,
        "assets/icn_play.png"
      );
      ...
      $scope.CONSTANTS = {
        tocUploadHeader:
          "Create Table of Contents via CSV Upload or Using Editor",
        tocUploadDescription:
          "Please upload the CSV file in the required format",
        tocUploadBtnUpload: "Upload",
        tocUploadBtnClose: "Use Editor",
        tocUploadSampleCsvFile: "Sample upload csv file",
        tocUploadSampleCsvFileLink:
          ecEditor.getConfig("absURL") +
          ecEditor.resolvePluginResource(
            manifest.id,
            manifest.ver,
            "assets/uploadsamplecsvfile.csv"
          ),
      };
      $scope.mode = ecEditor.getConfig("editorConfig").mode;
      $scope.collectionTreeHeight =
        $scope.mode == "Edit"
          ? "collection-tree-height-with-footer"
          : "collection-tree-height-without-footer";
      ...
      $scope.sortableOptions = {
        stop: function () {
          var collectionData = org.ekstep.collectioneditor._.cloneDeep(
            $scope.contentList
          );
          var activeNode =
            org.ekstep.services.collectionService.getActiveNode();
          var treeData = activeNode.getChildren();
          var fancyTreeChild =
            org.ekstep.collectioneditor._.cloneDeep(collectionData);
      ...
      $scope.resetContentList = function () {
        var rootNode = ecEditor
          ...
          .getFirstChild();
        $scope.isNewCollection =
          rootNode.data.objectType !== "Collection"
            ? rootNode.children
              ? false
              : true
            : false;
        ...
        if (iframe) {
          ...
          if (iframe.contentWindow.EkstepRendererAPI)
            iframe.contentWindow.EkstepRendererAPI.removeHtmlElements();
          iframe.style.display = "none";
          var previewImage =
            document.getElementsByClassName("preview-image")[0];
      ...
      $scope.playContent = function (data) {
        ...
        var initializeRenderer = function () {
          org.ekstep.services.contentService.getContent(
            data.data.metadata.identifier,
            function (err, content) {
              ...
              configuration.data =
                content.mimeType == "application/vnd.ekstep.ecml-archive"
                  ? content.body
                  : {};
        ...
        if (!previewIframe.src) {
          previewIframe.src =
            (ecEditor.getConfig("previewURL") ||
              "/content/preview/preview.html") + "?webview=true";
      ...
      $scope.readExistingQrCode = function () {
        ...
        rootNode.visit(function (node) {
          if (
            !_.isUndefined(node.data.metadata.dialcodes) &&
            node.data.metadata.dialcodes !== ""
          ) {
            org.ekstep.services.collectionService.dialcodeList.push(
              node.data.metadata.dialcodes[0]
            );
          }
        });
      };
      ...
      $scope.deleteNode = function (node, event) {
        if (!node.data.root) {
            ...
            controller: [
              ...
              function ($scope) {
                $scope.confirm = function () {
                  ...
                  node.remove();
                  delete org.ekstep.collectioneditor.cache.nodesModified[
                    node.data.id
                  ];
          ...
          var ngDialogEventListener = $scope.$on(
            "ngDialog.opened",
            function (e, $dialog) {
              ...
              ecEditor
                .jQuery("#deletePopup")
                .offset({ top: height, left: event.pageX - dialogWidth })
                .fadeIn();
        ...
        if (collection) {
          ...
          ecEditor
            ...
            .getCollectionHierarchy(
              { contentId: contentId, mode: mode },
              function (err, res) {
                if (res && res.data && res.data.responseCode === "OK") {
                  $scope.contentList = $scope.generateCollectionContent(
                    res.data.result.content.children
                  );
      ...
      org.ekstep.contenteditor.headerManager.initialize({
        loadNgModules: $scope.loadNgModules,
        scope: $scope,
      });
      ...
      $scope.loadContent = function (event, showToc = true, callback) {
        ...
        ecEditor
          .getService(ServiceConstants.CONTENT_SERVICE)
          .getCollectionHierarchy(
            { contentId: $scope.contentId, mode: mode },
            function (err, res) {
              if (res && res.data && res.data.responseCode === "OK") {
                res.data.result.content.keywords = $scope.parseKeywords(
                  res.data.result.content.keywords
                );
                org.ekstep.services.collectionService.fromCollection(
                  res.data.result.content
                );
                $scope.sidebarPages =
                  org.ekstep.collectioneditor.metaPageManager.getSidebar();
                $scope.breadcrumb =
                  org.ekstep.collectioneditor.metaPageManager.getBreadcrumb();
                $scope.showsuggestedContent =
                  res.data.result.content.contentType === "TextBook"
                    ? true
                    : false;
                if (
                  showToc &&
      ...
      $scope.expandNode = function () {
        ecEditor
          ...
          .expandAll($scope.expandNodeFlag);
        ...
        function () {
          ...
          $scope.loadContent(event, (showToc = true), function (err, res) {
            if (res) {
              var activeNode =
                org.ekstep.services.collectionService.getActiveNode();
              $scope.contentDetails.contentTitle = activeNode.title
                ? activeNode.title
                : "Untitled Content";
              setTimeout(function () {
                ecEditor.dispatchEvent(
                  "org.ekstep.collectioneditor:node:selected",
                  activeNode
                );
                ecEditor.dispatchEvent(
                  "org.ekstep.collectioneditor:node:selected:" +
                    activeNode.data.objectType,
                  activeNode
                );
                ecEditor.dispatchEvent(
              ...
              var editorConfig = _.get(
                ecEditor.getConfig("editorConfig"),
                "mode"
              );
              var mode =
                (editorConfig &&
                  (editorConfig.toLowerCase() === "read"
                    ? "view"
                    : editorConfig)) ||
                "edit";
              ...
              ecEditor
                .jQuery(".pg-loading-html")
                .prepend(
                  '<p class="loading-message">Unable to fetch content! Please try again later</p><button class="ui red button" onclick="ecEditor.dispatchEvent(\'org.ekstep.collectioneditor:content:notfound\');"><i class="window close icon"></i>Close Editor!</button>'
                );
            }
          });
        }
      ...
      $scope.reloadContent = function () {
        ...
        $("#treeWrapper").append(
          '<div id="collection-tree" ng-class="collectionTreeHeight" class="collection-tree-height-with-footer"></div>'
        );
        var mode = "edit";
        // if (ecEditor.getConfig('editorConfig').contentStatus === "draft") mode = "edit";
        ecEditor
      ...
      $scope.setActiveNode = function (node) {
        if (node.key) {
          ...
          if (
            node.data.metadata.mimeType !==
            "application/vnd.ekstep.content-collection"
          )
            ecEditor.dispatchEvent(
              "org.ekstep.collectioneditor:content:update",
              node.data.metadata
            );
          ecEditor.dispatchEvent(
            "org.ekstep.collectioneditor:addToBreadcrumb",
            node.data.metadata
          );
        }
      };
      ...
      $scope.addResource = function () {
        ...
        org.ekstep.collectioneditor
          .jQuery(collectionTree)
          .trigger("nodeCommand", { cmd: "addLesson" });
      };
      ...
      ecEditor.addEventListener(
        "org.ekstep.collectioneditor:reload",
        $scope.reloadContent,
        $scope
      );
