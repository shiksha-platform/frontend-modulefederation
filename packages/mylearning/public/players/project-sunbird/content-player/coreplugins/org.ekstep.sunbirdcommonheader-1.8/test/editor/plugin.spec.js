describe("Sunbird header plugin:", function () {
  ...
  beforeAll(function (done) {
    CollectionEditorTestFramework.init(function () {
      ...
      var checkListTemplate = ecEditor.resolvePluginResource(
        manifest.id,
        manifest.ver,
        "editor/checkList.html"
      );
      ecEditor
  ...
  describe("Content review", function () {
    it("Should invoke downloadToc method to download Toc csv file if response is given", function (done) {
      ...
      var resp = {
        data: {
          ...
          ver: "v1",
          ts: "2018-11-30 10:50:29:057+0000",
          params: {
            resmsgid: null,
            msgid: "8a9a21a5-d18f-4969-9fc4-ab116d46a3e8",
            err: null,
            status: "success",
            errmsg: null,
          },
      ...
      ecEditor.getService("content").downloadTableContent = jasmine
        .createSpy()
    ...
    it("Should invoke downloadToc method to download Toc csv file, if response is null", function (done) {
      ...
      ecEditor.getService("content").downloadTableContent = jasmine
        .createSpy()
        .and.callFake(function (contentId, callBack) {
          callBack(true, resp);
        });
      spyOn($scope, "downloadToc").and.callThrough();
      $scope.downloadToc();
    ...
    it("Should enable downloadtoc button if rootnode has no any child", function (done) {
      ...
      var node = {
        ...
        data: {
          ...
          id: "462a48a1-29bc-4acc-81e1-6b99c1189da0",
          root: false,
          metadata: {
            mimeType: "application/vnd.ekstep.content-collection",
            topicData: "(0) topics selected",
    ...
    it("Should invoke updateContentCreditList method to update content owner list", function (done) {
      var node = {
        data: {
          objectType: "TextBookUnit",
          id: "dbd23d1e-1c9e-4a73-8bc8-0c6c5471e8a9",
          root: false,
          metadata: {
            mimeType: "application/vnd.ekstep.content-collection",
            topicData: "(0) topics selected",
    ...
    it("Should invoke removeOwnershipList method to content owner from list", function (done) {
      ecEditor.jQuery("#collection-tree").fancytree("getRootNode").rootNode =
        ecEditor
          .jQuery("#collection-tree")
          .fancytree("getRootNode").getFirstChild;
      spyOn($scope, "removeOwnershipList").and.callThrough();
    ...
    it("Should invoke previewContent  method to preview creted content", function (done) {
      ...
      ecEditor.dispatchEvent("org.ekstep.contenteditor:preview", {
        mofromBeginningde: "fromBeginning",
      });
      done();
    });
    ...
    describe("Open Request Changes & Publish checklist", function () {
      it("When initPopup is called with api response available & reject popup is opened", function () {
        ...
        spyOn(
          org.ekstep.services.metaService,
          "getFormConfigurations"
        ).and.returnValue(Promise.resolve(checklistJSON));
        $scope.initPopup();
        expect(
          org.ekstep.services.metaService.getFormConfigurations
        ).toHaveBeenCalled();
        expect($scope.checklistMode).toBeDefined();
        expect(checklistConfig.reject.subtitle).toEqual(
          checklistJSON.data.result.form.data.fields[0].title
        );
        expect(checklistConfig.reject.otherReason).toEqual(
          checklistJSON.data.result.form.data.fields[0].otherReason
        );
        expect(checklistConfig.reject.contents).toEqual(
          checklistJSON.data.result.form.data.fields[0].contents
        );
      ...
      it("When initPopup is called with empty api response & reject popup is opened", function () {
        ...
        spyOn(
          org.ekstep.services.metaService,
          "getFormConfigurations"
        ).and.returnValue(Promise.resolve(checklistEmptyResponse));
        spyOn(ecEditor, "dispatchEvent").and.callThrough();
        $scope.initPopup();
        expect(
          org.ekstep.services.metaService.getFormConfigurations
      ...
      it("When initPopup is called with empty api response & publish popup is opened", function () {
        ...
        expect(checklistConfig.publish.subtitle).toEqual(
      ...
      it("When initPopup is called with & user opened to see the changes requested option", function () {
        ...
        expect($scope.checklistItems.subtitle).toEqual(
          checklistConfig.read.subtitle
        );
      });
    });
    describe("update TOC", function () {
      ...
      it("updateToC gets the error response ", function () {
        ...
        var config = {};
        var data = {
          err: "ERR_SCRIPT_NOT_FOUND",
          status: "failed",
          errmsg:
            "Invalid request path: /content/v1/textbook/toc/upload/do_1126448093921853441209",
          responseCode: "SERVER_ERROR",
          result: {},
        };
        var errTitle = "CSV update error";
        ecEditor.dispatchEvent(
          "org.ekstep.uploadfile:show",
          function (event, callback) {
  ...
  describe("refresh content lock", function () {
    ...
    it("Should refresh lock if online", function (done) {
      ...
      spyOn(ecEditor.getService("lock"), "refreshLock").and.returnValue(
        Promise.resolve({
          data: {
            id: "api.v1.refresh",
            ver: "1.0",
            ts: "2018-12-31T04:15:37.055Z",
            params: {
              resmsgid: "b9b01ef0-0cb2-11e9-a2c7-bd997da83dbb",
              msgid: "b9a597a0-0cb2-11e9-95a6-75e890e44e91",
              status: "successful",
              err: null,
              errmsg: null,
            },
            responseCode: "OK",
            result: {
              lockKey: "ca8966a5-d91f-4989-8cb0-5a06dadf8d83",
              expiresAt: "2018-12-31T05:15:36.987Z",
              expiresIn: 60,
            },
          },
        })
