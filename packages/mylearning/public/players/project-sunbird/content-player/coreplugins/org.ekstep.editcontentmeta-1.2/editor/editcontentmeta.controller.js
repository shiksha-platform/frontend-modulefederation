...
angular
  ...
  .controller("editcontentmetaController", [
    ...
    function ($scope, $q, $rootScope, $http, $timeout, data) {
      ...
      var metaInfo = ecEditor
        .getService("content")
        .getContentMeta(ctrl.contentId);
      metaInfo.keywords = $scope.parseKeywords(metaInfo.keywords);
      if (data.contentMeta) {
        data.contentMeta.keywords = $scope.parseKeywords(
          data.contentMeta.keywords
        );
      }
      ...
      $scope.initDropdown = function () {
        $timeout(function () {
          ...
          $("#contentmeta-category-1").dropdown(
            "set selected",
            ctrl.contentMeta[$scope.categoryModelList[1]]
          );
          $("#contentmeta-category-2").dropdown(
            "set selected",
            ctrl.contentMeta[$scope.categoryModelList[2]]
          );
          $("#contentmeta-category-3").dropdown(
            "set selected",
            ctrl.contentMeta[$scope.categoryModelList[3]]
          );
          $("#contentmeta-category-4").dropdown(
            "set selected",
            ctrl.contentMeta[$scope.categoryModelList[4]]
          );
        }, 500);
        if (ctrl.contentMeta[$scope.categoryModelList[2]])
          $scope.categoryValues = ctrl.contentMeta[$scope.categoryModelList[2]]
            .join()
      ...
      $scope.updatedDependentCategory = function (categoryIndex, categoryVal) {
        $scope.categoryValues = $("#contentmeta-category-2")
          .dropdown("get value")
          .replace(/\b\w/g, (l) => l.toUpperCase());
        ctrl.contentMeta[$scope.categoryModelList[2]] = $(
          "#contentmeta-category-2"
        )
          .dropdown("get value")
          .split(",");
        if (categoryIndex == "2") {
          categoryVal = $("#contentmeta-category-2")
            .dropdown("get value")
            .split(",");
          if (categoryVal[0] == "") {
            categoryVal = [];
