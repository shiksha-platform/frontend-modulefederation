...
org.ekstep.contenteditor.basePlugin.extend({
  ...
  initialize: function () {
    ...
    ecEditor.addEventListener(
      ...
      this.openAssessmentBrowser,
      this
    );
    ecEditor.addEventListener(
      this.manifest.id + ":renderQuiz",
      this.renderQuiz,
      this
    );
    ecEditor.addEventListener(
      this.manifest.id + ":showQuizConfig",
      this.showQuizConfig,
      this
    );
    var templatePath = ecEditor.resolvePluginResource(
      ...
      "editor/templates/quizconfig.html"
    );
  ...
  newInstance: function () {
    ...
    instance.data.questionnaire.optionShuffle = _.isUndefined(
      questionnaire.optionShuffle
    )
      ? true
      : questionnaire.optionShuffle;
    ...
    if (_.isUndefined(this.config.media)) {
      ...
      instance
        .getItems(questionnaire.items, "media")
    ...
    var templateArray = [],
      ...
      resCount = 0;
    if (
      (ecEditor._.isUndefined(instance.data.template) ||
        instance.data.template.length == 0 ||
        !instance.hasTemplateMedia) &&
      _.size(templateIds) > 0
    ) {
      for (var index in templateIds) {
        // get Template based on ID and push all templates response to arrray.
        ecEditor
          .getService("assessment")
          .getTemplate(templateIds[index], function (err, res) {
            try {
              if (res) {
                resCount++;
  ...
  showQuizbgImage: function (questionnaire, _parent) {
    var instance = this;
    var quizImage = ecEditor.resolvePluginResource(
      instance.manifest.id,
      instance.manifest.ver,
      "editor/assets/QuizImage.png"
    );
    fabric.Image.fromURL(
      quizImage,
      function (img) {
        var count =
          questionnaire.total_items +
          "/" +
          instance.getItems(questionnaire.items);
        var quizDetails = instance.getPropsForEditor(
          questionnaire.title,
          count,
  ...
  getTemplateData: function (templateArray) {
    ...
    templateArray.forEach(function (element, index) {
      if (!_.isNull(element)) {
        _.isArray(element.template)
          ? $.merge(templates, element.template)
  ...
  showpopupMessage: function (templateIds, errTempurl, _parent) {
    ...
    if (
      _.size(errTemplateids) === _.size(errTempurl) &&
      _.size(errTemplateids) > 0
    ) {
      ecEditor.getService("popup").open({
        ...
        controller: [
          ...
          function ($scope) {
            ...
            $scope.errorMessage =
              "There are few invalid questions in the question set being created. Would you like to remove the questions?";
            $scope.invalidQuestioncount = instance.getInvlidQuestioncount(
              questionnaire,
              errTemplateids
            );
  ...
  clearItem: function (questionnaire, errTemplateids, cb) {
    ...
    questionnaire.items[setId].forEach(function (element, index) {
      errTemplateids.includes(element.template_id)
        ? delete questionnaire.items[setId][index]
        : console.info("Item is not deleted");
    });
  ...
  setQuizdata: function (question, config) {
    ...
    controller.questionnaire["item_sets"] = [
      { count: config.total_items, id: question[0].identifier },
    ];
    controller["questionnaire"] = ecEditor._.assign(
      controller.questionnaire,
      config
    );
    ...
    _assessmentData["config"] = {
      __cdata: JSON.stringify({ type: "items", var: "item" }),
    };
    ...
    if (!_.isUndefined(value)) {
      ...
      switch (key) {
        ...
        case "total_items":
          ...
          this.editorObj._objects[1]._objects[1].setText(
            value + "/" + itemLength + "Questions,"
          );
          break;
        case "max_score":
        ...
        case "browser":
          ...
          ecEditor.dispatchEvent(this.manifest.id + ":renderQuiz", {
            items: value.items,
            config: value.config,
          });
          break;
  ...
  getConfig: function () {
    ...
    config.showImmediateFeedback =
      this.data.questionnaire.showImmediateFeedback;
    ...
    if (ecEditor._.isUndefined(callback)) {
      callback = function (data) {
        ...
        ecEditor.dispatchEvent(
          instance.manifest.id + ":renderQuiz",
          assessmentData
        );
      };
    } else {
  ...
  getSummary: function () {
    ...
    _.forEach(
      instance.data.questionnaire.items[
        instance.data.questionnaire.item_sets[0].id
      ],
      function (data) {
