...
angular
  .module("quizconfigapp", ["ui.sortable"])
  .controller("quizconfigcontroller", [
    "$scope",
    "quizInstance",
    function ($scope, quizInstance) {
      var config = { showStartPage: false, showEndPage: false },
        ctrl = this,
        itemIframe;
      (ctrl.previewURL =
        (ecEditor.getConfig("previewURL") || "/content/preview/preview.html") +
        "?webview=true"),
        (ctrl.activePreviewItem = "");
      var pluginId = "org.ekstep.quiz",
        ver = "1.1";
      ...
      ctrl.setupQuestionsConfig = function () {
        ctrl.activityOptions = {
          title: quizInstance.data.questionnaire.title,
          shuffle: quizInstance.data.questionnaire.shuffle,
          showImmediateFeedback:
            quizInstance.data.questionnaire.showImmediateFeedback,
          concepts: quizInstance.data.questionnaire.concepts,
        };
        ctrl.cart = {
          items:
            quizInstance.data.questionnaire.items[
              quizInstance.data.questionnaire.item_sets[0].id
            ],
        };
      ...
      $scope.sortableOptions = {
        update: function (e, ui) {
          ctrl.generateTelemetry({
            ...
            subtype: "reorder",
            target: "question",
            targetid: ui.item.sortable.model.identifier,
          });
        },
        "ui-floating": true,
      };
      ...
      ctrl.distributedScore = function (options, max_score) {
        var answerCoutn;
        answerCoutn =
          ctrl.currentQuestion.type === "ftb"
            ? _.size(_.map(_.filter(options, "value")))
            : _.size(_.map(_.filter(options, ["answer", true])));
        if (answerCoutn != 0) {
      ...
      ctrl.parseObject = function (question) {
        jQuery.each(question, function (key, value) {
          if (key === "options" || key === "rhs_options" || key === "answer") {
            question[key] = !_.isObject(question[key])
              ? JSON.parse(question[key])
              : question[key];
          }
        });
      ...
      ctrl.removeItem = function (item) {
        ...
        ctrl.activityOptions.total_items =
          quizInstance.data.questionnaire.total_items = ctrl.cart.items.length;
        $scope.$safeApply();
      };
      $scope.$on("ngDialog.opened", function (e, $dialog) {
        itemIframe = org.ekstep.contenteditor.jQuery("#itemIframe")[0];
        if (itemIframe.src == "") itemIframe.src = ctrl.previewURL;
      });
      ctrl.previewItem = function (item) {
        ...
        var templateData = _.filter(quizInstance.data.template, [
          "id",
          item.template,
        ]);
        ctrl.itemPreviewContent = quizBrowserUtil.getQuestionPreviwContent(
          templateData,
          item
        );
      ...
      ctrl.doneConfig = function () {
        ...
        _assessmentData["data"] = {
          __cdata: JSON.stringify(quizInstance.data),
        };
