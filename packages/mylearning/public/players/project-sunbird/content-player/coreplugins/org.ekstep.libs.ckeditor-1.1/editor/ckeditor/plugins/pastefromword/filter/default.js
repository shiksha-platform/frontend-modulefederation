...
(function () {
      ...
      elements: {
        ...
        font: function (element) {
          ...
          if (
            ...
            CKEDITOR.tools.arrayCompare(
              CKEDITOR.tools.objectKeys(element.attributes),
              ["class", "style"]
            )
          ) {
            Style.createStyleStack(element, filter, editor);
        ...
        table: function (element) {
          ...
          if (
            ...
            tools.objectKeys(parent.attributes).length === 1 &&
        ...
        td: function (element) {
          ...
          var background =
            ascendantStyle.background || ascendantStyle.BACKGROUND;
          if (background) {
            Style.setStyle(element, "background", background, true);
          }

          var backgroundColor =
            ascendantStyle["background-color"] ||
            ascendantStyle["BACKGROUND-COLOR"];
          if (backgroundColor) {
  ...
  CKEDITOR.plugins.pastefromword.styles = {
    ...
    createStyleStack: function (element, filter, editor, skipStyles) {
      ...
      for (var style in styles) {
        if (
          style.match(skipStyles || /margin|text\-align|width|border|padding/i)
        ) {
          continue;
        }
