  ...
  CKEDITOR.plugins.add("mathtext", {
    ...
    init: function (editor) {
      ...
      editor.addCommand("mtPrompt", {
        exec: function (editor) {
          var callbackFn = function (mathtextObj) {
            ...
            editor.execCommand("cmdMathText", {
              startupData: {
                math: mathtextObj.latex,
                advance: mathtextObj.advance,
              },
            });
          };
          var latex = null;
          ...
          var isMathtext =
            selectedNode && selectedNode.hasClass("cke_widget_cmdMathText");
          if (isMathtext) {
      ...
      editor.addContentsCss(
        CKEDITOR.basePath + "plugins/mathtext/mathtext.css"
      );
    ...
    afterInit: function (editor) {
      editor.on("selectionChange", function (evt) {
        ...
        var isMathtext =
          selectedNode &&
          (selectedNode.hasClass("cke_widget_cmdMathText") ||
            selectedNode.find(".cke_widget_cmdMathText").$.length > 0);
        var containsMathtext =
          selection.getStartElement().find(".math-text").count() > 0;
  ...
  function registerWidget(editor) {
    editor.widgets.add("cmdMathText", {
      ...
      upcast: function (el, data) {
        if (!(el.name == "span" && el.hasClass("math-text"))) return;
