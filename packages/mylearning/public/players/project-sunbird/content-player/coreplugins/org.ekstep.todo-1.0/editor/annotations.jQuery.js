(function ($) {
  $.fn.annotations = function (options) {
    ...
    ecEditor.jQuery.ajax({
      url:
        ecEditor.getConfig("baseURL") +
        "/index.php?option=com_api&app=jlike&resource=init&format=raw",
      ...
      success: function (result) {
        ...
        if (typeof result.data.usersInfo != "undefined") {
          ...
          // Mention user list
          var userslist =
            type == "annotations" ? JSON.stringify(result.data.userslist) : "";
    ...
    function loadComments(element) {
      ecEditor.jQuery(element).comments({
        ...
        getComments: function (success, error) {
          ecEditor.jQuery.ajax({
            url:
              ecEditor.getConfig("baseURL") +
              "/index.php?option=com_api&app=jlike&resource=annotations&format=raw",
            headers: { "x-auth": "session" },
            type: "GET",
            data: {
            ...
            success: function (result) {
              for (var i = 0; i < result.data.results.length; i++) {
                ...
                //
                if (
                  typeof result.data.results != undefined ||
                  typeof result.data.results.user != undefined
                ) {
                  result.data.results[i].user_name =
                    result.data.results[i].user.name;
                  result.data.results[i].user_id =
                    result.data.results[i].user.id;
                  result.data.results[i].profile_picture_url =
                    result.data.results[i].user.avatar;
                  result.data.results[i].profile_url =
                    result.data.results[i].user.profile_link;
                  result.data.results[i].created = new Date(
                    t[0],
                    t[1] - 1,
                    t[2],
                    t[3],
              ...
              if (result.data.total > limit) {
                ...
                if (
                  ecEditor.jQuery(".data-container .loadMore", element).length
                ) {
                  ...
                  var loadMoreBtn = jQuery(
                    '<button class="loadMore pull-right"><img class="cmt_loader" src="' +
                      ecEditor.getConfig("baseURL") +
                      'components/com_jlike/assets/images/ajax-loading.gif">Load More</button>'
                  );
                  ...
                  ecEditor.jQuery(".loadMore").click(function () {
                    ...
                    ecEditor.jQuery.ajax({
                      ...
                      data: {
                        ...
                        client: element.attr("data-jlike-client"),
                        limitstart: parseInt(
                          element.attr("data-jlike-limitstart"),
                          10
                        ),
                        limit: parseInt(element.attr("data-jlike-limit"), 10),
                      },
                      success: function (result) {
                        ...
                        for (var i = 0; i < result.data.results.length; i++) {
                          var t =
                            result.data.results[i].annotation_date.split(
                              /[- :]/
                            );
                        ...
                        if (result.data.total < limitstart + 10) {
                          ecEditor
                            .jQuery(".data-container .loadMore", element)
                            .hide();
        ...
        postComment: function (commentJSON, success, error, div) {
          ...
          if (
            parseInt(commentJSON.annotation_id) != commentJSON.annotation_id
          ) {
          ...
          ecEditor.jQuery.ajax({
            ...
            success: function (result) {
              ...
              if (
                ...
                result.data.results.profile_url =
                  result.data.results.user.profile_link;
                result.data.results.created = new Date(
                  t[0],
                  t[1] - 1,
                  t[2],
                  t[3],
                ...
                result.data.results.profile_picture_url =
                  result.data.results.user.avatar;
        ...
        deleteComment: function (commentJSON, success, error, div) {
          ...
          ecEditor.jQuery.ajax({
            ...
            url:
              ecEditor.getConfig("baseURL") +
              "/index.php?option=com_api&app=jlike&resource=annotations&format=raw&id=" +
              commentJSON.annotation_id,
            headers: {
              "x-auth": "session",
            },
            data: commentJSON,
        ...
        putComment: function (commentJSON, success, error, div) {
          ...
          ecEditor.jQuery.ajax({
            ...
            url:
              ecEditor.getConfig("baseURL") +
              "/index.php?option=com_api&app=jlike&resource=annotations&format=raw&annotation_id=" +
              commentJSON.annotation_id,
            headers: { "x-auth": "session" },
            data: commentJSON,
            success: function (comment) {
      ...
      if (userslistObj) {
        var userslistObj = JSON.parse(
          element.attr("data-jlike-mentionsUserslist")
        );
