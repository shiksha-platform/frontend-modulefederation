Plugin.extend({
  ...
  initPlugin: function (data) {
    ...
    switch (data.textType) {
      case "readalong":
        ...
        var event = {
          type: "click",
          action: { type: "command", command: "togglePlay", asset: data.id },
        };
      ...
      case "wordinfo":
        ...
        var text = _.isUndefined(this._plginConfig.text)
          ? data.__text
          : this._plginConfig.text;
        text = text.replace(/(?:\r\n|\r|\n)/g, " <br /> ");
        ...
        data.__text = _.map(text.split(" "), function (word) {
          ...
          if (index != -1) {
            return (
              "<a style='font-weight:bold; cursor:pointer; font-size:" +
              fontsize +
              "; color:" +
              instance._plginConfig.wordfontcolor +
              "; background:" +
              instance._plginConfig.wordhighlightcolor +
              "; border-bottom: 1px solid " +
              instance._plginConfig.wordunderlinecolor +
              ";' data-event='" +
        ...
        if (_.isUndefined(instance._stage.events.event)) {
          ...
          _.forEach(wordsArr, function (value, key) {
            event.push({
              type: value + "_click",
              action: [
                { type: "command", command: "show", asset: value + "_info" },
                {
                  type: "command",
          ...
          _.forEach(wordsArr, function (value, key) {
            instance._stage.events.event.push({
              type: value + "_click",
              action: [
                { type: "command", command: "show", asset: value + "_info" },
                {
  ...
  invokeController: function () {
    if (_.has(this._theme._controllerMap, this._plginData.controller.id)) {
      _.extend(
        this._theme._controllerMap[this._plginData.controller.id]["_data"],
        this._plginData.controller.data
      );
    } else {
      var controllerData = {};
  ...
  invokeTemplate: function () {
    this._theme._templateMap[this._plginData.template.id] =
      this._plginData.template;
  },
    ...
    _.forEach(wordsArr, function (value, key) {
      ...
      embedData.event = {
        ...
        action: [
          ...
          { type: "command", command: "hide", asset: value + "_info" },
        ],
      };
      PluginManager.invoke(
        "embed",
