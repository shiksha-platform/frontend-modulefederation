...
org.ekstep.contenteditor.basePlugin.extend({
    ...
    if (instance.comments == undefined) {
      ...
      if (!ecEditor._.get(this.context.contentId)) {
        ...
        var data = {
          request: {
            contextDetails: {
              ...
              contentType: ecEditor
                .getService("content")
                .getContentMeta(
                  org.ekstep.contenteditor.api.getContext("contentId")
                ).mimeType,
              contentVer: !_.isUndefined(pkgVersion)
                ? pkgVersion.toString()
                : "0",
            },
          },
        ...
        ecEditor
          ...
          .getComments(data, function (error, response) {
            try {
              if (
                response &&
  ...
  displayNoComments: function () {
    ...
    ecEditor
      .jQuery("#reviewerComments")
      .html("<div>" + this.NO_COMMENTS + "</div>");
  },
  ...
  filterComments: function () {
    var instance = this;
    var filterComments = _.filter(instance.comments, [
      "stageId",
