...
org.ekstep.questionunitmtf.RendererPlugin =
  org.ekstep.contentrenderer.questionUnitPlugin.extend({
    ...
    preQuestionShow: function (event) {
      ...
      if (this._question.config.layout == this._constant.vertial) {
      ...
      this._question.template =
        "<div class='mtf-layout' id='mtf-container'>" +
        MTFController.getQuestionContent() +
        templateTypeContent() +
        "</div>";
      ...
      this._question.data.option.optionsRHS = _.shuffle(
        this._question.data.option.optionsRHS
      );
      ...
      if (
        this._question.data.question.image &&
        this._question.data.question.image
      ) {
    ...
    evaluateQuestion: function (event) {
      ...
      if (this._question.config.partial_scoring) {
        questionScore =
          (correctAnswersCount / qLhsData.length) *
    ...
    onDropElement: function (elem, target, source, sibling, question) {
      ...
      if (!_.isUndefined($(source).attr("mapIndex"))) {
        ...
        if (!existEle) {
          instance._selectedAnswers[Number($(target).attr("leftindex")) - 1] =
            rhsData;
          var responseData = [
            {
              lhs: question.data.option.optionsLHS[
                $(source).attr("mapIndex") - 1
              ].text,
              rhs: text.trim(),
            },
          ];
        ...
        instance._selectedAnswers = _.filter(
          instance._selectedAnswers,
          function (item) {
