...
/* ** Grid ** */
MCQController.grid =
  MCQController.grid || jQuery.extend({}, MCQController.layout);
...
MCQController.grid.getOptionTemplate = function (option, index) {
  ...
  <div class="mcq-grid-option-outer">\
    ...
    <% if (option.image){ %> \
      ...
      <img onclick=MCQController.zoomImage("<%=option.image%>") class="grid-option-image-zoom-icon"  src="<%= MCQController.pluginInstance.getDefaultAsset("' +
    zoomIcon +
    '") %>"/>\
     </div> </div>\
    <% } %> \
...
MCQController.grid.getOptionsForRow = function (optIndices, i, options) {
  ...
  if (i == 0)
    rowOpts =
      optIndices.length > 4
        ? _.take(optIndices, Math.round((optIndices.length * 1.0) / 2))
        : _.take(optIndices, optIndices.length);
  else {
    var remainingOptions =
      optIndices.length - Math.round((optIndices.length * 1.0) / 2);
...
MCQController.grid.getOptionRows = function (optIndices, options) {
  ...
  while (r < maxRow) {
    rowTemplate +=
      '<div class="mcq-grid-options-row">\
...
MCQController.grid.getOptionsTemplate = function (options) {
  return MCQController.grid.getOptionRows(
    MCQController.grid.config.optIndices,
    options
  );
};
...
/** Horizontal */
MCQController.horizontal =
  MCQController.horizontal || jQuery.extend({}, MCQController.layout);
...
MCQController.horizontal.getTemplateForLayout = function (layout, question) {
  var wrapperStartQuestionComponent =
    '<div class="question-content-container">';
  var wrapperEndQuestionComponent = "</div>";
  var wrapperEnd = "</div>";
  var layoutTemplate = MCQController.horizontal.getOptionLayout(
    layout,
    question
  );
  return (
    '<div class="mcq-content-container plugin-content-container" id="mcq-question-container">' +
    wrapperStartQuestionComponent +
    org.ekstep.mcq.questionComponent.generateQuestionComponent(
      MCQController.pluginInstance._manifest.id
    ) +
...
MCQController.horizontal.getTemplate = function (question) {
  return MCQController.horizontal.getTemplateForLayout(
    question.config.layout.toLowerCase(),
    question
  );
};
...
MCQController.horizontal.getOptionLayout = function (layout, question) {
  MCQController.horizontal.isVerticalLayout =
    "vertical" == layout ? true : false;
  if ("vertical" == layout) {
  ...
  return (
    '<div class="outer-option-container ' +
    layout +
    '">\
                ...
                <div class="option-block org-ekstep-questionunit-mcq-option-element<% if(val.isCorrect) { %> mcq-correct-answer<% } %>" onclick="MCQController.horizontal.onSelectOption(this, <%= key %>);MCQController.horizontal.onOptionSelected(event,<%= key %>)">\
                  ...
                  <%  if(val.image) { %>\
                    <%  if(MCQController.horizontal.isVerticalLayout) { %>\
                        <div class="border-solid position-relative mcq-' +
    layout +
    '">\
                            <img class="mcq-option-img" src="<%= MCQController.pluginInstance.getAssetUrl(val.image) %>"/>\
                            <img onclick=MCQController.zoomImage("<%=val.image%>") class="option-image-zoom-icon" src="<%= MCQController.pluginInstance.getDefaultAsset("' +
    zoomIcon +
    '") %>"/>\
                        </div>\
                  <% } %>\
    ...
    '") %>"/>\
                  ...
                  <% } %>\
                    ...
                    <%  if(val.audio) { %>\
                      <img src="<%= MCQController.pluginInstance.getDefaultAsset("' +
    audioIcon +
    '") %>" class="audio <% if(val.image) { %>with-image<% } %>" onclick=MCQController.imagehorizontal.playAudioHorizontal(<%=key+1%>,"play",{src:\'<%= val.audio %>\'})  />\
                    <% } %>\
...
MCQController.vertical =
  MCQController.vertical ||
  jQuery.extend({}, MCQController.layout, MCQController.horizontal);

/** Vertical2 */
MCQController.vertical2 =
  MCQController.vertical2 || jQuery.extend({}, MCQController.layout);
...
MCQController.vertical2.getTemplate = function (question) {
  ...
  var optionsTemplate = MCQController.vertical2.getOptionsTemplate(
    question.data.options
  );
  return (
    "<div class='mcq-qLeft-content-container plugin-content-container'>" +
    questionTemplate +
    MCQController.backgroundComponent.getBackgroundGraphics(
      question.config.layout.toLowerCase()
    ) +
...
MCQController.vertical2.getQuestionTemplate = function (question) {
  var qTemplate =
    "<div class='mcq-qLeft-question-container'>\
    <div class='mcq-question-image'>\
                ...
                <img onclick=MCQController.zoomImage('<%=question.data.question.image%>') class='zoom-image' src=<%= MCQController.pluginInstance.getDefaultAsset('" +
    zoomIcon +
    "') %>>\
                </div>\
                <%}%>\
...
MCQController.vertical2.getOptionsTemplate = function (options) {
  ...
  return (
    "<div class='mcq-2-options-outer-container-vertical'><div class='mcq-2-options-container-vertical'><div class='mcq-2-options-block-container-vertical'>" +
    opts +
    "</div></div>\
"
  );
...
MCQController.onImageDomLoad = function (container, question) {
  ...
  $(".mcq-option-img").on("load", function () {
    ...
    var size = MCQController.getMaxWidthHeight(
      question.data.options,
      defaultMaxHeight,
      defaultMaxWidth
    );
...
/** Grid2 */
MCQController.grid2 =
  MCQController.grid2 || jQuery.extend({}, MCQController.layout);
...
MCQController.grid2.getTemplate = function (question) {
  var questionTemplate = MCQController.vertical2.getQuestionTemplate(question);
  var optionsTemplate = MCQController.grid2.getOptionsTemplate(
    question.data.options
  );
  return (
    "<div class='mcq-qLeft-content-container plugin-content-container'>" +
...
MCQController.grid2.getOption = function (option, key) {
  var optTemplate =
    " <div class='org-ekstep-questionunit-mcq-option-element mcq2-2-option mcq2-2-option<%=key+1%>' onClick=MCQController.grid2.onOptionSelected(event,<%= key %>)>\
  <%if(option.image){%>\
...
/** imageHorizontal */
MCQController.imagehorizontal =
  MCQController.imagehorizontal || jQuery.extend({}, MCQController.layout);
...
MCQController.imagehorizontal.getTemplate = function (question) {
  ...
  return (
    '<div class="mcq-content-container plugin-content-container mcq-imageHzt" id="mcq-question-container">' +
    wrapperStartQuestionComponent +
    '<div class="imagegrid-mcq-question-head">' +
    org.ekstep.mcq.questionComponent.generateQuestionComponent({
      layout: "imagehorizontal",
    }) +
    "</div>" +
    wrapperEndQuestionComponent +
...
/** imageGrid */
MCQController.imagegrid =
  MCQController.imagegrid || jQuery.extend({}, MCQController.layout);
...
MCQController.imagegrid.getTemplate = function (question) {
  ...
  return (
    '<div class="mcq-content-container mcq-imagegrid-content-container plugin-content-container">' +
    wrapperStartQuestionComponent +
    '<div class="imagegrid-mcq-question-head">' +
    org.ekstep.mcq.questionComponent.generateQuestionComponent({
      layout: "imagegrid",
    ...
    <% _.each(question.data.options,function(val,key){ %>\
        <div class="option-width-50 option-block org-ekstep-questionunit-mcq-option-element<% if(val.isCorrect) { %> mcq-correct-answer<% } %>" onclick="MCQController.onSelectingOption(this, <%= key %>);MCQController.horizontal.onOptionSelected(event,<%= key %>)">\
            <%  if(val.audio) { %>\
              <img  src="<%= MCQController.pluginInstance.getDefaultAsset("' +
    audioIcon +
    '") %>" class="audio-icon" id="mcq-imagegird-audio-play<%=key+1%>" style="display:block"/>\
              <img  src="<%= MCQController.pluginInstance.getDefaultAsset("' +
    audioStop +
    '") %>" class="audio-icon" id="mcq-imagegird-audio-stop<%=key+1%>" style="display:none" />\
            <% } %>\
...
MCQController.imagegrid.getOption = function (option, key) {
  var optTemplate =
    '<div class="outer-option-container-horizontal">\
...
MCQController.backgroundComponent = {
  ...
  getBackgroundGraphics: function (layoutType) {
    org.ekstep.questionunit.backgroundComponent.settings.bgColor =
      org.ekstep.questionunit.backgroundComponent.settings.bgColors[
        _.random(
          0,
          org.ekstep.questionunit.backgroundComponent.settings.bgColors.length -
            1
        )
      ];
