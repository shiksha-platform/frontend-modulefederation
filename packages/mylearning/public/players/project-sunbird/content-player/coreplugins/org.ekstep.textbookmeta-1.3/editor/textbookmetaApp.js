...
var textbookApp = angular.module("textbookmetaApp", [
  "ngTagsInput",
  "Scope.safeApply",
]);
textbookApp.controller("textbookmetaController", [
  "$scope",
  "$timeout",
  "$filter",
  function ($scope, $timeout, $filter) {
    ...
    ecEditor.addEventListener(
      "title:update:textbook",
      $scope.updateTitle,
      $scope
    );
    var categoryMasterList = _.cloneDeep(
      org.ekstep.services.collectionService.categoryList
    );
    ...
    $scope.updatedDependentCategory = function (categoryIndex, categoryVal) {
      $scope.categoryValues = $("#textbookmeta-category-2")
        .dropdown("get value")
        .replace(/\b\w/g, (l) => l.toUpperCase());
      $scope.textbook[$scope.categoryModelList[2]] = $(
        "#textbookmeta-category-2"
      )
        .dropdown("get value")
        .split(",");
      if (categoryIndex == "2") {
        categoryVal = $("#textbookmeta-category-2")
          .dropdown("get value")
          .split(",");
        if (categoryVal[0] == "") {
          categoryVal = [];
    ...
    $scope.initDropdown = function () {
      $timeout(function () {
        $(".textbookmeta-category-1").dropdown(
          "set selected",
          $scope.textbook[$scope.categoryModelList[1]]
        );
        $(".textbookmeta-category-2").dropdown(
          "set selected",
          $scope.textbook[$scope.categoryModelList[2]]
        );
        $(".textbookmeta-category-3").dropdown(
          "set selected",
          $scope.textbook[$scope.categoryModelList[3]]
        );
        $(".textbookmeta-category-4").dropdown(
          "set selected",
          $scope.textbook[$scope.categoryModelList[4]]
        );
        $("#textbookmeta-year").dropdown("set selected", $scope.textbook.year);
        $("#textbookmeta-resource").dropdown(
          "set selected",
          $scope.textbook.resource
        );
        if ($scope.textbook[$scope.categoryModelList[2]])
          $scope.categoryValues = $scope.textbook[$scope.categoryModelList[2]]
            .join()
            .replace(/\b\w/g, (l) => l.toUpperCase());
      });
    };
    ...
    $scope.updateNode = function () {
      if (!_.isEmpty($scope.nodeId) && !_.isUndefined($scope.nodeId)) {
        ...
        if (
          !_.isEmpty($scope.textbook.gradeLevel) &&
          _.isString($scope.textbook.gradeLevel)
        ) {
          $scope.textbook.gradeLevel = [$scope.textbook.gradeLevel];
        }

        if (
          !_.isEmpty($scope.textbook.language) &&
          _.isString($scope.textbook.language)
        ) {
          $scope.textbook.language = [$scope.textbook.language];
        }
        if (
          !_.isEmpty($scope.textbook.audience) &&
          _.isString($scope.textbook.audience)
        ) {
          $scope.textbook.audience = [$scope.textbook.audience];
        }
        $scope.textbook.contentType = $scope.nodeType;
        var mergedData = _.pickBy(
          _.assign(
            org.ekstep.collectioneditor.cache.nodesModified[$scope.nodeId]
              .metadata,
            $scope.getUpdatedMetadata($scope.metadataCloneObj, $scope.textbook)
          ),
          _.identity
        );
    ...
    $scope.onNodeSelect = function (evant, data) {
      ...
      $scope.editMode = true;
      $scope.newNode = false;
      $scope.editable = org.ekstep.collectioneditor.api
        .getService("collection")
        .getObjectType(data.data.objectType).editable;
      $scope.defaultImage = ecEditor.resolvePluginResource(
        "org.ekstep.textbookmeta",
        "1.2",
        "assets/default.png"
      );
        ...
        if (!_.isUndefined(activeNode.data.metadata.concepts)) {
          ...
          if ($scope.textbook.concepts.length > 0) {
            $scope.textbook.conceptData =
              "(" + $scope.textbook.concepts.length + ") concepts selected";
    ...
    ecEditor.addEventListener(
      "org.ekstep.collectioneditor:node:selected:TextBook",
      $scope.onNodeSelect
    );
    ...
    $scope.init = function () {
      ...
      $scope.$watch(
        ...
        function () {
          if ($scope.textbook) {
            if (/^[a-z\d\-_\s]+$/i.test($scope.textbook.name) == false)
              $scope.textbook.name =
    ...
    $scope.changeTitle = function () {
      org.ekstep.collectioneditor.api
        .getService("collection")
        .setNodeTitle($scope.textbook.name);
    };
