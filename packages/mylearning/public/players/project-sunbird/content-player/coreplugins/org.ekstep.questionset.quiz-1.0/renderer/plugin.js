Plugin.extend({
  ...
  removeHtmlElements: function () {
    ...
    jQuery(chilElemtns).each(function () {
      ...
      if (
        this.id !== "overlay" &&
        this.id !== "qs-custom-prev" &&
        this.id !== "gameCanvas" &&
        this.id !== "qs-feedback-model-popup" &&
        this.id !== "qs-custom-next"
      ) {
        jQuery(this).remove();
      }
    });
  ...
  registerEvents: function () {
    ...
    EkstepRendererAPI.addEventListener(
      "org.ekstep.questionset.quiz:hide",
      function () {
        var qsQuizIndex = _.findIndex(instance._stage._childIds, function (ci) {
          ...
          if (childInstance._data && childInstance._data.pluginId) {
            return (
              childInstance._data.pluginId == "org.ekstep.questionset.quiz"
            );
          } else {
            return false;
          }
    ...
    EkstepRendererAPI.addEventListener(
      "org.ekstep.questionset.quiz:evaluate",
      function (event) {
  ...
  initquestionnaire: function () {
    ...
    if (stageController) {
      ...
      if (typeof this._theme.getParam === "function") {
        ...
        if (questionState) {
          this._stage._stageController._model = JSON.parse(
            JSON.stringify(questionState.model)
          );
        }
      }
    }
  ...
  evaluate: function (callback) {
    ...
    if (GlobalContext.registerEval[item.type.toLowerCase()]) {
      ...
      var customEvalInstance =
        GlobalContext.registerEval[item.type.toLowerCase()]; // eslint-disable-line no-undef
      ...
      if (item.type.toLowerCase() == "ftb") {
        ...
        item.type.toLowerCase() == "mcq" ||
        item.type.toLowerCase() == "mmcq"
      ) {
