...
org.ekstep.questionunitFTB.RendererPlugin =
  org.ekstep.contentrenderer.questionUnitPlugin.extend({
    ...
    postQuestionShow: function (event) {
      ...
      $(FTBController.constant.qsFtbElement).on(
        "click",
        ".ans-field",
        FTBController.invokeKeyboard
    ...
    evaluateQuestion: function (event) {
      ...
      if (this._question.config.evalUnordered) {
        correctAnswer = _.isEqual(
          answerArray,
          _.intersection(answerArray, this._question.data.answer)
        );
        correctAnswersCount = _.intersection(
          answerArray,
        ...
        correctAnswersCount = _.reduce(
          _.map(this._question.data.answer, function (ans, index) {
            return ans.toLowerCase().trim() ==
              answerArray[index].toLowerCase().trim()
              ? 1
              : 0;
          }),
          function (score, s) {
      ...
      if (this._question.config.partial_scoring) {
        questionScore =
          (correctAnswersCount / this._question.data.answer.length) *
          this._question.config.max_score;
      } else {
    ...
    getTelemetryParams: function () {
      ...
      var instance = this;
      var params = [],
        qData = instance._question.data.question;
      var textBoxCollection = $(FTBController.constant.qsFtbQuestion).find(
        "input[type=text]"
      ); // eslint-disable-line no-undef
      _.each(textBoxCollection, function (element, index) {
        var temp = {};
        temp[index + 1] = JSON.stringify({
          text: instance._question.data.answer[index],
        });
        params.push(temp);
      });
      var evalOrder = instance._question.config.evalUnordered
        ? "unorder"
        : "order";
