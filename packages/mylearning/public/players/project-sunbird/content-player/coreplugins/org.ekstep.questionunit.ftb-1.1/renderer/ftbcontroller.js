...
FTBController.setStateInput = function () {
  var textBoxCollection = $(FTBController.constant.qsFtbQuestion).find(
    "input[type=text]"
  );
 ...
 * @memberof org.ekstep.questionunit.ftb
 */
FTBController.invokeKeyboard = function (event) {
  ...
  if (
    !(
      isbrowserpreview &&
      (_.isUndefined(FTBController.question.data.question.keyboardConfig) ||
        FTBController.question.data.question.keyboardConfig.keyboardType ==
          "Device")
    )
  ) {
  ...
  EkstepRendererAPI.dispatchEvent(
    FTBController.constant.keyboardPlugin + ":invoke",
    keyboardConfig,
    FTBController.keyboardCallback
  );
};
 ...
 * @memberof org.ekstep.questionunit.ftb
 */
FTBController.keyboardCallback = function (ans) {
  // eslint-disable-line no-unused-vars
  //TODO: check
...
(FTBController.showImageModel = function () {
  ...
  var modelTemplate =
    "<div class='popup image-model-popup' id='image-model-popup' onclick='FTBController.hideImageModel()'><div class='popup-overlay' onclick='FTBController.hideImageModel()'></div> \
  ...
  (FTBController.generateHTML = function (quesData) {
    ...
    quesData.question.text = quesData.question.text.replace(
      /\[\[.*?\]\]/g,
      function (a, b) {
        // eslint-disable-line no-unused-vars
        index = index + 1;
...
FTBController.postQuestionShow = function () {
  ...
  _.each($(".ans-field"), function (element, index) {
    var width =
      ($("#" + element.id + "-temp").width() *
        FTBController.constant.fieldWidthScale) /
      maxWidth; // add 20% more than the answer
    width =
      width <= FTBController.constant.fieldMinWidth
        ? FTBController.constant.fieldMinWidth
        : width;
