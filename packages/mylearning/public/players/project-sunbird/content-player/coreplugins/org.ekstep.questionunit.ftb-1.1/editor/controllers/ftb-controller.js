 ...
 */
angular.module("orgEkstepQuestion").controller("ftbQuestionFormController", [
  "$scope",
  "$rootScope",
  "questionServices",
  function ($scope, $rootScope, questionServices) {
    ...
    questionInput = CKEDITOR.replace("ftbQuestion", {
    ...
    $scope.init = function () {
      ...
      $scope.ftbPluginInstance =
        org.ekstep.pluginframework.pluginManager.getPluginManifest(
          "org.ekstep.questionunit.ftb"
        );
      ecEditor.addEventListener(
        "org.ekstep.questionunit.ftb:validateform",
      ...
      ecEditor.addEventListener(
        "org.ekstep.questionunit.ftb:editquestion",
      ...
      addAllMedia = [
        {
          ...
          src: ecEditor.resolvePluginResource(
            $scope.keyboardPluginInstance.id,
            $scope.keyboardPluginInstance.ver,
            "renderer/assets/eras_icon.png"
          ),
        ...
        },
        {
          ...
          src: ecEditor.resolvePluginResource(
            $scope.keyboardPluginInstance.id,
            $scope.keyboardPluginInstance.ver,
            "renderer/assets/language_icon.png"
          ),
    ...
    $scope.createAnswerArray = function () {
      var regexForAns = /(?:^|)\[\[(.*?)(?:\]\]|$)/g;
      $scope.ftbFormData.answer = $scope
        .splitAnswer($scope.ftbFormData.question.text, regexForAns, 1)
        .map(function (a) {
    ...
    $scope.formValidation = function () {
      var ftbFormQuestionText,
        formValid,
        formConfig = {};
      $scope.submitted = true;
      ftbFormQuestionText = $scope.ftbFormData.question.text;
      formValid =
        ftbFormQuestionText.length > 0 &&
        /\[\[.*?\]\]/g.test(ftbFormQuestionText);

      $scope.ftbFormData.media = [];
      $scope.addAllMedia();
      _.isEmpty($scope.ftbFormData.question.image)
        ? 0
        : $scope.ftbFormData.media.push($scope.questionMedia.image);
      _.isEmpty($scope.ftbFormData.question.audio)
        ? 0
        : $scope.ftbFormData.media.push($scope.questionMedia.audio);
    ...
    $scope.addMedia = function (type, index, mediaType) {
      ...
      mediaObject.callback = function (data) {
        var telemetryObject = {
          ...
          id: "button",
          target: {
            id: "questionunit-ftb-add-" + mediaType,
            ver: "",
            type: "button",
          },
        };
    ...
    $scope.deleteMedia = function (type, index, mediaType) {
      var telemetryObject = {
        ...
        id: "button",
        target: {
          id: "questionunit-ftb-delete-" + mediaType,
          ver: "",
          type: "button",
        },
      };
