 ...
 */
angular
  .module("mtfApp", ["org.ekstep.question"])
  .controller("mtfQuestionFormController", [
    "$scope",
    "$rootScope",
    "questionServices",
    function ($scope, $rootScope, questionServices) {
      ...
      $scope.init = function () {
        ...
        ecEditor.addEventListener(
          "org.ekstep.questionunit.mtf:editquestion",
      ...
      $scope.formValidation = function () {
        ...
        $scope.optionsMedia["image"] = $scope.rhsMedia["image"].concat(
          $scope.lhsMedia["image"]
        );
        $scope.optionsMedia["audio"] = $scope.rhsMedia["audio"].concat(
          $scope.lhsMedia["audio"]
        );
        //check form valid and lhs should be more than 3
        formValid =
          $scope.mtfForm.$valid &&
          $scope.mtfFormData.option.optionsLHS.length > 2;
        $scope.mtfFormData.question.text = _.isUndefined(
          $scope.mtfFormData.question.text
        )
          ? ""
          : $scope.mtfFormData.question.text;
        if (
          !(
            $scope.mtfFormData.question.text.length ||
            $scope.mtfFormData.question.image.length ||
            $scope.mtfFormData.question.audio.length
          )
        ) {
        ...
        _.isEmpty($scope.questionMedia.image)
          ? 0
          : tempArray.push($scope.questionMedia.image);
        _.isEmpty($scope.questionMedia.audio)
          ? 0
          : tempArray.push($scope.questionMedia.audio);
      ...
      $scope.addMedia = function (type, index, mediaType) {
        ...
        mediaObject.callback = function (data) {
          var telemetryObject = {
            type: "TOUCH",
            id: "button",
            target: { id: "", ver: "", type: "button" },
          };
          var media = {
            id: Math.floor(Math.random() * 1000000000), // Unique identifier
            src: org.ekstep.contenteditor.mediaManager.getMediaOriginURL(
              data.assetMedia.src
            ), // Media URL
          ...
          if (type == "q") {
            telemetryObject.target.id =
              "questionunit-mtf-add-" + data.assetMedia.type;
            $scope.mtfFormData.question[data.assetMedia.type] =
              org.ekstep.contenteditor.mediaManager.getMediaOriginURL(
                data.assetMedia.src
              );
            $scope.mtfFormData.question.audioName =
              data.assetMedia.type == "audio" ? data.assetMedia.name : "";
            $scope.questionMedia[data.assetMedia.type] = media;
          } else if (type == "LHS") {
            telemetryObject.target.id =
              "questionunit-mtf-lhs-add-" + data.assetMedia.type;
            $scope.mtfFormData.option.optionsLHS[index][data.assetMedia.type] =
              org.ekstep.contenteditor.mediaManager.getMediaOriginURL(
                data.assetMedia.src
              );
            $scope.mtfFormData.option.optionsLHS[index].audioName =
              data.assetMedia.type == "audio" ? data.assetMedia.name : "";
            $scope.lhsMedia[data.assetMedia.type][index] = media;
          } else if (type == "RHS") {
            telemetryObject.target.id =
              "questionunit-mtf-rhs-add-" + data.assetMedia.type;
            $scope.mtfFormData.option.optionsRHS[index][data.assetMedia.type] =
              org.ekstep.contenteditor.mediaManager.getMediaOriginURL(
                data.assetMedia.src
              );
            $scope.mtfFormData.option.optionsLHS[index].audioName =
      ...
      $scope.setEditorInputOptions = function (index) {
        ...
        CKEDITOR.instances[optionelement.id].setData(
          $scope.mtfFormData.option[optionSide][parseInt(index / 2)].text
        );
        ...
        if (optionSide == "optionsRHS") {
          optionInput.on("focus", function () {
            ...
            var inputWidth = parseInt(
              $($(".mtfoption-text-ck")[index]).css("width")
            );
        ...
        optionInput.on("change", function () {
          ...
          var optionSide =
            this.name.indexOf("LHS") >= 0 ? "optionsLHS" : "optionsRHS";
          var idx = this.name.split("_")[2];
          $scope.mtfFormData.option[optionSide][idx].text =
            CKEDITOR.instances[this.name].getData();
          $scope.$safeApply();
        });
      ...
      $scope.BindCkeditor = function () {
        angular.element(document).ready(function () {
          ...
          for (
            var index = 0;
            index < $(".mtfoption-text-ck-lhs").length;
            index++
          ) {
