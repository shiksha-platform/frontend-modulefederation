...
org.ekstep.questionunitReorder.RendererPlugin =
  org.ekstep.contentrenderer.questionUnitPlugin.extend({
    ...
    preQuestionShow: function (event) {
      ...
      if (this._question.state) {
        ...
        this._question.data.sentence.tabs = _.shuffle(
          this._question.data.sentence.tabs
        );
      }
    },
    /**
    ...
    onWordSelect: function (id) {
      ...
      $("#reorder-tarea").text(
        _.map(this._userWords, function (w) {
          return w.text;
        }).join(" ")
      );
    },
    ...
    evaluateQuestion: function (event) {
      ...
      if (
        ...
        this._question.data.sentence.text.trim().replace(/\s/g, "")
      ) {
    ...
    getTelemetryParams: function () {
      ...
      var answer = [];
      questionData.sentence.tabs.forEach(function (tab, key) {
        // eslint-disable-line no-undef
        var temp = {};
