...
QSTelemetryLogger.logInteract = function (data) {
  TelemetryService.interact(data.type, data.id, data.type, {
...
QSTelemetryLogger.logResponse = function (data) {
  var edata = {
    target: {
      ...
      type: this._plugin._manifest.type
        ? this._plugin._manifest.type
        : "plugin",
    },
    optionTag: data.type,
    res: data.values,
  ...
  if (this._qData.questionnaire) {
    for (var quesIdentifier in this._qData.questionnaire.items) {
      if (this._qData.questionnaire.items.hasOwnProperty(quesIdentifier)) {
        var maxscore = qsetConfig.shuffle_questions
          ? 1
          : this._qData.questionnaire.items[quesIdentifier][0].max_score;
        this._assessStart = TelemetryService.assess(
          this._question.id,
          this._qData.questionnaire.items[quesIdentifier][0].language,
          this._qData.questionnaire.items[quesIdentifier][0].qlevel,
          { maxscore: maxscore }
        ).start();
      }
    }
    ...
    this._assessStart = TelemetryService.assess(
      this._question.id,
      this._qConfig.metadata.medium,
      this._qConfig.metadata.qlevel,
      { maxscore: maxscore }
    ).start();
  }
};
