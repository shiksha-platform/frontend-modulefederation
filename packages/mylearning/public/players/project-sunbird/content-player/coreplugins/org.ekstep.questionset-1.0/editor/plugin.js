...
org.ekstep.questionset.EditorPlugin =
  org.ekstep.contenteditor.basePlugin.extend({
    ...
    initialize: function () {
      ...
      ecEditor.loadAndInitPlugin(
        instance._dependencyPlugin,
        "1.0",
        publishedDate
      );
      ...
      ecEditor.addEventListener(
        this.manifest.id + ":getPlugins",
        this.getplugins,
        this
      );

      ecEditor.addEventListener(
        instance.manifest.id + ":showPopup",
        instance.openQuestionBank,
        instance
      );
      ecEditor.addEventListener(
        instance.manifest.id + ":addQS",
        instance.addQS,
        instance
      );
      var pluginsData = [
        {
          ...
          appIcon: ecEditor.resolvePluginResource(
            this.manifest.id,
            this.manifest.ver,
            "editor/assets/mtf_app_icon.jpg"
          ),
          semanticVersion: "1.2",
          contentType: "Plugin",
          objectType: "Content",
        },
        {
          ...
          appIcon: ecEditor.resolvePluginResource(
            this.manifest.id,
            this.manifest.ver,
            "editor/assets/mcq_app_icon.jpg"
          ),
          semanticVersion: "1.3",
          contentType: "Plugin",
          objectType: "Content",
        },
        {
          ...
          appIcon: ecEditor.resolvePluginResource(
            this.manifest.id,
            this.manifest.ver,
            "editor/assets/ftb_app_icon.jpg"
          ),
          semanticVersion: "1.1",
          contentType: "Plugin",
          objectType: "Content",
        },
        {
          ...
          appIcon: ecEditor.resolvePluginResource(
            this.manifest.id,
            this.manifest.ver,
            "editor/assets/reorder_app_icon.jpg"
          ),
          semanticVersion: "1.1",
          contentType: "Plugin",
          objectType: "Content",
        },
        {
          ...
          appIcon: ecEditor.resolvePluginResource(
            this.manifest.id,
            this.manifest.ver,
            "editor/assets/sequence_app_icon.jpg"
          ),
          semanticVersion: "1.1",
          contentType: "Plugin",
          objectType: "Content",
    ...
    newInstance: function () {
      ...
      // Add stage object
      var stageImage = ecEditor.resolvePluginResource(
        this.manifest.id,
        this.manifest.ver,
    ...
    toECML: function () {
      ...
      if (_.isArray(instance.data)) {
        instance.data.forEach(function (question) {
          ...
          if (question.version == 1) {
            questionECML = {
              id: _.isUndefined(question.identifier)
                ? UUID()
                : question.identifier,
              type: question.type,
              pluginId: instance._constants.v1PluginId,
              pluginVer:
                question.version === 1 ? "1.0" : question.version.toString(),
            ...
            ecEditor._.forEach(question.media, function (asset) {
              if (!ecEditor._.isEmpty(asset)) instance.addMedia(asset);
            });
      ...
      if (!_.isUndefined(value)) {
        ...
        switch (key) {
          ...
          case "shuffle_questions":
            ...
            if (value) {
              var maxscore = this.config.shuffle_questions
                ? this.data.length
                : this.config.max_score;
              this.editorObj._objects[1]._objects[2].setText(
                maxscore + " Marks"
              );
        ...
        instance._questions.forEach(function (question, key) {
          if (key < totalQuestionsToRender) {
            if (question.body != undefined) {
              var questionCount =
                JSON.parse(question.body).data.config.questionCount == undefined
                  ? 1
                  : JSON.parse(question.body).data.config.questionCount;
              var scoreCount =
                JSON.parse(question.body).data.config.max_score == undefined
                  ? 1
                  : JSON.parse(question.body).data.config.max_score;
              summary.totalQuestions =
                summary.totalQuestions + parseInt(questionCount);
    ...
    loadQSPlugins: function () {
      ...
      if (_.isFunction(ecEditor.getService("search").pluginsSearch)) {
        var url = ecEditor.getConfig("pluginsRepoUrl")
          ? ecEditor.getConfig("pluginsRepoUrl")
          : undefined;
        ecEditor
          .getService("search")
          .pluginsSearch(url, data, function (err, resp) {
            if (!err) {
    ...
    pluginsRespHandler: function (pluginsData) {
      ...
      org.ekstep.pluginframework.pluginManager.loadAllPlugins(
