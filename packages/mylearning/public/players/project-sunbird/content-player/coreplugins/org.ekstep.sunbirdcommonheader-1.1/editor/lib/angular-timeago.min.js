...
angular.module("yaru22.angular-timeago", []),
  ...
  angular.module("yaru22.angular-timeago").config([
    ...
    function (a) {
      ...
      a.strings.cs_CZ = {
        ...
        suffixAgo: null,
        suffixFromNow: null,
        seconds: b("méně než minutou", "méne než minutu", "méne než minutu"),
        minute: b("minutou", "minutu", "minutu"),
        minutes: b("%d minutami", "%d minuty", "%d minút"),
        hour: b("hodinou", "hodinu", "hodinu"),
        hours: b("%d hodinama", "%d hodiny", "%d hodin"),
        day: b("dnem", "den", "den"),
        days: b("%d dny", "%d dny", "%d dnů"),
        month: b("měsícem", "měsíc", "měsíc"),
        months: b("%d měsíci", "%d měsíce", "%d měsíců"),
        year: b("rokem", "rok", "rok"),
        years: b("%d lety", "%d roky", "%d let"),
        numbers: [],
      };
    },
  ]),
  ...
  angular.module("yaru22.angular-timeago").directive("timeAgo", [
    ...
    function (a, b) {
      return {
        scope: { fromTime: "@", format: "@" },
        restrict: "EA",
        link: function (c, d) {
          var e;
          c.$watch("fromTime", function () {
            e = a.parse(c.fromTime);
          }),
            c.$watch(
              function () {
                return b() - e;
              ...
              function (b) {
                angular.element(d).text(a.inWords(b, e, c.format));
              }
            );
        },
      };
  ...
  angular.module("yaru22.angular-timeago").factory("timeAgo", [
    ...
    function (a, b) {
      var c = {};
      return (
        (c.inWords = function (c, d, e, f) {
          function g(a, b) {
            b = Math.round(b);
            var d = angular.isFunction(a) ? a(b, c) : a,
              e = (k.numbers && k.numbers[b]) || b;
            return d.replace(/%d/i, e);
          }
          var h = parseInt(b.fullDateAfterSeconds, 10);
          if (!isNaN(h)) {
            var i = 1e3 * h;
            if ((c >= 0 && i <= c) || (c < 0 && i >= c))
              return e ? a("date")(d, e, f) : d;
          }
          var j,
            k,
            l = b.overrideLang,
          ...
          b.allowFuture &&
            c < 0 &&
            ((o = k.prefixFromNow), (p = k.suffixFromNow));
          var q = Math.abs(c) / 1e3,
            r = q / 60,
            s = r / 60,
            t = s / 24,
            u = t / 365,
            v = b.breakpoints,
            w =
              (q < v.secondsToMinute && g(k.seconds, q)) ||
              (q < v.secondsToMinutes && g(k.minute, 1)) ||
              (r < v.minutesToHour && g(k.minutes, r)) ||
              (r < v.minutesToHours && g(k.hour, 1)) ||
              (s < v.hoursToDay && g(k.hours, s)) ||
              (s < v.hoursToDays && g(k.day, 1)) ||
              (t < v.daysToMonth && g(k.days, t)) ||
              (t < v.daysToMonths && g(k.month, 1)) ||
              (t < v.daysToYear && g(k.months, t / 30)) ||
              (u < v.yearToYears && g(k.year, 1)) ||
              g(k.years, u),
            x = void 0 === k.wordSeparator ? " " : k.wordSeparator;
          return "he_IL" === j
            ? [o, p, w].join(x).trim()
            : [o, w, p].join(x).trim();
        }),
        (c.parse = function (a) {
          ...
          if ("undefined" != typeof moment && moment.isMoment(a))
            return a.toDate();
          if (angular.isNumber(a)) return new Date(a);
          if (/^\d+$/.test(a)) return new Date(parseInt(a, 10));
          var b = (a || "").trim();
          return (
            (b = b.replace(/\.\d+/, "")),
            (b = b.replace(/-/, "/").replace(/-/, "/")),
