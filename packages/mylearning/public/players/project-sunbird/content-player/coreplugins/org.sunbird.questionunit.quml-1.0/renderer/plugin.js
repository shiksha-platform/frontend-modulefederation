...
org.ekstep.questionunit.quml.RendererPlugin =
  org.ekstep.contentrenderer.questionUnitPlugin.extend({
    ...
    preQuestionShow: function (event) {
      ...
      questionData.question = this.replaceAssetWithBaseURL(
        questionData.question
      );
      if (
        /<div\s(?:class="mathText")>(.*?)<\/div>/.test(questionData.question)
      ) {
        questionData.question = questionData.question.replace(
          /<div\s(?:class="mathText")>(.*?)<\/div>/gm,
      ...
      if (questionData.solution) {
        questionData.solution[0] = this.replaceAssetWithBaseURL(
          questionData.solution[0]
        );
        if (
          /<div\s(?:class="mathText")>(.*?)<\/div>/.test(
            questionData.solution[0]
          )
        ) {
          questionData.solution[0] = questionData.solution[0].replace(
            /<div\s(?:class="mathText")>(.*?)<\/div>/gm,
            '<span class="mathText">$1</span>'
          );
        }
      ...
      if (
        this._question.data.solution &&
        this._question.data.solution[0].length > 0
      ) {
        this._question.template =
          "<div class='sb-question-dsp-body'> \
            <div class='sb-question-header question-bg'> \
                ...
                <button  class='sb-btn sb-btn-secondary sb-btn-normal mr-0' id='answerBtn' type='button' style='display: inline-block;'>" +
          buttonLabel +
          "</button> \
            </div> \
            <div class='sb-question-content'> \
            <div class='page-section question-bg' id='question'> \
              <div class='sb-question'>Question" +
          starDiv +
          "</div>\
    ...
    postQuestionShow: function () {
      ...
      $(".mcq-options .mcq-option").on("click", function (e) {
        ...
        if (
          this.attributes.hasOwnProperty("data-simple-choice-interaction") ===
          true
        ) {
      ...
      if (
        ...
        document.getElementById("questionset").className =
    ...
    evaluateQuestion: function (event) {
      ...
      if (this._question.data.responseDeclaration) {
        ...
        if (
          key.length > 0 &&
          responseDeclaration[key[0]].correct_response.value ===
            instance.responseValueMap[key[0]]
        ) {
          correctAnswer = true;
        }
        ...
        if (this._question.data.options) {
          _.forEach(this._question.data.options, function (val) {
            ...
            if (val.answer) {
              params.push({
                answer: JSON.stringify({ correct: [index.toString()] }),
              });
            }
    ...
    replaceAssetWithBaseURL: function (questionData) {
      if (isbrowserpreview || _.isUndefined(isbrowserpreview)) {
        ...
        return questionData
          .split("/assets/")
          .join(EkstepRendererAPI.getBaseURL() + "assets/");
      } else {
        return questionData
          .split("/assets/")
          .join("file:///" + EkstepRendererAPI.getBaseURL() + "assets/");
      }
    },
